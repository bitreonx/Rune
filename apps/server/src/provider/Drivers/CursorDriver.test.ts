...
const testLayer = ServerConfig.layerTest(process.cwd(), {
  prefix: "RUNE-cursor-driver-copy-command-",
}).pipe(
  ...
  it.effect.skipIf(windowsHost)(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "RUNE-cursor-driver-" });
    ...
    Effect.gen(function* () {
      const instance = yield* CursorDriver.create({
        ...
        config: {
          ...
          binaryPath: NodePath.join(NodeOS.tmpdir(), "RUNE-cursor-missing", "cursor-agent"),
        },
      });
