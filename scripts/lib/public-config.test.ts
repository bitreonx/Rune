...
describe("loadRepoEnv", () => {
  it("does not project cloud configuration for an unconfigured clone", () => {
    ...
    expect(env.RUNE_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.RUNE_CLERK_CLI_OAUTH_CLIENT_ID).toBeUndefined();
    expect(env.VITE_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.RUNE_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.VITE_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.EXPO_PUBLIC_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.RUNE_RELAY_URL).toBeUndefined();
    expect(env.VITE_RUNE_RELAY_URL).toBeUndefined();
    expect(env.RUNE_MOBILE_OTLP_TRACES_URL).toBeUndefined();
    expect(env.RUNE_MOBILE_OTLP_TRACES_DATASET).toBeUndefined();
    expect(env.RUNE_MOBILE_OTLP_TRACES_TOKEN).toBeUndefined();
    ...
    expect(env.RUNE_RELAY_CLIENT_OTLP_TRACES_URL).toBeUndefined();
    expect(env.RUNE_RELAY_CLIENT_OTLP_TRACES_DATASET).toBeUndefined();
    expect(env.RUNE_RELAY_CLIENT_OTLP_TRACES_TOKEN).toBeUndefined();
  ...
  it("applies process, root local, and root precedence in that order", () => {
    ...
    NodeFS.writeFileSync(
      ...
      "RUNE_CLERK_PUBLISHABLE_KEY=pk_root\nRUNE_CLERK_JWT_TEMPLATE=template_root\nRUNE_CLERK_CLI_OAUTH_CLIENT_ID=oauth_root\nRUNE_RELAY_URL=https://root.example.test\n",
    );
    NodeFS.writeFileSync(
      NodePath.join(repoRoot, ".env.local"),
      "RUNE_CLERK_PUBLISHABLE_KEY=pk_local\nRUNE_CLERK_JWT_TEMPLATE=template_local\nRUNE_CLERK_CLI_OAUTH_CLIENT_ID=oauth_local\nRUNE_RELAY_URL=https://local.example.test\n",
    );

    expect(loadRepoEnv({ baseEnv: {}, repoRoot }).RUNE_RELAY_URL).toBe(
      "https://local.example.test",
    );
    expect(
      loadRepoEnv({
        baseEnv: {
          RUNE_CLERK_PUBLISHABLE_KEY: "pk_ci",
          RUNE_CLERK_JWT_TEMPLATE: "template_ci",
          RUNE_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_ci",
          RUNE_RELAY_URL: "https://ci.example.test",
        },
        repoRoot,
      }),
    ).toMatchObject({
      ...
      RUNE_RELAY_URL: "https://ci.example.test",
      VITE_RUNE_RELAY_URL: "https://ci.example.test",
    });
  });
  ...
  it("accepts legacy framework aliases as root overrides", () => {
    expect(
      resolvePublicConfig({
        ...
        RUNE_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_canonical",
        VITE_RUNE_RELAY_URL: "https://legacy.example.test",
  ...
  it("projects canonical relay client tracing values to web build aliases", () => {
    expect(
      loadRepoEnv({
        baseEnv: {
          RUNE_RELAY_CLIENT_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
          RUNE_RELAY_CLIENT_OTLP_TRACES_DATASET: "relay-client-traces",
          RUNE_RELAY_CLIENT_OTLP_TRACES_TOKEN: "relay-client-token",
        },
  ...
  it("projects canonical mobile tracing values to Expo public aliases", () => {
    expect(
      loadRepoEnv({
        baseEnv: {
          RUNE_RELAY_URL: "https://relay.example.test",
          RUNE_MOBILE_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
          RUNE_MOBILE_OTLP_TRACES_DATASET: "mobile-traces",
          RUNE_MOBILE_OTLP_TRACES_TOKEN: "mobile-token",
        },
        repoRoot: makeTemporaryDirectory(),
      }),
    ).toEqual({
      RUNE_RELAY_URL: "https://relay.example.test",
      VITE_RUNE_RELAY_URL: "https://relay.example.test",
      RUNE_MOBILE_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
      RUNE_MOBILE_OTLP_TRACES_DATASET: "mobile-traces",
      RUNE_MOBILE_OTLP_TRACES_TOKEN: "mobile-token",
      EXPO_PUBLIC_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
...
function makeTemporaryDirectory() {
  const directory = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "rune-public-config-"));
