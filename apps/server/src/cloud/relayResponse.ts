...
import { RelayProtectedError } from "@rune/contracts/relay";
...
export function relayRequestError(cause: unknown) {
  return isRelayResponseError(cause)
    ...
    : new EnvironmentHttpInternalServerError({
        message: `Could not complete the RUNE Connect relay request. ${isHttpClientError(cause) ? `The relay request failed (${cause.reason._tag}).` : "The relay returned an unexpected response."} Check this machine's network connection and relay availability, then retry.`,
      });
}
...
function recoveryHint(error: RelayProtectedError): string {
  switch (error._tag) {
    case "RelayEnvironmentLinkLimitExceededError":
      return "Unlink an unused environment in RUNE Connect, then restart RUNE Code on this machine.";
    case "RelayAuthInvalidError":
      return "Run `RUNE connect login` to check this machine's authorization. If the stored credential was revoked, sign out with `RUNE connect logout`, then run `RUNE connect` again. Restart RUNE Code after signing in.";
    case "RelayEnvironmentLinkProofExpiredError":
    case "RelayEnvironmentLinkProofInvalidError":
      return "Check this machine's date and time, update RUNE Code, then restart it.";
    default:
...
export const filterRelayResponse = Effect.fn("cloud.filter_relay_response")(function* (
  ...
  const message = Option.isSome(decoded)
    ? `RUNE Connect: ${decoded.value.message}. ${recoveryHint(decoded.value)} Trace ID: ${decoded.value.traceId}.`
    : `RUNE Connect relay returned HTTP ${response.status} without a recognized error response. Check relay access and any proxy or firewall restrictions, then restart RUNE Code.${requestId}`;
