...
// RUNE_CODEX_COLLAB_SCRIPT env var (a JSON file path) when the first turn
...
const script = JSON.parse(NodeFS.readFileSync(process.env.RUNE_CODEX_COLLAB_SCRIPT, "utf8"));
...
rl.on("line", (line) => {
  ...
  if (method === undefined && script.serverRequests?.some((request) => request.id === id)) {
    NodeFS.appendFileSync(
      `${process.env.RUNE_CODEX_COLLAB_SCRIPT}.responses`,
  ...
  if (method === "turn/interrupt") {
    ...
    NodeFS.appendFileSync(
      `${process.env.RUNE_CODEX_COLLAB_SCRIPT}.interrupts`,
