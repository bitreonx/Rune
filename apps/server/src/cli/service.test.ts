...
import { HostProcessEnvironment } from "@rune/shared/hostProcess";
...
it("reports a newer installed service and gives an exact-version repair command", () => {
  ...
  assert.include(output, "RUNE@0.0.32-nightly.1 (newer than this RUNE@0.0.31 CLI)");
  assert.include(output, "npx RUNE@0.0.32-nightly.1 service update");
  assert.notInclude(output, "npx RUNE@latest service update");
});
  ...
  it.effect.each(["install", "update"] as const)(
    ...
    (command) =>
      Effect.gen(function* () {
        ...
        const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "RUNE-service-cli-test-" });
