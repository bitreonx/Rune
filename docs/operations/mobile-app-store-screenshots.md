...
the software keyboard, and supplies deterministic RUNE Connect discovery rows to the real
...
ARM64 by default for local Apple Silicon development; CI sets `RUNE_SHOWCASE_ANDROID_ABI=x86_64` so the
...
The Pending rows use the production offline outbox and point at the real RUNE and React fixture
