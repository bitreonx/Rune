import { EnvironmentId, ProjectId, type AgentSessionProjectCandidate } from "@rune/contracts";
...
describe("groupOnboardingProjects", () => {
  it("groups clones by origin, keeps local repos separate, and folds non-git folders away", () => {
    ...
    const clone = candidate("/code/clones/rune-2", {
      git: github("pingdotgg/t3code"),
      threadCount: 13,
    ...
    expect(
      ...
      },
      {
        ...
        paths: ["/code/t3code", "/code/clones/rune-2"],
        threadCount: 92,
        lastActiveAt: "2026-08-21T12:00:00.000Z",
      },
      {
