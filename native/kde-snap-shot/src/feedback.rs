...
const OBJECT: &str = "/com/runetools/KdeCapture/Feedback";
...
#[zbus::interface(name = "com.runetools.KdeCapture.Feedback")]
impl Bridge {
...
pub(super) fn run(connection: &Connection, directory: &Path, options: &str) -> Result<()> {
    ...
    let name = format!("RUNE-capture-feedback-{}", std::process::id());
...
mod tests {
    ...
    fn only_kwin_can_read_commands_or_acknowledge_feedback() {
        ...
        let unrelated = Proxy::new(
            ...
            destination.as_str(),
            OBJECT,
            "com.runetools.KdeCapture.Feedback",
        )
        .unwrap();
