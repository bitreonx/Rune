...
describe("ssh tunnel scripts", () => {
  it("builds the remote rune runner with npx and npm fallbacks", () => {
    const script = buildRemoteRuneRunnerScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE });

    assert.include(script, "RUNE_NODE_SCRIPT_PATH=''");
    assert.include(script, 'exec rune "$@"');
    assert.include(script, "exec npx --yes 'rune@latest' \"$@\"");
    assert.include(script, "exec npm exec --yes 'rune@latest' -- \"$@\"");
    assert.include(script, "could not install 'rune@latest'");
    assert.include(script, "require_installed_rune_cli npx --yes --package 'rune@latest'");
    assert.include(script, "require_installed_rune_cli npm exec --yes --package 'rune@latest'");
    assert.include(script, "npm produced no rune executable");
    assert.include(script, 'prepend_path_if_dir "$HOME/.local/bin"');
    assert.include(script, `RUNE_NODE_ENGINE_RANGE='${TEST_NODE_ENGINE_RANGE}'`);
    ...
    assert.include(script, 'for RUNE_NODE_BIN in "$NVM_DIR"/versions/node/*/bin');
    assert.notInclude(script, "ensure $NVM_DIR/nvm.sh is available");
  });

  it("does not hard-code a remote node engine range", () => {
    const script = buildRemoteRuneRunnerScript();

    assert.include(script, "RUNE_NODE_ENGINE_RANGE=''");
    assert.notInclude(script, TEST_NODE_ENGINE_RANGE);
  });

  it("shell-quotes package specs in the remote rune runner", () => {
    const script = buildRemoteRuneRunnerScript({
      packageSpec: "rune@nightly; touch /tmp/rune-owned",
    });

    assert.include(script, "exec npx --yes 'rune@nightly; touch /tmp/rune-owned' \"$@\"");
    assert.include(script, "exec npm exec --yes 'rune@nightly; touch /tmp/rune-owned' -- \"$@\"");
    assert.include(
      script,
      "require_installed_rune_cli npx --yes --package 'rune@nightly; touch /tmp/rune-owned'",
    );
    assert.notInclude(script, "exec npx --yes rune@nightly; touch /tmp/rune-owned");
  });

  it("builds the remote rune runner with a node script override", () => {
    const script = buildRemoteRuneRunnerScript({
    ...
    assert.include(
      script,
      "RUNE_NODE_SCRIPT_PATH='/Users/julius/Development/Work/codething-mvp/apps/server/dist/bin.mjs'",
    );
    assert.include(script, 'exec node "$RUNE_NODE_SCRIPT_PATH" "$@"');
  });

  it("uses the remote rune runner for launch and pairing scripts", () => {
    const target = {
      alias: "devbox",
    ...
    assert.include(
      buildRemoteLaunchScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE }),
      `RUNE_NODE_ENGINE_RANGE='${TEST_NODE_ENGINE_RANGE}'`,
    );
    assert.include(
      buildRemoteLaunchScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE }),
    ...
    assert.include(buildRemoteLaunchScript(), "Remote RUNE server did not become ready");
    ...
    assert.include(buildRemoteLaunchScript({ packageSpec: "rune@nightly" }), "rune@nightly");
    assert.include(
    ...
    assert.include(buildRemotePairingScript(target, { packageSpec: "rune@nightly" }), "rune@nightly");
    assert.include(
          ...
          if (args.includes("sh")) {
            ...
            if (mode === "failed stop" && stopCommandCount === 1) {
              return {
                ...
                stderr: Stream.make(
                  new TextEncoder().encode("Remote RUNE server did not stop within 2 seconds.\n"),
                ),
              };
            }
        ...
        if (mode === "failed stop") {
          ...
          if (Result.isFailure(disconnected)) {
            ...
            assert.equal(
              ...
              "Remote RUNE server did not stop within 2 seconds.",
            );
          }
        } else {
