...
const decodeRawPayload = (payload: string): unknown => {
  const result = Schema.decodeUnknownOption(Schema.fromJsonString(Schema.Unknown))(payload);
...
function mapSessionRow(
  ...
  return {
    ...
    ...(row.runtimeManifestFingerprint !== null
      ...
      : {}),
    ...(row.runtimeManifestVersion !== null
      ? { runtimeManifestVersion: row.runtimeManifestVersion }
      : {}),
...
const makeProjectionSnapshotQuery = Effect.gen(function* () {
    ...
    execute: () =>
      sql`
        SELECT
          ...
          service_connection_id AS "serviceConnectionId",
          model_profile_id AS "modelProfileId",
          runtime_manifest_fingerprint AS "runtimeManifestFingerprint",
          runtime_manifest_version AS "runtimeManifestVersion",
    ...
    execute: () =>
      sql`
        SELECT
          ...
          sessions.service_connection_id AS "serviceConnectionId",
          sessions.model_profile_id AS "modelProfileId",
          sessions.runtime_manifest_fingerprint AS "runtimeManifestFingerprint",
          sessions.runtime_manifest_version AS "runtimeManifestVersion",
  ...
  const fetchRankedClaims = Effect.fn("ProjectionSnapshotQuery.fetchRankedClaims")(
    function* (input: { readonly sourceThreadId: ThreadId; readonly query: string }) {
      ...
      return rankClaims({
        query: input.query,
        activeFiles: new Set(),
        claims: rows.map(decodeClaimRow),
        now,
        bm25,
      });
    },
  ...
  const listThreadsForPicker: ProjectionSnapshotQueryShape["listThreadsForPicker"] = Effect.fn(
    ...
    `.pipe(
      Effect.mapError(toPersistenceSqlError("ProjectionSnapshotQuery.listThreadsForPicker:query")),
    );
    return {
      matches: rows.map((row) => ({
  ...
  const capsulePreview: ProjectionSnapshotQueryShape["capsulePreview"] = Effect.fn(
    ...
    let fallbackTexts: ReadonlyArray<string> = [];
    if (fitted.selected.length === 0) {
      const fallbackRows = yield* sql<{ readonly text: string }>`
        SELECT text
        FROM projection_thread_messages
        ...
        LIMIT 3
      `.pipe(
        Effect.mapError(toPersistenceSqlError("ProjectionSnapshotQuery.capsulePreview:fallback")),
      );
      fallbackTexts = fallbackRows.map((row) => row.text.trim().slice(0, 1200));
    }
    return {
      ...
      tokenEstimate:
        fitted.selected.length > 0
          ? fitted.totalTokens
          : fallbackTexts.reduce((total, text) => total + estimateTokens(text), 0),
      topClaimTexts:
        fitted.selected.length > 0
          ? fitted.selected.slice(0, 3).map((entry) => entry.digest.text)
          : fallbackTexts.slice(0, 3),
    };
  });
  ...
  const capsuleExpand: ProjectionSnapshotQueryShape["capsuleExpand"] = Effect.fn(
    ...
    `.pipe(Effect.mapError(toPersistenceSqlError("ProjectionSnapshotQuery.capsuleExpand:raw")));
    ...
    `.pipe(Effect.mapError(toPersistenceSqlError("ProjectionSnapshotQuery.capsuleExpand:message")));
