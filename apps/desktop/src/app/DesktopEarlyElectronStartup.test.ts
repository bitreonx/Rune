...
describe("DesktopEarlyElectronStartup", () => {
  ...
  it("reads the persisted linux password-store preference before Electron is ready", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: { RUNE_HOME: "/home/user/.rune-test" },
      homeDirectory: "/home/user",
      joinPath,
      readFileString: (path) => {
        assert.equal(path, "/home/user/.rune-test/userdata/desktop-settings.json");
  ...
  it("keeps implicit development state under ~/.rune/dev when RUNE_HOME is unset", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: {
        VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
      },
      ...
      joinPath,
      readFileString: (path) => {
        assert.equal(path, "/home/user/.rune/dev/desktop-settings.json");
  ...
  it("treats whitespace-only RUNE_HOME as unconfigured in development", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: {
        RUNE_HOME: "   ",
        VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
