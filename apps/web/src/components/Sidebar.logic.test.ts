...
describe("getSidebarThreadIdsToPrewarm", () => {
  it("returns only the first visible thread ids up to the prewarm limit", () => {
    expect(getSidebarThreadIdsToPrewarm(["t1", "t2", "rune"], 2)).toEqual(["t1", "t2"]);
  });
...
describe("resolveWorkingStepLabel", () => {
  it("returns the current plan step while a turn runs", () => {
    expect(
      resolveWorkingStepLabel({
        planProgress: { step: "Refactor the parser", completedSteps: 1, totalSteps: 3 },
      }),
    ).toBe("Refactor the parser");
  });

  it("returns null without plan progress", () => {
    expect(resolveWorkingStepLabel({ planProgress: null })).toBeNull();
    expect(resolveWorkingStepLabel({})).toBeNull();
  });
});
...
function makeThread(overrides: Partial<Thread> = {}): Thread {
  return {
    ...
    messages: [],
    proposedPlans: [],
    chatDiff: { files: [], computedAt: "2026-03-09T10:00:00.000Z", throughTurnCount: 0 },
    baseline: null,
    fileOwnership: [],
    createdAt: "2026-03-09T10:00:00.000Z",
    archivedAt: null,
...
describe("splitTemporarySidebarThreads", () => {
  it("splits temporary threads out of the permanent sidebar stream", () => {
    const permanent = makeThread({ id: ThreadId.make("thread-permanent") });
    const temporary = makeThread({
      id: ThreadId.make("thread-temp"),
      temporaryAt: "2026-03-09T10:05:00.000Z",
    });

    const { permanent: kept, temporary: flagged } = splitTemporarySidebarThreads([
      temporary,
      permanent,
    ]);

    expect(kept).toEqual([permanent]);
    expect(flagged).toEqual([temporary]);
  });

  it("keeps relative order inside each bucket", () => {
    const first = makeThread({ id: ThreadId.make("thread-first") });
    const second = makeThread({ id: ThreadId.make("thread-second") });
    const tempFirst = makeThread({
      id: ThreadId.make("thread-temp-1"),
      temporaryAt: "2026-03-09T10:05:00.000Z",
    });
    const tempSecond = makeThread({
      id: ThreadId.make("thread-temp-2"),
      temporaryAt: "2026-03-09T10:06:00.000Z",
    });

    const { permanent, temporary } = splitTemporarySidebarThreads([
      tempSecond,
      second,
      tempFirst,
      first,
    ...
    expect(permanent.map((thread) => thread.id)).toEqual([second.id, first.id]);
    expect(temporary.map((thread) => thread.id)).toEqual([tempSecond.id, tempFirst.id]);
  });

  it("returns everything unflagged when no threads are temporary", () => {
    const only = makeThread();
    const { permanent, temporary } = splitTemporarySidebarThreads([only]);
    expect(permanent).toEqual([only]);
    expect(temporary).toEqual([]);
  });
});

describe("temporary threads and project activity sort", () => {
  it("does not use temporary threads as scoped project activity", () => {
    const projects = [
      makeProject({
        id: ProjectId.make("project-quiet"),
        title: "Quiet project",
        updatedAt: "2026-03-09T10:01:00.000Z",
      }),
      makeProject({
        id: ProjectId.make("project-temp-only"),
        title: "Temp-only project",
        updatedAt: "2026-03-09T10:00:00.000Z",
      }),
    ];
    const threads = [
      makeThread({
        ...
        projectId: ProjectId.make("project-quiet"),
        updatedAt: "2026-03-09T10:02:00.000Z",
      }),
      makeThread({
        id: ThreadId.make("thread-temp"),
        projectId: ProjectId.make("project-temp-only"),
        updatedAt: "2026-03-09T10:30:00.000Z",
        temporaryAt: "2026-03-09T10:30:00.000Z",
      }),
    ];

    const sorted = sortScopedProjectsForSidebar(projects, threads, "updated_at");

    expect(sorted.map((project) => project.title)).toEqual(["Quiet project", "Temp-only project"]);
  });

  it("does not use temporary threads as logical project activity", () => {
    const quietProjectId = ProjectId.make("project-quiet");
    const tempOnlyProjectId = ProjectId.make("project-temp-only");
    const projects = [
      {
        ...makeProject({ id: quietProjectId, title: "Quiet project" }),
        projectKey: "logical-quiet",
        memberProjectRefs: [{ environmentId: localEnvironmentId, projectId: quietProjectId }],
      },
      {
        ...makeProject({ id: tempOnlyProjectId, title: "Temp-only project" }),
        projectKey: "logical-temp",
        memberProjectRefs: [{ environmentId: localEnvironmentId, projectId: tempOnlyProjectId }],
      },
    ];
    const threads = [
      makeThread({
      ...
      makeThread({
        projectId: tempOnlyProjectId,
        updatedAt: "2026-03-09T10:30:00.000Z",
        temporaryAt: "2026-03-09T10:30:00.000Z",
      }),
    ];

    const sorted = sortLogicalProjectsForSidebar(projects, threads, "updated_at");

    expect(sorted.map((project) => project.projectKey)).toEqual(["logical-quiet", "logical-temp"]);
  });
});
