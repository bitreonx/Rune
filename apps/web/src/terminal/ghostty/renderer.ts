...
import type { ViewportSearchRange } from "./search";
...
const DEFAULT_SEARCH_MATCH_BACKGROUND = "rgba(234, 179, 8, 0.22)";
const DEFAULT_SEARCH_ACTIVE_BACKGROUND = "rgba(234, 179, 8, 0.48)";
...
export function renderGhosttySnapshot(options: {
  ...
  /** Buffer-search highlights in viewport coordinates; drawn like selection tints. */
  readonly searchRanges?: {
    readonly ranges: readonly ViewportSearchRange[];
    readonly active: ViewportSearchRange | null;
  } | null;
  ...
  const searchRows = new Map<number, { startCol: number; endCol: number; active: boolean }[]>();
  if (options.searchRanges) {
    for (const range of options.searchRanges.ranges) {
      const rowRanges = searchRows.get(range.y) ?? [];
      rowRanges.push({ startCol: range.startCol, endCol: range.endCol, active: false });
      searchRows.set(range.y, rowRanges);
    }
    if (options.searchRanges.active) {
      const active = options.searchRanges.active;
      const rowRanges = searchRows.get(active.y) ?? [];
      // Drawn last within its row so the active match reads above its siblings.
      rowRanges.push({ startCol: active.startCol, endCol: active.endCol, active: true });
      searchRows.set(active.y, rowRanges);
    }
  }
  ...
  for (const rowIndex of rowsToDraw) {
    ...
    const rowSearchRanges = searchRows.get(rowIndex);
    if (rowSearchRanges) {
      for (const range of rowSearchRanges) {
        context.fillStyle = range.active
          ? DEFAULT_SEARCH_ACTIVE_BACKGROUND
          : DEFAULT_SEARCH_MATCH_BACKGROUND;
        context.fillRect(
          padding + range.startCol * metrics.width,
          top,
          Math.max(1, (range.endCol - range.startCol) * metrics.width),
          metrics.height,
        );
      }
    }
