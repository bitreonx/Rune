...
export function sortHomeProjectScopes(input: {
  ...
  for (const thread of input.threads) {
    ...
    // Temporary chats must not reorder scopes either.
...
export interface HomeThreadGroup {
  readonly key: string;
  readonly title: string;
  /** Null only for synthetic groups (Temporary) with no project chrome. */
...
export function buildHomeThreadGroups(input: {
  ...
  for (const thread of input.threads) {
    ...
    // Temporary chats never enter project groups: their only home presence
    // is the Temporary section at the bottom.
...
/**
 * The Temporary section at the bottom of the home list: every alive temporary
 * chat, newest activity first, with none of the project chrome. Null when
 * nothing is alive so the section vanishes entirely.
 */
export function buildTemporaryHomeThreadGroup(input: {
  readonly threads: ReadonlyArray<EnvironmentThreadShell>;
  readonly searchQuery?: string;
  readonly matchedThreadKeys?: ReadonlySet<string>;
  readonly threadSortOrder: SidebarThreadSortOrder;
}): HomeThreadGroup | null {
  const temporary = input.threads.filter((thread) => thread.temporaryAt != null);
  ...
  // The recency window never trims this section: temp chats are short-lived
  // by definition, so every alive one stays visible.
  const sortedThreads = sortThreads(matching, input.threadSortOrder);
  return {
    key: "temporary",
    title: "Temporary",
    representative: null,
