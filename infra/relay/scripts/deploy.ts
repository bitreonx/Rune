...
const publicConfigEnvEntries = (config: RelayPublicConfig) =>
  ({
    ...
    RUNE_MOBILE_OTLP_TRACES_URL: config.mobileTracingUrl,
    RUNE_MOBILE_OTLP_TRACES_DATASET: config.mobileTracingDataset,
    RUNE_MOBILE_OTLP_TRACES_TOKEN: config.mobileTracingToken,
    RUNE_RELAY_CLIENT_OTLP_TRACES_URL: config.clientTracingUrl,
    RUNE_RELAY_CLIENT_OTLP_TRACES_DATASET: config.clientTracingDataset,
    RUNE_RELAY_CLIENT_OTLP_TRACES_TOKEN: config.clientTracingToken,
  }) as const;
...
export function reconcileRootEnvRelayUrl(contents: string, relayUrl: string): string {
  return reconcileRootEnvPublicConfig(contents, {
    ...
    .split("\n")
    .filter((line) => !line.startsWith("RUNE_MOBILE_OTLP_TRACES_"))
    .filter((line) => !line.startsWith("RUNE_RELAY_CLIENT_OTLP_TRACES_"))
    .join("\n");
}
...
const readRelayPublicConfig = Effect.fn("relay.deploy.readState")(function* (stage: string) {
  ...
  const output = yield* service.getOutput({ stack: "RuneRelay", stage });
...
export const relayDeployCommand = Command.make(
  ...
  deploy,
).pipe(Command.withDescription("Deploy the RUNE relay through Alchemy."));
