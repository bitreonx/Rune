import { MAX_SCRIPT_ID_LENGTH } from "@rune/contracts";
...
describe("projectScripts helpers", () => {
  ...
  it("allows overriding runtime env values", () => {
    const env = projectScriptRuntimeEnv({
      project: { cwd: "/repo" },
      extraEnv: {
        RUNE_PROJECT_ROOT: "/custom-root",
        CUSTOM_FLAG: "1",
      },
    });

    expect(env.RUNE_PROJECT_ROOT).toBe("/custom-root");
    expect(env.CUSTOM_FLAG).toBe("1");
    expect(env.RUNE_WORKTREE_PATH).toBeUndefined();
  });
