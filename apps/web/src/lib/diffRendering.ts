...
/** The parsed file diff a workspace path belongs to, matching renames against either name. */
export function findFileDiffByPath(
  files: ReadonlyArray<FileDiffMetadata>,
  path: string,
): FileDiffMetadata | null {
  return (
    files.find(
      (file) => resolveFileDiffPath(file) === path || resolveFileDiffPreviousPath(file) === path,
    ) ?? null
  );
}
