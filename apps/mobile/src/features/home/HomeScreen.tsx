...
} from "@rune/client-runtime/state/thread-search";
import { sortPinnedThreadsByOrderKey } from "@rune/client-runtime/state/thread-sort";
...
export function HomeScreen(props: HomeScreenProps) {
  ...
  // Temporary chats sit in their own chrome-less section at the very bottom,
  // outside every project group.
  const temporaryGroup = useMemo(
    () =>
      buildTemporaryHomeThreadGroup({
        threads:
          props.selectedEnvironmentId === null
            ...
            : scopedThreads.filter(
                (thread) => thread.environmentId === props.selectedEnvironmentId,
              ),
        searchQuery: props.searchQuery,
        matchedThreadKeys,
        threadSortOrder: props.threadSortOrder,
  ...
  const listGroups = useMemo(
    () => (temporaryGroup === null ? projectGroups : [...projectGroups, temporaryGroup]),
    [projectGroups, temporaryGroup],
  );
  ...
  const listLayout = useMemo(
    ...
    [listGroups, effectiveGroupDisplayStates, hasSearchQuery],
  );
  ...
  const hasAnyThreads =
    props.threads.some(
      (thread) => thread.archivedAt === null || thread.temporaryAt != null,
    ) || props.pendingTasks.length > 0;
  // A lone Temporary section still counts as content: the full-page empty
  // state must not bury it.
  const hasResults = projectGroups.length > 0 || temporaryGroup !== null;
          ...
          <FlatList
            ...
            ListFooterComponent={
              settledShelfExpanded && threadListV2Layout.hiddenSettledCount > 0 ? (
                ...
                >
                  <Text className="text-xs font-rune-medium text-foreground-muted">
