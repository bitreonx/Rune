...
import {
  ...
  splitTemporarySidebarThreads,
  shouldCreateNewThreadInCurrentProject,
  resolveWorkingStartedAt,
  resolveWorkingStepLabel,
...
import {
  ...
  type InstanceBadgePresentation,
...
import { ThreadSidebarRail } from "./sidebar/ThreadSidebarRail";
...
function SidebarThreadTooltip({
  ...
  instanceBadge: InstanceBadgePresentation;
          ...
          {driverKind ? (
            <div className="flex min-w-0 items-center gap-2">
              <ProviderInstanceIcon
                ...
                showBadge={instanceBadge.show && providerEntry?.accentColor !== undefined}
                badgeContent="none"
                badgeClassName="h-2 min-w-2 px-0"
                iconClassName="size-3 shrink-0 grayscale opacity-60"
              />
              <div className="min-w-0 truncate text-foreground/75">
                {instanceBadge.show && providerEntry
...
/** Static semantic dots — no pulse; running agents get violet, done green. */
function subagentStatusDotClass(status: RuntimeSubagent["status"]): string {
  switch (status) {
    case "running":
    case "waiting":
      return "bg-[var(--rune-violet-strong)]";
    case "completed":
      return "bg-success";
    case "failed":
      return "bg-error";
...
/**
 * The sidebar only renders this branch for the active thread. The activity
 * projection is already the server-backed source used by the Agents panel,
 * so the folder never invents child threads or provider sessions that cannot
 * be opened. Selecting a file opens that agent's live activity surface.
 */
const SidebarSubagentBranch = memo(function SidebarSubagentBranch({
  threadRef,
}: {
    ...
    () =>
      deriveAgentPanelModel({
        agents: foldSubagentActivities(activities, {
          sessionLive: session?.status !== "stopped",
        }),
      }),
    [activities, session?.status],
  );
  const agents = useMemo(() => subagentMembers(model), [model]);

  if (agents.length === 0) {
    return null;
  }

  const listId = `sidebar-subagents-${threadRef.threadId}`;
  const openAgentActivity = (agentId: string) => {
    useRightPanelStore.getState().focusAgent(threadRef, agentId);
  };

  return (
    <li
      className="list-none pb-1 pl-4"
      data-rune-sidebar-subagent-branch
      data-rune-sidebar-section="subagents"
    >
      <button
        type="button"
        className="flex h-7 w-full cursor-pointer items-center gap-1.5 rounded-lg px-2 text-left text-[11px] font-medium text-sidebar-muted-foreground transition-colors hover:bg-sidebar-row-hover hover:text-sidebar-foreground"
        aria-controls={listId}
        aria-expanded={expanded}
        onClick={() => setExpanded((value) => !value)}
        data-rune-sidebar-row="subagents"
      >
        {expanded ? (
          <FolderOpenIcon
            aria-hidden
            className="size-3.5 shrink-0 text-[var(--rune-violet-strong)]"
          />
        ) : (
          <FolderIcon
            aria-hidden
        ...
        <span className="min-w-0 flex-1 truncate">Sub-agents</span>
        <span className="shrink-0 rounded-full bg-sidebar-control-surface px-1.5 font-mono text-[10px] leading-4 tabular-nums text-sidebar-muted-foreground/80">
          {agents.length}
        </span>
        <ChevronDownIcon
          aria-hidden
          className={cn(
            "size-3 shrink-0 transition-transform duration-[var(--rune-motion-fast)] ease-out motion-reduce:transition-none",
            expanded && "rotate-180",
          )}
        />
      </button>
      <div
        id={listId}
        className="rune-sidebar-subagent-list"
        data-rune-sidebar-subagent-list
        data-rune-sidebar-subagent-state={expanded ? "expanded" : "collapsed"}
        aria-hidden={!expanded}
      >
        <div className="min-h-0 overflow-hidden border-l border-sidebar-border/70 pl-2">
          {agents.map((agent) => (
            <button
              key={agent.id}
              type="button"
              tabIndex={expanded ? 0 : -1}
              className="group flex h-7 w-full cursor-pointer items-center gap-1.5 rounded-md px-2 text-left text-[11px] text-sidebar-muted-foreground transition-colors hover:bg-sidebar-row-hover hover:text-sidebar-foreground"
              aria-label={`Open ${agent.title} activity (${agent.status}) in the Agents panel`}
              onClick={() => openAgentActivity(agent.id)}
              data-rune-sidebar-subagent={agent.id}
            >
              <FileCode2Icon aria-hidden className="size-3 shrink-0 text-sidebar-muted-foreground/70" />
...
const SidebarThreadRow = memo(function SidebarThreadRow(props: {
  thread: SidebarThreadSummary;
  variant: "card" | "slim";
  // Slim rows are either settled (action: un-settle), snoozed (action:
  // wake), or merely quiet (seen Ready threads — action: settle). Null on
  // temporary-shelf rows: their lifecycle is Keep/Delete via the context
  // menu, never the settle machinery.
  variantAction: "settle" | "unsettle" | "unsnooze" | null;
  ...
  // The live plan step rides the shell the row already renders from; null on
  // planless turns, so those rows keep the duration-only status.
  const workingStepLabel = status === "working" ? resolveWorkingStepLabel(thread) : null;
  ...
  const topStatus =
    status === "working"
      ? {
          ...
          // full of them. The circle is the only live part — a compositor-only
          // rotation (working-spin) that never repaints — while the label
          // rests at the dim end of what the old pulse cycled through; only
          // the thread you have open gets the label at full strength.
  ...
  const instanceBadge =
    providerEntry !== null
      ? instanceBadgePresentation(providerEntry, props.providerEntryByInstanceId.values())
      : ({ show: false, content: "initials" } satisfies InstanceBadgePresentation);
  ...
  // browser. A plain click still opens RUNE's pull request view.
  const prBadge =
    prStatus && pr ? (
      <a
        href={pr.url}
        ...
        <Tooltip>
          ...
          >
            ...
            <span className="relative ml-auto flex h-6 min-w-8 shrink-0 items-center justify-end">
              ...
              {variantAction === null ? null : variantAction === "unsnooze" ? (
                    ...
                    isWokeStatus ? (
                      ...
                      >
                        {topStatus.icon === "working" ? (
                          <CircleDashedIcon aria-hidden className="size-4 shrink-0 working-spin" />
                        ) : topStatus.icon === "done" ? (
                        ...
                        {workingStepLabel ? (
                          <span className="min-w-0 max-w-28 truncate text-secondary-label">
                            · {workingStepLabel}
                          </span>
                        ) : null}
                      </span>
                ...
                {driverKind ? (
                  <span className="inline-flex shrink-0 items-center">
                    <ProviderInstanceIcon
                      ...
                      accentColor={providerEntry?.accentColor}
                      showBadge={instanceBadge.show}
                      badgeContent={instanceBadge.content}
                      {...(instanceBadge.logoUrl ? { badgeLogoUrl: instanceBadge.logoUrl } : {})}
...
export default function Sidebar() {
  ...
  // Threads on non-primary environments (RUNE Connect, hosted) resolve their
  ...
  const {
    ...
    // Temporary chats never enter the lifecycle partitions: their only
  ...
  // The Temporary shelf is collapsed by default and sits at the very bottom:
  // temp chats stay findable while they live without ever reading as inbox
  // history. Same collapsed-route exception as snoozed: a temp chat reached
  // by link keeps its row so highlight and Keep stay reachable.
  const [temporaryShelfExpanded, setTemporaryShelfExpanded] = useLocalStorage(
    TEMPORARY_SHELF_EXPANDED_KEY,
    false,
    Schema.Boolean,
  );
  const toggleTemporaryShelf = useCallback(
    () => setTemporaryShelfExpanded((value) => !value),
    [setTemporaryShelfExpanded],
  );
  const visibleTemporaryThreads = useMemo(() => {
    if (temporaryShelfExpanded) return temporaryThreads;
    if (routeThreadKey === null) return [];
    const routeThread = temporaryThreads.find(
      (thread) =>
        scopedThreadKey(scopeThreadRef(thread.environmentId, thread.id)) === routeThreadKey,
    );
    return routeThread === undefined ? [] : [routeThread];
  }, [routeThreadKey, temporaryShelfExpanded, temporaryThreads]);
      ...
      void (async () => {
        ...
        switch (clicked.value) {
          ...
          case "mark-unread":
            ...
            return;
          case "keep": {
            const result = await keepThread(threadRef);
            if (result._tag === "Failure" && !isAtomCommandInterrupted(result)) {
              const error = squashAtomCommandFailure(result);
              toastManager.add(
                stackedThreadToast({
                  type: "error",
                  title: "Failed to keep chat",
                  description: error instanceof Error ? error.message : "An error occurred.",
                }),
              );
            }
  ...
  // Minimized instead of hidden: the icon-mode collapse swaps the thread list
  // for a slim action rail that stays on screen.
  if (!isMobile && sidebarState === "collapsed") {
    return (
      <ThreadSidebarRail
        isElectron={isElectron}
        canCreateThreads={projects.length > 0}
        ...
        fixedHeader={
          ...
          >
            <div className="flex items-center gap-1">
              <div
                className="flex h-8 min-w-0 flex-1 items-center gap-2 rounded-md bg-[var(--rune-sidebar-surface-subtle)] px-2 py-1.5 text-sm font-medium text-sidebar-muted-foreground hover:bg-sidebar-row-hover hover:text-sidebar-foreground"
                data-rune-sidebar-row="search"
              >
                  ...
                  <TooltipTrigger
                    render={
                      <SidebarMenuButton
                        size="icon"
                        type="button"
                        data-rune-sidebar-row="new-thread"
                ...
                <Tooltip>
                  <TooltipTrigger
                    render={
                      <SidebarMenuButton
                        size="icon"
                        data-rune-sidebar-row="new-project"
            ...
            threadSearchResults.length > 0 ? (
              ...
              >
                <ul
                  ...
                  data-rune-sidebar-section="search-results"
            ...
            >
                ...
                {(() => {
                  const renderThreadRow = (
                    thread: EnvironmentThreadShell,
                    section: "pinned" | "active" | "snoozed" | "settled" | "temporary",
                  ...
                  if (snoozedThreads.length > 0) {
                    items.push(
                      <li
                        ...
                        data-rune-sidebar-section="snoozed"
                        className="list-none"
                      >
                        <button
                          type="button"
                          ...
                          data-rune-sidebar-row="shelf"
                          className="mb-1 mt-3 flex w-full cursor-pointer items-center gap-2 px-2.5 text-left font-mono text-[11px] tracking-[0.08em] uppercase"
                        >
                          ...
                          <ChevronDownIcon
                            aria-hidden
                            className={cn(
                              "size-3 text-sidebar-muted-foreground/70 transition-transform duration-[var(--rune-motion-fast)] ease-out motion-reduce:transition-none",
                  ...
                  if (settledThreads.length > 0) {
                    items.push(
                      <li
                        ...
                        data-thread-selection-safe
                        data-rune-sidebar-section="settled"
                        className="list-none"
                      >
                        <button
                          type="button"
                        ...
                        >
                          ...
                          <ChevronDownIcon
                            aria-hidden
                            className={cn(
                              "size-3 text-muted-foreground/50 transition-transform duration-[var(--rune-motion-fast)] ease-out motion-reduce:transition-none",
                  ...
                  // Temporary shelf: the very bottom, collapsed by default —
                  // out of history's way until it auto-deletes. The header
                  // always renders while anything is temporary (the count is
                  // the whole footprint when collapsed); vanishes at count 0
                  // like every other lifecycle section.
                  if (temporaryThreads.length > 0) {
                    items.push(
                      <li
                        key="temporary-shelf-header"
                        data-thread-selection-safe
                        data-rune-sidebar-section="temporary"
                        className="list-none"
                      >
                        <button
                          type="button"
                          onClick={toggleTemporaryShelf}
                          aria-expanded={temporaryShelfExpanded}
                          data-testid="sidebar-temporary-shelf-toggle"
                          data-rune-sidebar-row="shelf"
                          className="mb-1 mt-3 flex w-full cursor-pointer items-center gap-2 px-2.5 text-left font-mono text-[11px] tracking-[0.08em] uppercase"
                        >
                          <span className="text-xs font-medium text-muted-foreground/50">
                            {temporaryShelfExpanded
                              ? "Temporary"
                              : `Temporary (${temporaryThreads.length})`}
                          </span>
                          <span className="h-px flex-1 bg-sidebar-border/60" />
                          <ChevronDownIcon
                            aria-hidden
                            className={cn(
                              "size-3 text-muted-foreground/50 transition-transform duration-[var(--rune-motion-fast)] ease-out motion-reduce:transition-none",
                              temporaryShelfExpanded && "rotate-180",
                            )}
                          />
                        </button>
                      </li>,
                    );
                    for (const thread of visibleTemporaryThreads) {
                      items.push(renderThreadRow(thread, "temporary"));
                    }
                  }
                  return items;
                })()}
