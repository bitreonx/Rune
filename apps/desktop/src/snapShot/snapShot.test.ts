...
describe("findAccessibleWindow", () => {
  ...
  it.each(["⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏"])(
    ...
    (frame) => {
      const windows = [{ name: `${frame} rune`, bounds: captured.bounds }];

      expect(findAccessibleWindow(windows, { ...captured, title: "⠋ rune" }, "wayland")).toBe(
        windows[0],
      );
    },
  );
  ...
  it("rejects matching spinners when the window sizes differ", () => {
    expect(
      findAccessibleWindow(
        [{ name: "⠙ rune", bounds: { ...captured.bounds, width: 400 } }],
        { ...captured, title: "⠋ rune" },
        "wayland",
      ),
    ).toBeUndefined();
  ...
  it("rejects ambiguous normalized titles even if one matches the captured spinner exactly", () => {
    const windows = [
      { name: "⠋ rune", bounds: captured.bounds },
      { name: "⠙ rune", bounds: captured.bounds },
    ];

    expect(
      findAccessibleWindow(windows, { ...captured, title: "⠋ rune" }, "wayland"),
    ).toBeUndefined();
  });

  it("keeps exact title matching outside Wayland", () => {
    expect(
      findAccessibleWindow([{ name: "⠙ rune", bounds: captured.bounds }], {
        ...captured,
        title: "⠋ rune",
      }),
    ).toBeUndefined();
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* Effect.promise(async () => {
          ...
          const runtimeDirectory = await mkdtemp(join(tmpdir(), "RUNE-wayland-"));
          ...
          try {
            ...
            expect(isWaylandSession("linux", { XDG_RUNTIME_DIR: "/nonexistent-RUNE-test" })).toBe(
              false,
            );
          } finally {
            if (server.listening) {
