...
beforeEach(async () => {
  directory = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "RUNE-kde-test-"));
  paths = {
...
it("captures with verified identity, cleans private files, and activates only the owning RUNE PID/title", async () => {
  await setup.perform("install-kde-helper");
