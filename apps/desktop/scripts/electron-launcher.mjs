// This file mostly exists because we want dev mode to say "RUNE (Dev)" instead of "electron"
...
export function makeDevelopmentEnvironmentScript(environment) {
  const envEntries = [
    ...
    ["RUNE_PORT", environment.RUNE_PORT],
    ["RUNE_HOME", environment.RUNE_HOME],
    ["RUNE_COMMIT_HASH", environment.RUNE_COMMIT_HASH],
    ["RUNE_OTLP_TRACES_URL", environment.RUNE_OTLP_TRACES_URL],
    ["RUNE_OTLP_EXPORT_INTERVAL_MS", environment.RUNE_OTLP_EXPORT_INTERVAL_MS],
    ["RUNE_DESKTOP_APP_USER_MODEL_ID", APP_BUNDLE_ID],
  ...
  return [
    ...
    `exec ${shellSingleQuote(electronBinaryPath)} --rune-dev-root=${shellSingleQuote(desktopRoot)} ${shellSingleQuote(mainEntryPath)} "$@"`,
    "",
  ].join("\n");
}
...
export function resolveMacBundleInfoPlistStrings(executableName) {
  return {
    ...
    NSScreenCaptureUsageDescription:
      "RUNE Code captures the active window when you use the snapshot shortcut.",
    NSDocumentsFolderUsageDescription: "RUNE Code reads project files you open in the desktop app.",
  };
}
...
function buildMacLauncher(electronBinaryPath) {
  ...
  if (isDevelopment) {
    ...
    // Electron.app even though this bundle's Info.plist has the RUNE name.
