        ...
        <TooltipTrigger
          render={
            <SidebarTrigger
              ...
              aria-label={isSidebarVisible ? "Minimize main sidebar" : "Expand main sidebar"}
            />
          }
        />
        <TooltipPopup side="bottom">
          {isSidebarVisible ? "Minimize" : "Expand"} main sidebar
...
/** Shared body boundary for every workspace route outside the settings shell. */
export function WorkspaceRouteContent({
  ...
  return (
    <RunePageTransition routeKey={pathname} className="flex min-h-0 min-w-0 flex-1 flex-col">
      {children}
    </RunePageTransition>
  );
}
...
export function AppSidebarLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <SidebarProvider className="h-dvh! min-h-0!" defaultOpen style={sidebarProviderStyle}>
      ...
      <WorkspaceRouteContent pathname={pathname}>{children}</WorkspaceRouteContent>
      <SidebarControl />
    </SidebarProvider>
  );
}
