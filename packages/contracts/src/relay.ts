...
}).annotate({ description: "Links an authenticated cloud user to a RUNE environment." });
...
>()("@rune/contracts/relay/RelayClientPrincipal") {}
...
>()("@rune/contracts/relay/RelayEnvironmentPrincipal") {}

const RelayClientBearerAuthorization = HttpApiSecurity.http({ scheme: "bearer" }).pipe(
  HttpApiSecurity.annotate(
    OpenApi.Description,
    "Clerk session or OAuth bearer token for the signed-in RUNE Connect user.",
  ),
);
...
export const RelayPublicClientId = Schema.Literals(["rune-mobile", "rune-web"]);
export type RelayPublicClientId = typeof RelayPublicClientId.Type;
export const RelayMobileClientId = "rune-mobile" as const;
export const RelayWebClientId = "rune-web" as const;
...
export const RelayApi = HttpApi.make("RelayApi")
  ...
  .annotate(OpenApi.Title, "RUNE Relay API")
  .annotate(OpenApi.Version, "1.0.0")
  .annotate(
    OpenApi.Description,
    "Control-plane API for linking RUNE environments, connecting authorized clients, and publishing agent activity.",
  );
