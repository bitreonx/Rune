...
export interface TelemetryCounters {
  ...
  readonly allRUNE: GroupCounters;
}
...
export interface MergeProcessesResult {
  ...
  readonly groups: {
    ...
    readonly allRUNE: ResourceTelemetryAggregate;
  };
...
function applyLifecycleCounters(input: {
  ...
  let allRUNE = input.counters.allRUNE;
  for (const processDelta of input.deltas) {
    ...
    allRUNE = incrementCounters(allRUNE, processDelta);
  }
  ...
  for (const [identityKey, current] of input.current) {
    ...
    allRUNE = incrementCounters(allRUNE, { processStarts: 1 });
  }
  ...
  for (const [identityKey, previous] of input.previous) {
    ...
    allRUNE = incrementCounters(allRUNE, { processExits: 1 });
  }

  return { backend, electron, monitor, allRUNE };
}
...
export function mergeProcesses(input: MergeProcessesInput): MergeProcessesResult {
  ...
  return {
    ...
    groups: {
      ...
      allRUNE: aggregate(ordered, counters.allRUNE),
    },
    deltas: processDeltas,
  };
}
