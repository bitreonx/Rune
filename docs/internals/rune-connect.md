...
[rune-connect-auth-flow.html](./rune-connect-auth-flow.html).

## Application Keys

RUNE Connect is disabled in a fresh clone. To enable it for source builds against the production
...
```dotenv
RUNE_CLERK_PUBLISHABLE_KEY=<publishable key>
RUNE_CLERK_JWT_TEMPLATE=<JWT template name>
RUNE_CLERK_CLI_OAUTH_CLIENT_ID=<public OAuth application client ID>
RUNE_RELAY_URL=https://relay.example.com
```
...
should set `RUNE_CLERK_PUBLISHABLE_KEY`, `RUNE_CLERK_JWT_TEMPLATE`,
`RUNE_CLERK_CLI_OAUTH_CLIENT_ID`, and `RUNE_RELAY_URL` before building. EAS preview and
production builds only need the Clerk publishable key, JWT template name, and relay URL in their EAS
environment.

When any client-facing public value is absent, cloud UI is omitted. The `rune connect` command group is
...
deployment defaults.
`vp run --filter rune-relay deploy` invokes Alchemy from the relay directory, so Alchemy loads
...
The `rune connect` commands authorize a headless environment with a separate Clerk OAuth application.
...
5. Set `RUNE_CLERK_CLI_OAUTH_CLIENT_ID` in the repository-root `.env` file and release build
...
logout leaves it running; manage it with `rune service status`, `install`, `update`, and `uninstall`.
...
Local desktop development uses `rune-dev://app`, while packaged builds use `rune://app`. Add the
matching origin to each Clerk instance's Backend API `allowed_origins` array as well. The development
Clerk instance should only need `rune-dev://app`; the production Clerk instance should only need
`rune://app`. `@clerk/electron` owns the native request adapter, encrypted Clerk token persistence,
...
RUNE_MACOS_PROVISIONING_PROFILE=/absolute/path/to/rune.provisionprofile
# Optional: comma-separated override when Clerk's RP ID differs from the Frontend API hostname.
RUNE_CLERK_PASSKEY_RP_DOMAINS=example.clerk.accounts.dev,clerk.example.com
```

When `RUNE_CLERK_PASSKEY_RP_DOMAINS` is absent, the build derives the RP domain from
`RUNE_CLERK_PUBLISHABLE_KEY`. Signed macOS builds fail early if the Team ID, provisioning profile,
...
installed app executable with `VITE_DEV_SERVER_URL` and `RUNE_PORT` set. Rebuild the signed app
...
RUNE_PORT=13773 \
  "/Applications/RUNE (Alpha).app/Contents/MacOS/RUNE (Alpha)"
```
...
```sh
codesign --verify --deep --strict "/Applications/RUNE (Alpha).app"
codesign -d --entitlements :- "/Applications/RUNE (Alpha).app"
```
