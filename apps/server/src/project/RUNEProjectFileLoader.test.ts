...
const TestLayer = Layer.empty.pipe(
  Layer.provideMerge(RUNEProjectFileLoader.layer),
...
const writeProjectFile = Effect.fn("writeProjectFile")(function* (cwd: string, contents: string) {
  ...
  yield* fileSystem.writeFileString(path.join(cwd, "rune.json"), contents).pipe(Effect.orDie);
});

it.layer(TestLayer)("RUNEProjectFileLoader", (it) => {
  describe("load", () => {
    it.effect("loads and decodes a valid rune.json", () =>
      Effect.gen(function* () {
        const loader = yield* RUNEProjectFileLoader.RUNEProjectFileLoader;
    ...
    it.effect("returns none when rune.json is missing", () =>
      Effect.gen(function* () {
        const loader = yield* RUNEProjectFileLoader.RUNEProjectFileLoader;
        const cwd = yield* makeTempDir;
