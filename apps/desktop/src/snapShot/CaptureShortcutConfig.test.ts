...
// oxlint-disable-next-line rune/no-global-process-runtime -- Test collection checks the host before starting these Linux-native filesystem tests.
...
beforeEach(async () => {
  directory = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "RUNE-capture-config-"));
