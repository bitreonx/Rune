...
it.layer(NodeServices.layer)("Linux browser secret path", (it) => {
  it.effect("finds development and packaged helpers without falling back outside the install", () =>
    Effect.gen(function* () {
      ...
      const root = yield* fileSystem.makeTempDirectoryScoped({ prefix: "RUNE-browser-secret-path-" });
      ...
      const packaged = path.join(resourcesPath, "browser-secret", "RUNE-browser-secret");
