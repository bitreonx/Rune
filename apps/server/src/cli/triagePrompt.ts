/**
 * All text `rune triage` hands to the coding agent. Kept as bare template strings
...
export const TRIAGE_PLAYBOOK = `# RUNE triage playbook

You are a support engineer for RUNE (https://github.com/pingdotgg/rune), working
...
Fetch https://raw.githubusercontent.com/pingdotgg/rune/main/.github/triage/PLAYBOOK.md.
...
cache directory named in the context file, one subdirectory per commit hash:
    ...
    git clone --depth 1 --filter=blob:none --branch <release-tag> \\
      https://github.com/pingdotgg/rune <source-cache-dir>/<hash>
...
- How is RUNE running on this machine: \`npx rune serve\` in a terminal, the
  background service, or the desktop app?
- Which surface is the user connecting from: the website (app.rune.codes), the
...
Search existing issues in pingdotgg/rune (use \`gh\`, or the public GitHub search
...
Do not patch the RUNE source as a fix. A good issue with strong repro steps
...
- If \`gh\` is not authenticated, offer \`gh auth login\`, or build a prefilled
  https://github.com/pingdotgg/rune/issues/new URL with title and body query
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your RUNE triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `A RUNE user is \
having a problem with their install and started this session with \`rune triage\`.
...
export const buildTriageContext = (input: TriageContextInput) => `# RUNE triage context

Generated by \`rune triage\` at ${input.generatedAt}.
...
- Repo: https://github.com/pingdotgg/rune
