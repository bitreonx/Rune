...
projectionRepositoriesLayer("Projection repositories", (it) => {
  ...
  it.effect("round-trips the temporary flag through the thread row", () =>
    Effect.gen(function* () {
      const threads = yield* ProjectionThreadRepository;

      yield* threads.upsert({
        threadId: ThreadId.make("thread-temporary"),
        projectId: ProjectId.make("project-1"),
        ...
        snoozedUntil: null,
        snoozedAt: null,
        temporaryAt: "2026-03-24T12:00:00.000Z",
        pinnedAt: null,
        latestUserMessageAt: null,
        pendingApprovalCount: 0,
        pendingUserInputCount: 0,
      ...
      const persisted = Option.getOrNull(
        yield* threads.getById({ threadId: ThreadId.make("thread-temporary") }),
      );
      assert.strictEqual(persisted?.temporaryAt, "2026-03-24T12:00:00.000Z");

      // Keep ("un-temp") clears the flag without leaving a stale value.
      if (!persisted) {
        return yield* Effect.die("Expected temporary projection_threads row to exist.");
      }
      yield* threads.upsert({ ...persisted, temporaryAt: null });
      const cleared = Option.getOrNull(
        yield* threads.getById({ threadId: ThreadId.make("thread-temporary") }),
      );
      assert.strictEqual(cleared?.temporaryAt, null);
    }),
  );
});
