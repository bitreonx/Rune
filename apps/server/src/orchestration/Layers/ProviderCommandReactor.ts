...
// Hidden messages (the restart-recovery nudge) never reach the SQLite
// projections, so the thread-detail lookup cannot see them. They arrive on the
// domain event stream immediately before their turn-start request; this bounds
// the one-shot bridge that carries them across.
const HIDDEN_TURN_MESSAGES_MAX = 16;
...
const make = Effect.gen(function* () {
  ...
  const hiddenTurnMessages = new Map<
    MessageId,
    Extract<OrchestrationEvent, { type: "thread.message-sent" }>["payload"]
  >();
  ...
  const processTurnStartRequested = Effect.fn("processTurnStartRequested")(function* (
    ...
    // Hidden messages only exist in the event stream (they are kept out of the
    // SQLite projections), so fall back to the bridge recorded above when the
    // thread detail does not know the requested message.
    const hiddenMessage = hiddenTurnMessages.get(event.payload.messageId);
    if (hiddenMessage !== undefined && hiddenMessage.threadId === event.payload.threadId) {
      hiddenTurnMessages.delete(event.payload.messageId);
    }
    const message =
      thread.messages.find((entry) => entry.id === event.payload.messageId) ??
      (hiddenMessage !== undefined && hiddenMessage.threadId === event.payload.threadId
        ? {
            ...
            updatedAt: hiddenMessage.updatedAt,
          }
        : undefined);
  ...
  const start: ProviderCommandReactorShape["start"] = Effect.fn("start")(function* () {
    ...
    const processEvent = Effect.fn("processEvent")(function* (event: OrchestrationEvent) {
      if (event.type === "thread.message-sent" && event.payload.hidden === true) {
        if (hiddenTurnMessages.size >= HIDDEN_TURN_MESSAGES_MAX) {
          const oldest = hiddenTurnMessages.keys().next();
          if (oldest.done !== true) {
            hiddenTurnMessages.delete(oldest.value);
          }
        }
        hiddenTurnMessages.set(event.payload.messageId, event.payload);
      }
      if (
