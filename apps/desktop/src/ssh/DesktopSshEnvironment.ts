...
import * as SshAuth from "@rune/ssh/auth";
import { discoverSshHosts } from "@rune/ssh/config";
import {
  SshCommandError,
  ...
  SshReadinessError,
} from "@rune/ssh/errors";
import * as SshTunnel from "@rune/ssh/tunnel";
...
>()("@rune/desktop/ssh/DesktopSshEnvironment") {}
...
export function toSshPasswordPromptError(
  ...
  switch (cause._tag) {
    ...
    case "DesktopSshPromptPresentationError":
      message = "RUNE window is not available for SSH authentication.";
      break;
