import type { EnvironmentId, ProjectId, PullRequestListEntry } from "@rune/contracts";
...
function entry(
  ...
  return {
    ...
    url: `https://github.com/rune-dev/rune/pull/${overrides.number}`,
...
describe("the list snapshot across a reload", () => {
  ...
  const data = {
    ...
    truncated: true,
    nextCursors: { "rune-dev/rune": "cursor-1" },
  } as never;
  ...
  it("shrugs off corrupt storage and no storage at all", () => {
    const storage = makeStorage();
    storage.setItem("rune.pullRequests.list:env-1", "{not json");
    expect(readPullRequestListSnapshot(storage, "env-1")).toBeNull();
...
describe("merging the environments' own listings", () => {
  ...
  it("keeps each environment's continuation to itself", () => {
    const merged = mergePullRequestLists([
      [ENV_1, answer({ nextCursors: { "github.com rune-dev/rune": "cursor-1" } })],
      [ENV_2, answer()],
    ]);
    expect(merged?.nextCursors).toEqual({
      [ENV_1]: { "github.com rune-dev/rune": "cursor-1" },
    });
  });
...
describe("colon-namespaced labels typed as a search", () => {
  ...
  it("leaves a pasted link alone rather than naming a label after its scheme", () => {
    const parsed = parsePullRequestQuery("https://github.com/rune-dev/rune/pull/1");
    expect(parsed.filters.labels).toBeUndefined();
    expect(parsed.text).toBe("https://github.com/rune-dev/rune/pull/1");
  });
