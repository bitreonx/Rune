...
// oxlint-disable-next-line rune/no-global-process-runtime -- The native compiler targets the actual host; this script has no Effect runtime.
const hostArch = process.arch;
// oxlint-disable-next-line rune/no-global-process-runtime -- Native compilation only runs on the actual Linux host.
...
describe.skipIf(hostPlatform !== "linux")("bundled libsecret helper", () => {
  ...
  let executable;
  beforeAll(() => {
    directory = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "RUNE-browser-secret-test-"));
    executable = NodePath.join(directory, "RUNE-browser-secret");
  ...
  it("builds an executable for the requested architecture into a staged resource directory", () => {
    const output = NodePath.join(directory, "resources", "browser-secret", "RUNE-browser-secret");
