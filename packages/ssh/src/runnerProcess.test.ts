    ...
    it.live.each(["graceful", "timeout", "external"] as const)(
      ...
      (mode) =>
        Effect.gen(function* () {
          ...
          const path = yield* Path.Path;
          const spawner = yield* ChildProcessSpawner.ChildProcessSpawner;
          const fixture = yield* fs.makeTempDirectoryScoped({ prefix: "RUNE-stop-" });
  ...
  () => {
    ...
    it.live.each(cases)("handles $packageManager/$mode", ({ packageManager, mode }) =>
      Effect.gen(function* () {
        ...
        const path = yield* Path.Path;
        const spawner = yield* ChildProcessSpawner.ChildProcessSpawner;
        const fixture = yield* fs.makeTempDirectoryScoped({ prefix: "RUNE-runner-install-" });
        const bin = path.join(fixture, "bin");
        const cliPath = path.join(fixture, "installed cli.mjs");
        const callsPath = path.join(fixture, "installer-calls.jsonl");
        const packageSpec = "RUNE@0.0.39-nightly.20260905.1286";
...
`,
        ...
        yield* fs.chmod(path.join(bin, packageManager), 0o700);
        if (mode === "existing-cli") yield* fs.symlink(cliPath, path.join(bin, "RUNE"));
        ...
        if (installFailed) {
          ...
          assert.include(stderr, "npm produced no RUNE executable");
