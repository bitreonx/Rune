...
import type { RelayClientInstallProgressStage } from "@rune/contracts";
    ...
    >
      <DialogPopup className="max-w-md" showCloseButton={isConfirming}>
        <DialogHeader>
          ...
          <DialogDescription>
            {isInstalling
              ? `${APP_BASE_NAME} is preparing this environment for secure access through RUNE Connect.`
              : `${APP_BASE_NAME} needs the relay client to make this environment available through RUNE Connect.`}
          </DialogDescription>
        </DialogHeader>
          ...
          {isInstalling ? (
            <div className="space-y-2.5">
              ...
              <p className="text-xs leading-relaxed text-muted-foreground">
                Keep {APP_BASE_NAME} open while the relay client is installed.
              </p>
            </div>
          ) : (
            <div className="rounded-xl border border-border/70 bg-muted/35 p-3">
              ...
              <p className="mt-1 text-sm leading-relaxed text-muted-foreground">
                {APP_BASE_NAME} will download and install version{" "}
