...
describe("device hub proxy", () => {
  it("releases the upstream response after forwarding its body and strips tickets", async () => {
    ...
    const response = await handler(
      new Request("http://RUNE.test/api/device-hub/api/devices?wsTicket=secret"),
    );
  ...
  it("releases resources when upstream acquisition fails", async () => {
    ...
    const response = await handler(new Request("http://RUNE.test/api/device-hub/api/devices"));
  ...
  it.each(["/vendor/serve-sim/helper/ws", "/vendor/serve-emu/ws"])(
    ...
    async (path) => {
      ...
      const response = await handler(
        new Request(`http://RUNE.test/api/device-hub${path}`, { headers: { upgrade: "websocket" } }),
      );
  ...
  it("requires operate scope for stream tuning", async () => {
    ...
    const path = "http://RUNE.test/api/device-hub/vendor/serve-emu/api/stream-settings";
  ...
  it("never forwards the vendor shell endpoint", async () => {
    ...
    expect(
      (
        await handler(
          new Request("http://RUNE.test/api/device-hub/vendor/serve-sim/exec", { method: "POST" }),
        )
      ).status,
    ).toBe(404);
    expect(requests).toEqual([]);
