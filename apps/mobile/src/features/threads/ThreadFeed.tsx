...
import type { EnvironmentId, MessageId, ThreadId, TurnId } from "@rune/contracts";
import { classifyMarkdownImageSource } from "@rune/client-runtime/markdown-images";
import { CHAT_LIST_ANCHOR_OFFSET, resolveChatListAnchoredEndSpace } from "@rune/shared/chatList";
import { formatElapsed } from "@rune/shared/orchestrationTiming";
...
import { formatAttachmentSize } from "@rune/client-runtime/state/attachments";
import { squashAtomCommandFailure } from "@rune/client-runtime/state/runtime";
import {
  classifyMarkdownImageSource,
  markdownImageSourceFragment,
} from "@rune/client-runtime/markdown-images";
import { resolveViewedImageAsset } from "@rune/client-runtime/work-log/presentation";
import {
  renderCodexFileCitationsAsMarkdown,
  splitCodexArtifactTemplateMarkdown,
} from "@rune/client-runtime/codex-markdown-directives";
import { CHAT_LIST_ANCHOR_OFFSET, resolveChatListAnchoredEndSpace } from "@rune/shared/chatList";
...
import { faviconUrlForOrigin } from "@rune/shared/favicon";
import {
...
import { markdownFileIconSource } from "@rune/mobile-markdown-text/file-icons";
import { resolveMarkdownLinkPresentation } from "@rune/mobile-markdown-text/links";
import {
...
function MarkdownInlineCode(props: {
  ...
  return (
    <NativeText
      className={presentation ? "font-RUNE-bold" : "font-mono"}
  ...
  return (
    <View className="my-2 min-w-0 flex-row items-center gap-3 rounded-2xl border border-border bg-card px-3 py-3">
      ...
      <View className="min-w-0 flex-1">
        <Text className="font-RUNE-bold text-sm text-foreground" numberOfLines={1}>
      ...
      {props.onUse ? (
        ...
        >
          <Text className="font-RUNE-bold text-xs text-foreground">Use template</Text>
        </Pressable>
      ) : null}
    </View>
  );
...
function renderFeedEntry(
  ...
  if (entry.type === "turn-fold") {
    return (
      ...
      >
        <Text className="font-rune-medium text-sm tabular-nums text-foreground-muted">
          {entry.label}
        </Text>
        <ThreadDisclosureChevron
      ...
      >
        ...
        <View className="shrink-0 flex-row items-center gap-1.5">
          <SymbolView
            ...
            type="monochrome"
          />
          <Text className="font-RUNE-medium text-xs text-foreground-muted">{label}</Text>
        </View>
        <View className="h-px flex-1 bg-adaptive-neutral-200-a80-white-a8" />
      </View>
    );
      ...
      return (
        <View className="mb-5 items-end">
          ...
          <View className="mt-1 flex-row items-center justify-end gap-1 pr-0.5">
            <Text className="font-rune-medium text-xs tabular-nums text-neutral-600 dark:text-neutral-400">
              {timestampLabel}
            </Text>
  ...
  return (
    ...
    >
      <View className="max-w-[320px] items-center gap-2">
        <Text className="text-center font-rune-bold text-lg text-foreground">{props.title}</Text>
