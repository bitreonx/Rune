...
beforeEach(() => {
  useRightPanelStore.setState({ byThreadKey: {}, focusedAgentIdByThreadKey: {} });
});
...
describe("rightPanelStore", () => {
  ...
  it("focuses a child agent while opening the agents surface", () => {
    useRightPanelStore.getState().focusAgent(refA, "agent-1");

    expect(selectActiveRightPanel(useRightPanelStore.getState().byThreadKey, refA)).toBe("agents");
    expect(
      selectFocusedAgentId(useRightPanelStore.getState().focusedAgentIdByThreadKey, refA),
    ).toBe("agent-1");

    useRightPanelStore.getState().clearAgentFocus(refA);
    expect(
      selectFocusedAgentId(useRightPanelStore.getState().focusedAgentIdByThreadKey, refA),
    ).toBeNull();
  });
