...
import { VcsProcessExitError } from "@rune/contracts";
  ...
  it.effect("parses merge request view output", () =>
    Effect.gen(function* () {
        ...
        Effect.succeed(
          processOutput(
            ...
            JSON.stringify({
              ...
              web_url: "https://gitlab.com/pingdotgg/rune/-/merge_requests/42",
              target_branch: "main",
              ...
              source_project: {
                path_with_namespace: "octocat/rune",
              },
            }),
          ),
        ),
      ...
      assert.deepStrictEqual(result, {
        number: 42,
        title: "Add MR thread creation",
        url: "https://gitlab.com/pingdotgg/rune/-/merge_requests/42",
        baseRefName: "main",
        headRefName: "feature/mr-threads",
        state: "open",
        isCrossRepository: true,
        headRepositoryNameWithOwner: "octocat/rune",
            ...
            JSON.stringify([
              {
                iid: 0,
                title: "invalid",
                web_url: "https://gitlab.com/pingdotgg/rune/-/merge_requests/0",
                target_branch: "main",
              ...
              },
              {
                iid: 43,
                title: "  Valid MR  ",
                web_url: " https://gitlab.com/pingdotgg/rune/-/merge_requests/43 ",
                target_branch: " main ",
      ...
      assert.deepStrictEqual(result, [
        {
          number: 43,
          title: "Valid MR",
          url: "https://gitlab.com/pingdotgg/rune/-/merge_requests/43",
          baseRefName: "main",
    ...
    Effect.gen(function* () {
      mockedRun.mockReturnValueOnce(
        Effect.succeed(
          processOutput(
            ...
            JSON.stringify({
              path_with_namespace: "octocat/rune",
              web_url: "https://gitlab.com/octocat/rune",
              http_url_to_repo: "https://gitlab.com/octocat/rune.git",
              ssh_url_to_repo: "git@gitlab.com:octocat/rune.git",
            }),
          ),
        ),
      );
      ...
      const result = yield* Effect.gen(function* () {
        ...
        return yield* glab.getRepositoryCloneUrls({
          cwd: "/repo",
          repository: "octocat/rune",
        });
      });

      assert.deepStrictEqual(result, {
        nameWithOwner: "octocat/rune",
        url: "https://gitlab.com/octocat/rune",
        sshUrl: "git@gitlab.com:octocat/rune.git",
      });
    }),
  );
      ...
      expect(mockedRun).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          args: [
            ...
            "title=Provider MR",
            "--field",
            "description=@/tmp/rune-mr-body.md",
          ],
        }),
      );
    }),
