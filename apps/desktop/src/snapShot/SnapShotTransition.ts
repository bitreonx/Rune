...
export type SnapShotAnimationDestination = {
  ...
  /** Unit coordinates in RUNE's content area; GNOME supplies the real compositor origin. */
...
function createWindow(
  ...
  const window = new Electron.BrowserWindow({
    ...
    show: false,
    skipTaskbar: true,
    title: "RUNE Code Snapshot Animation",
    transparent: true,
    webPreferences: {
  ...
  private async showOverlays(
    ...
    try {
      ...
      if (this.waitForCompositorFrame) {
        // Cover the source with a composited snapshot before the caller reveals RUNE.
