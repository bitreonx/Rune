...
function getThreadSidebarMarkupSegment(start: string, end: string) {
  const startIndex = threadSidebarMarkup.indexOf(start);
  const endIndex = threadSidebarMarkup.indexOf(end, startIndex);

  return threadSidebarMarkup.slice(startIndex, endIndex);
}

describe("sidebar interactive cursors", () => {
  it("marks the thread sidebar's stable hierarchy hooks", () => {
    const markup = threadSidebarMarkup;
  ...
  it("keeps the snoozed shelf header in the semantic sidebar hierarchy", () => {
    const markup = getThreadSidebarMarkupSegment(
      'key="snoozed-shelf-header"',
      'key="settled-shelf-header"',
    );

    expect(markup).toContain("text-sidebar-muted-foreground");
    expect(markup).toContain("bg-sidebar-border");
    expect(markup).not.toMatch(/(?:text|bg)-blue-/);
  });

  it("uses RUNE reduced-motion-safe transforms for shelf chevrons", () => {
    const snoozedMarkup = getThreadSidebarMarkupSegment(
      'key="snoozed-shelf-header"',
      'key="settled-shelf-header"',
    );
    const settledMarkup = getThreadSidebarMarkupSegment(
      'key="settled-shelf-header"',
      "for (const thread of renderedSettledThreads)",
    );

    for (const markup of [snoozedMarkup, settledMarkup]) {
