...
>()("@rune/server/serverRuntimeStartup") {}
...
export const reconcileProviderSessions = Effect.gen(function* () {
  ...
  type AntigravityRestoreResult =
    | { readonly status: "not-applicable" }
    | { readonly status: "restored" }
    | { readonly status: "failed"; readonly errorMessage: string };

  const tryRestoreAntigravitySession = (input: {
    readonly thread: (typeof orphanedThreads)[number];
    readonly session: NonNullable<(typeof orphanedThreads)[number]["session"]>;
    readonly binding: ProviderSessionDirectory.ProviderRuntimeBinding;
  }) =>
    Effect.gen(function* () {
      if (input.binding.provider !== ("antigravity" as ProviderDriverKind)) {
        return { status: "not-applicable" } satisfies AntigravityRestoreResult;
      }
      ...
      const startInput = {
        ...
        ...(input.binding.resumeCursor !== null && input.binding.resumeCursor !== undefined
          ...
          : {}),
        ...(input.thread.modelSelection ? { modelSelection: input.thread.modelSelection } : {}),
        runtimeMode: input.binding.runtimeMode ?? input.session.runtimeMode,
      } as const;
      const result = yield* providerService
        .startSession(input.thread.id, startInput)
        .pipe(Effect.result);

      if (Result.isFailure(result)) {
        const errorMessage =
          result.failure instanceof Error ? result.failure.message : String(result.failure);
        yield* Effect.logWarning("failed to restore Antigravity session after server startup", {
          threadId: input.thread.id,
          cause: result.failure,
        });
        return { status: "failed", errorMessage } satisfies AntigravityRestoreResult;
      }

      const recovered = result.success;
      const status =
        recovered.status === "connecting"
          ? "starting"
          : recovered.status === "closed"
            ? "stopped"
      ...
      yield* orchestrationEngine.dispatch({
        ...
        session: {
          ...
          ...(recovered.providerInstanceId
            ? { providerInstanceId: recovered.providerInstanceId }
            : input.binding.providerInstanceId
              ? { providerInstanceId: input.binding.providerInstanceId }
              : {}),
          runtimeMode: recovered.runtimeMode,
          activeTurnId: null,
          lastError: recovered.lastError ?? null,
          updatedAt: restoredAt,
        },
        createdAt: restoredAt,
      });
      return { status: "restored" } satisfies AntigravityRestoreResult;
    });
  ...
  for (const thread of orphanedThreads) {
    ...
    const restoreResult = Option.isSome(bindingOption)
      ? yield* tryRestoreAntigravitySession({
          thread,
          session,
          binding: bindingOption.value,
        })
      : ({ status: "not-applicable" } satisfies AntigravityRestoreResult);
    if (restoreResult.status === "restored") {
      continue;
    }

    if (Option.isSome(bindingOption)) {
      yield* directory
        .upsert({
          ...bindingOption.value,
          status: "stopped",
        ...
        session: {
          ...
          lastError:
            restoreResult.status === "failed"
              ? `Antigravity could not be restored after startup: ${restoreResult.errorMessage}`
              : ORPHANED_PROVIDER_SESSION_ERROR,
          ...
          if (serverConfig.startupPresentation === "headless") {
            ...
            if (serverConfig.mode !== "desktop") {
              yield* Effect.logInfo(
                "Authentication required. Open RUNE using the pairing URL.",
