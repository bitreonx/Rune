...
} from "@rune/shared/git";
import { HostProcessEnvironment, HostProcessPlatform } from "@rune/shared/hostProcess";
import { normalizeProjectPathForComparison } from "@rune/shared/path";
...
>()("RUNE/project/AgentSessionScanner") {}
...
/**
 * RUNE Code runs its own agent sessions inside disposable worktrees. Their
 ...
 * worktrees directory plus the conventional `.rune/worktrees` layout, which
 * also catches sandboxes from other RUNE homes on the same machine. Separators
...
function isRuneManagedWorktree(
  ...
  return (
    ...
    normalized.includes("/.rune/worktrees/")
  );
}
...
export const make = Effect.gen(function* () {
  ...
  const isExcludedProjectPath = (candidatePath: string) =>
    ...
    isRuneManagedWorktree(candidatePath, worktreesDir, foldWorktreeCase);
