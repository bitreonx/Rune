...
/** Niri owns the keybinding; this endpoint triggers capture without first focusing RUNE. */
export async function startNiriCaptureShortcut(
  ...
  try {
    ...
    if (result !== RequestNameReply.PRIMARY_OWNER)
      throw new Error("Another RUNE Code instance already owns the capture shortcut.");
    return close;
  } catch (error) {
    close();
    throw error;
