...
export class CaptureFeedback {
  begin(sender, pid, snapshot, options) {
    ...
    try {
      ...
      this._clearActors();
      console.warn(`RUNE capture effects unavailable: ${error.message}`);
    }
  ...
  async activate(sender, title) {
    ...
    // A command-palette capture temporarily unmaps RUNE. Wait for its new surface, not a sleep.
    const target =
      find() ??
    ...
    if (this._owned(sender) !== session || !target)
      throw new Error("RUNE Code's window is not available for activation.");
