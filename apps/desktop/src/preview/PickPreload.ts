...
const OVERLAY_ATTRIBUTE = "data-rune-annotation-ui";
const Z_INDEX_OVERLAY = 2147483646;
const PRIMARY = "var(--rune-primary)";
const PRIMARY_FILL = "color-mix(in srgb, var(--rune-primary) 10%, transparent)";
...
const applyAnnotationTheme = (
  ...
  const variables = {
    "--rune-radius": theme.radius,
    "--rune-background": theme.background,
    "--rune-foreground": theme.foreground,
    "--rune-popover": theme.popover,
    "--rune-popover-foreground": theme.popoverForeground,
    "--rune-primary": theme.primary,
    "--rune-primary-foreground": theme.primaryForeground,
    "--rune-muted": theme.muted,
    "--rune-muted-foreground": theme.mutedForeground,
    "--rune-accent": theme.accent,
    "--rune-accent-foreground": theme.accentForeground,
    "--rune-border": theme.border,
    "--rune-input": theme.input,
    "--rune-ring": theme.ring,
    "--rune-font-sans": theme.fontSans,
    "--rune-font-mono": theme.fontMono,
  };
...
function startAnnotation(): void {
  ...
  const refreshToolButtons = (): void => {
    ...
    document.documentElement.setAttribute("data-rune-annotation-tool", tool);
  };
      ...
      if (isUsableRect(rect)) {
        ...
        if (found === 0) {
          ...
          const regionBox = createBox(
            PRIMARY,
            "color-mix(in srgb, var(--rune-primary) 6%, transparent)",
          );
  ...
  const teardown = (notifyMain: boolean): void => {
    ...
    document.documentElement.removeAttribute("data-rune-annotation-tool");
    cursorStyle.remove();
    host.remove();
    activeSession = null;
