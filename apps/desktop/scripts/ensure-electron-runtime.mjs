...
// oxlint-disable-next-line rune/no-global-process-runtime -- Standalone repair script has no Effect runtime.
...
function installElectronRuntime(electronDir, version) {
  ...
  try {
    ...
    if (hostPlatform === "darwin") {
      ...
      if (result.status !== 0) {
        throw new Error(
          `powershell.exe Expand-Archive failed with exit code ${result.status ?? "unknown"}`,
        );
      }
    } else {
      runChecked("python3", [
