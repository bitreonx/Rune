...
describe("brand-assets", () => {
  ...
  it("keeps development, nightly, and production icon families separate", () => {
    ...
    expect(BRAND_ASSET_PATHS.developmentDesktopIconPng).toMatch(/^assets\/dev\/rune-/);
    expect(BRAND_ASSET_PATHS.nightlyMacIconPng).toMatch(/^assets\/nightly\/rune-/);
    expect(BRAND_ASSET_PATHS.productionMacIconPng).toMatch(/^assets\/prod\/rune-/);
  });
});
