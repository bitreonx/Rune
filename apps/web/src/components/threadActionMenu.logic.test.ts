...
describe("buildThreadActionMenuItems", () => {
  ...
  it("offers Keep only on temporary threads, opening the lifecycle block", () => {
    const items = buildThreadActionMenuItems({ ...baseState, isTemporary: true });
    const keepIndex = items.findIndex((item) => item.id === "keep");
    expect(keepIndex).toBeGreaterThan(-1);
    expect(items[keepIndex]).toMatchObject({
      label: "Keep chat",
      separatorBefore: true,
    });
    expect(items[keepIndex]?.destructive).toBeFalsy();
    // Keep converts the temporary thread to permanent, so it leads the
    // keep-around block (Archive, Delete) instead of hiding inside it.
    expect(items.slice(keepIndex + 1).map((item) => item.id)).toEqual(["archive", "delete"]);
    expect(ids(baseState)).not.toContain("keep");
  });
});
