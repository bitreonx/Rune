...
beforeEach(async () => {
  directory = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "RUNE-niri-test-"));
...
it("selects the native adapter without needing a portal or GNOME extension", async () => {
  expect(await getLinuxCaptureSupport("com.runetools.Rune")).toEqual({
    linuxBackend: "niri",
    linuxFeedbackAvailable: false,
  });
  const snapshot = await captureLinuxWindow("com.runetools.Rune");
...
it("does not activate RUNE until requested, then matches PID and title", async () => {
  ...
  windows = [
    { ...window, id: 1, pid: 999, title: "RUNE Code" },
    { ...window, id: 2, pid: process.pid, title: "Other RUNE" },
    { ...window, id: 3, pid: process.pid, title: "RUNE Code" },
  ];
  await snapshot.feedback!.activate("RUNE Code");
  expect(calls).toContainEqual({ Action: { FocusWindow: { id: 3 } } });
});

it("waits for the restored RUNE window to map instead of polling", async () => {
  const snapshot = await captureNiriWindow(socketPath);
  ...
  handler = async (request, socket) => {
    ...
    if (request === "EventStream")
      send(socket, {
        WindowOpenedOrChanged: { window: { ...window, id: 4, pid: process.pid, title: "RUNE Code" } },
      });
  };
  await snapshot.feedback!.activate("RUNE Code");
...
it("rejects ambiguous activation targets", async () => {
  const snapshot = await captureNiriWindow(socketPath);
  windows = [1, 2].map((id) => ({ ...window, id, pid: process.pid, title: "RUNE Code" }));
  await expect(snapshot.feedback!.activate("RUNE Code")).rejects.toThrow("More than one");
});
...
it("cancels pending activation when capture feedback is closed", async () => {
  ...
  const activation = expect(snapshot.feedback!.activate("RUNE Code")).rejects.toThrow("cancelled");
...
it("rejects compositor errors and cleans up its temporary image", async () => {
  ...
  await expect(captureLinuxWindow("com.runetools.Rune")).rejects.toThrow("window disappeared");
  expect(await NodeFSP.stat(NodePath.dirname(capturePath!)).catch(() => undefined)).toBeUndefined();
});
