...
describe("findFileDiffByPath", () => {
  ...
  it("finds a parsed file diff by its workspace path", () => {
    const parsed = getRenderablePatch(patch);
    if (parsed?.kind !== "files") throw new Error("expected files");
    const file = findFileDiffByPath(parsed.files, "src/app.ts");
    expect(file && resolveFileDiffPath(file)).toBe("src/app.ts");
  });

  it("returns null for paths the patch does not touch", () => {
    const parsed = getRenderablePatch(patch);
    if (parsed?.kind !== "files") throw new Error("expected files");
    expect(findFileDiffByPath(parsed.files, "src/missing.ts")).toBeNull();
  });
});
