import type { ThreadPullRequestLink } from "@rune/contracts";
...
describe("resolveThreadReferenceCopyTarget", () => {
  it("does not copy another reference while the open panel URL is unavailable", () => {
    expect(
      resolveThreadReferenceCopyTarget({
        ...
        linkedPullRequestUrl: "https://github.com/RUNE/pr/12",
      }),
    ).toBeNull();
  });
  ...
  it("prefers the open panel pull request over the thread pull request", () => {
    expect(
      resolveThreadReferenceCopyTarget({
        threadId: "thread-1",
        openPanelPullRequestUrl: "https://github.com/RUNE/pr/14",
        pullRequests: [crossRepositoryPullRequest],
        linkedPullRequestUrl: "https://github.com/RUNE/pr/12",
      }),
    ).toMatchObject({
      kind: "pull-request",
      value: "https://github.com/RUNE/pr/14",
      successTitle: "PR link copied",
    });
  });

  it.each([null, "https://github.com/RUNE/pr/12"])(
  ...
  it("uses the fallback URL when native links are dismissed", () => {
    ...
    ).toMatchObject({ kind: "pull-request", value: "https://github.com/RUNE/pr/12" });
  });
  ...
  it("uses the thread pull request when no panel is open", () => {
    expect(
      ...
      kind: "pull-request",
      value: "https://github.com/RUNE/pr/12",
      successTitle: "PR link copied",
    });
  });
