...
import { isExplicitRelativePath, isWindowsAbsolutePath } from "@rune/shared/path";
import { normalizeSearchQuery } from "@rune/shared/searchRanking";

import * as WorkspacePaths from "./WorkspacePaths.ts";
import * as WorkspaceSearchIndex from "./WorkspaceSearchIndex.ts";
import * as WorkspaceTreeWalk from "./WorkspaceTreeWalk.ts";
...
export const WorkspaceEntriesError = Schema.Union([
  ...
  WorkspaceTreeWalk.WorkspaceTreeWalkError,
]);
...
>()("@rune/server/workspace/WorkspaceEntries") {}
...
export const make = Effect.gen(function* () {
  ...
  const list: WorkspaceEntries["Service"]["list"] = Effect.fn("WorkspaceEntries.list")(
    function* (input) {
      const normalizedCwd = yield* normalizeWorkspaceRoot(input.cwd);
      // The tree is a direct filesystem walk, not the search index: it must
      // include hidden and git-ignored entries, and render without waiting
      // for a full index scan. The index stays behind search and the picker.
      return yield* WorkspaceTreeWalk.workspaceTreeWalk(normalizedCwd);
    },
  );
