...
const COMING_SOON_DRIVER_OPTIONS: readonly ComingSoonDriverOption[] = [
  { value: ProviderDriverKind.make("githubCopilot"), label: "Github Copilot", icon: GithubCopilotIcon },
  { value: ProviderDriverKind.make("gemini"), label: "Gemini", icon: Gemini },
  { value: ProviderDriverKind.make("acpRegistry"), label: "ACP Registry", icon: ACPRegistryIcon },
  { value: ProviderDriverKind.make("piAgent"), label: "Pi Agent", icon: PiAgentIcon },
];
...
export function AddProviderInstanceDialog({
  ...
  const { copyToClipboard, isCopied } = useCopyToClipboard<{ command: string }>({
    ...
    onError: (error) => {
      toastManager.add({ type: "error", title: "Could not copy command", description: error.message });
    },
  });
  ...
  const setConfigDraft = (config: Record<string, unknown> | undefined) => {
    setConfigByDriver((existing) => {
      const next = { ...existing };
      if (config === undefined || Object.keys(config).length === 0) delete next[driver];
      else next[driver] = config;
      return next;
    });
  };
  const applyWizardNavigation = (navigation: WizardNavigation) => {
    if (navigation.kind === "blocked") setHasAttemptedSubmit(true);
  ...
  const handleSave = () => {
    ...
    try {
      updateSettings({
        providerInstances: {
          ...settings.providerInstances,
          [ProviderInstanceId.make(instanceId)]: nextInstance,
        },
      });
      toastManager.add({
        type: "success",
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <WizardPopup>
        ...
        <WizardPanel holdHeight>
              ...
              {COMING_SOON_DRIVER_OPTIONS.map((option) => {
                ...
                return (
                  <RadioPrimitive.Root
                    ...
                    value={option.value}
                    disabled
                    className="relative flex cursor-not-allowed items-center gap-3 rounded-lg bg-card/60 px-3 py-3 text-left opacity-55 outline-none ring-1 ring-black/5 dark:bg-white/2 dark:ring-white/5"
                  >
          ...
          <div className={cn("grid gap-4", wizardStep !== 1 && "hidden")}>
            <label className="grid gap-2">
              ...
              <Input
                ...
                onChange={(event) => {
                  ...
                  if (instanceIdOverride !== null) setInstanceIdOverride(null);
                }}
              />
            ...
            <div className="grid gap-2">
              ...
              <div className="flex min-w-0 flex-wrap items-center gap-2">
                  ...
                  {PROVIDER_ACCENT_SWATCHES.map((swatch) => (
                    <button
                      ...
                      className={cn(
                        ...
                        accentColor.toLowerCase() === swatch
                ...
                {accentColor ? (
                  <Button type="button" size="xs" variant="ghost" onClick={() => setAccentColor("")}>Clear</Button>
                ) : null}
              </div>
              <span className="text-[11px] text-muted-foreground">Optional marker shown in the picker.</span>
            </div>
          </div>
          ...
          <div className={cn("grid gap-4", wizardStep !== 2 && "hidden")}>
              ...
              <div className="grid gap-3 rounded-xl border border-primary/20 bg-primary/5 p-4 dark:bg-primary/10">
                <div className="grid gap-1">
                  ...
                  <p className="text-xs leading-5 text-muted-foreground">
                    RUNE runs <code className="rounded bg-background px-1 py-0.5">{setupGuide.binary}</code> on the machine running the server. Complete the one-time setup below, then add the instance.
                  </p>
                </div>
                {setupGuide.installCommand ? (
                  <div className="grid gap-1.5">
                    <span className="text-[11px] font-medium uppercase tracking-wide text-muted-foreground">Install once</span>
                    <div className="flex items-center gap-2 rounded-lg border border-border/70 bg-background px-2.5 py-2">
                      <code className="min-w-0 flex-1 overflow-x-auto whitespace-nowrap text-xs text-foreground">{setupGuide.installCommand}</code>
                      <Button type="button" size="sm" variant="ghost" className="h-7 shrink-0 px-2 text-xs" onClick={() => copyToClipboard(setupGuide.installCommand!, { command: setupGuide.installCommand! })}>
                ...
                {setupGuide.signInCommand ? (
                  <div className="grid gap-1.5">
                    <span className="text-[11px] font-medium uppercase tracking-wide text-muted-foreground">Sign in once</span>
                    <div className="flex items-center gap-2 rounded-lg border border-border/70 bg-background px-2.5 py-2">
                      <code className="min-w-0 flex-1 overflow-x-auto whitespace-nowrap text-xs text-foreground">{setupGuide.signInCommand}</code>
                      <Button type="button" size="sm" variant="ghost" className="h-7 shrink-0 px-2 text-xs" onClick={() => copyToClipboard(setupGuide.signInCommand!, { command: setupGuide.signInCommand! })}>
                        {isCopied ? <CheckIcon className="size-3.5" /> : <CopyIcon className="size-3.5" />}
                        {isCopied ? "Copied" : "Copy"}
                      </Button>
                    </div>
                ...
                <div className="flex flex-wrap items-center justify-between gap-2 border-t border-primary/15 pt-2 text-[11px] text-muted-foreground">
                  ...
                  {setupGuide.docsUrl ? <a href={setupGuide.docsUrl} target="_blank" rel="noreferrer" className="inline-flex shrink-0 items-center gap-1 font-medium text-primary hover:underline">Provider docs <ExternalLinkIcon className="size-3" /></a> : null}
                </div>
              </div>
            ) : null}
            {apiKeyEnvironmentVariableForDriver(driver) ? (
              <label className="grid gap-1.5">
                ...
                <Input
                  ...
                  onChange={(event) => setApiKeyByDriver((existing) => ({ ...existing, [driver]: event.target.value }))}
                  placeholder="Paste a key or leave blank to use the server environment"
                />
                <span className="text-[11px] text-muted-foreground">Stored as a sensitive instance environment value.</span>
              </label>
            ) : null}
            {driverSettingsFields.length > 0 ? (
              ...
              <p className="text-sm text-muted-foreground">This driver has no required configuration. You can add the instance now.</p>
            )}
          </div>
        </WizardPanel>
