...
export class DesktopAppSshUnsupportedError extends Schema.TaggedError<DesktopAppSshUnsupportedError>()(
  ...
  override get message(): string {
    return "`RUNE app` only controls a desktop app on the same machine. It cannot run over SSH.";
  }
}
...
export class DesktopAppPlatformUnsupportedError extends Schema.TaggedError<DesktopAppPlatformUnsupportedError>()(
  ...
  override get message(): string {
    return `\`RUNE app\` is not supported on ${this.platform}.`;
  }
}
...
export class DesktopAppUnreachableError extends Schema.TaggedError<DesktopAppUnreachableError>()(
  ...
  override get message(): string {
    return "Could not reach the RUNE Code desktop app. Start or update the desktop app on this machine, then run `RUNE app` again. A running RUNE Code server is not enough.";
  }
}
...
export class DesktopAppRequestFailedError extends Schema.TaggedError<DesktopAppRequestFailedError>()(
  ...
  override get message(): string {
    return `RUNE Code could not open ${this.workspaceRoot} (${this.code}).`;
  }
}
...
const runAppCommand = Effect.fn("cli.app")(function* (flags: {
  ...
  const configuredBaseDir = Option.getOrUndefined(flags.baseDir) ?? environment.runeHome;
  const baseDir = yield* resolveBaseDir(configuredBaseDir);
  const allowDevFallback = Option.isNone(flags.baseDir) && !environment.runeHome?.trim();
  ...
  yield* Console.log(`Opened ${workspaceRoot} in RUNE Code.`);
});
...
export const appCommand = Command.make("app", {
  ...
  Command.withDescription("Open a project in the running RUNE Code desktop app."),
