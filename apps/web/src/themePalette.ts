...
/**
 * The palette RUNE uses with no theme installed, captured from the app's
 ...
 */
const RUNE_STANDARD_LIGHT_THEME_COLORS: ThemeColors = {
  canvas: "#fcfcfc",
  chrome: "#fcfcfc",
  toolbar: "#fcfcfc",
...
const RUNE_STANDARD_DARK_THEME_COLORS: ThemeColors = {
  canvas: "#0a0a0a",
  chrome: "#0a0a0a",
  toolbar: "#0a0a0a",
  ...
  terminalCursor: "#a78bfa",
  terminalSelection: "#3e315b",
  terminalScrollbar: "#222222",
  terminalScrollbarHover: "#363636",
};

/**
 * The standard RUNE look as a theme palette, for seeding a new theme when
 * no theme is installed. Distinct from {@link getDefaultThemeColors}, which
 * carries the flagship RUNE Core palette used to fill roles omitted by theme
 * files.
 */
export function getStandardThemeColors(appearance: ThemeAppearance): ThemeColors {
  if (appearance === "dark") {
    return (standardDarkThemeColors ??= decodeThemeColors(RUNE_STANDARD_DARK_THEME_COLORS));
  }
  return (standardLightThemeColors ??= decodeThemeColors(RUNE_STANDARD_LIGHT_THEME_COLORS));
}
...
/**
 * The status colors RUNE shows without a theme, read from the app's own
...
export const RUNE_CORE_THEME: ThemeDefinition = {
  ...
  variants: {
    dark: createManagedThemeColors("dark", "#121118", "#a78bfa", {
      exactSeeds: true,
    }),
  },
  sidebarArtwork: false,
};
runeCoreTheme = RUNE_CORE_THEME;
