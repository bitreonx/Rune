...
describe("terminalStatusIndicator", () => {
  it("treats running and starting as live states with distinct tones", () => {
    expect(terminalStatusIndicator("running")).toEqual({ tone: "running", label: "Running" });
    expect(terminalStatusIndicator("starting")).toEqual({ tone: "starting", label: "Starting" });
  });

  it("groups closed and exited into one ended tone without losing the distinction", () => {
    expect(terminalStatusIndicator("closed")).toEqual({ tone: "ended", label: "Closed" });
    expect(terminalStatusIndicator("exited")).toEqual({ tone: "ended", label: "Exited" });
  });

  it("maps error sessions to the error tone", () => {
    expect(terminalStatusIndicator("error")).toEqual({ tone: "error", label: "Error" });
  });
});
