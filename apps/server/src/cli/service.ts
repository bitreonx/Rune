...
export function formatServiceStatus(
  ...
  return [
    "RUNE service",
    `  Status: ${status.current ? `installed · rune@${cliVersion}` : "needs an update or repair"}`,
    `  Unit: ${status.unitPath}`,
    `  Logs: ${status.logPath}`,
    ...(status.current ? [] : ["  Next: Run `npx rune@latest service update`."]),
  ].join("\n");
}
  ...
  Command.withHandler((flags) =>
    runServiceCommand(
      ...
      Effect.gen(function* () {
        ...
        yield* Console.log(
          `${result.previouslyInstalled ? "Updated" : "Installed"} RUNE service with rune@${packageJson.version}.\nLogs: ${result.plan.logPath}`,
        );
      }),
    ),
  ),
...
const serviceUninstallCommand = Command.make("uninstall", projectLocationFlags).pipe(
  Command.withDescription("Stop and remove the RUNE background service."),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
        ...
        yield* Console.log(
          removed ? "Removed the RUNE service." : "RUNE service is not installed.",
        );
      }),
    ),
  ),
...
const serviceStatusCommand = Command.make("status", projectLocationFlags).pipe(
  Command.withDescription("Show whether the RUNE background service is installed."),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
...
export const offerServiceDuringOnboarding = Effect.gen(function* () {
  ...
  if (installed && current) {
    yield* Console.log("RUNE is already set up to run in the background on this machine.");
    return true;
  }
  ...
  const wanted = yield* Prompt.run(
    Prompt.confirm({
      message: installed
        ? "The installed RUNE service needs an update or repair. Update it now?"
        : platform === "darwin"
          ? "Run RUNE in the background whenever you log in to this Mac? " +
            "It stays reachable through RUNE Connect while you are logged in."
          : "Run RUNE in the background whenever this machine boots? " +
            "It stays reachable through RUNE Connect even after you log out.",
      initial: true,
    }),
  );
  if (!wanted) {
...
export const serviceCommand = Command.make("service").pipe(
  Command.withDescription("Manage the RUNE background service."),
