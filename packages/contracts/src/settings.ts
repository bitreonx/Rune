...
export const AntigravitySettings = makeProviderSettingsSchema(
  {
    // Antigravity is available by default, but the server health-gates it
    // when `agy` is missing or has not been authenticated yet.
    enabled: Schema.Boolean.pipe(
      Schema.withDecodingDefault(Effect.succeed(true)),
      Schema.annotateKey({ providerSettingsForm: { hidden: true } }),
    ),
    binaryPath: makeBinaryPathSetting("agy").pipe(
      Schema.annotateKey({
        title: "Binary path",
        description: "Path to the Google Antigravity CLI binary.",
...
export const ServerSettings = Schema.Struct({
  ...
  /** Whether provider sessions may mutate RUNE-owned schedules through MCP. */
  enableAgentScheduleAccess: Schema.Boolean.pipe(Schema.withDecodingDefault(Effect.succeed(false))),
...
export const ServerSettingsPatch = Schema.Struct({
  ...
  enableAgentScheduleAccess: Schema.optionalKey(Schema.Boolean),
