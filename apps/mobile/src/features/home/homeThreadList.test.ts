...
describe("buildHomeThreadGroups", () => {
  it("builds one v2 scope for the same repository across environments", () => {
    ...
    expect(scopes[0]?.title).toBe("rune");
  ...
  it("matches web repository, repository-path, and separate grouping modes", () => {
    ...
    const projects = [
      makeProject({
        ...
        title: "Web",
        workspaceRoot: "/workspaces/rune/apps/web",
        repositoryIdentity,
      }),
      makeProject({
        environmentId,
        ...
        title: "Mobile",
        workspaceRoot: "/workspaces/rune/apps/mobile",
        repositoryIdentity,
      }),
    ];
    const threads = projects.map((project) =>
  ...
  it("does not apply the recency window while searching", () => {
    ...
    const group = buildGroups([project], threads, { searchQuery: "RUNE" })[0];
...
describe("temporary threads on home", () => {
  const localEnvironmentId = EnvironmentId.make("environment-local");
  ...
  function makeTemporaryThread(
    ...
    return makeThread({
      ...
      temporaryAt: "2026-06-28T12:00:00.000Z",
      ...input,
    });
  }

  it("excludes temporary threads from project groups", () => {
    const permanent = makeThread({
      environmentId: localEnvironmentId,
      projectId,
  ...
  it("does not use temporary threads as scope activity", () => {
    const quietProject = makeProject({
      environmentId: localEnvironmentId,
      id: ProjectId.make("project-quiet"),
      title: "Quiet",
      updatedAt: "2026-06-01T00:00:00.000Z",
    });
    const tempOnlyProject = makeProject({
      environmentId: localEnvironmentId,
      id: ProjectId.make("project-temp"),
      title: "Temp only",
      updatedAt: "2026-06-01T00:00:00.000Z",
    });
    const scopes = sortHomeProjectScopes({
      scopes: buildHomeProjectScopes({
        projects: [quietProject, tempOnlyProject],
        environmentId: null,
        projectGroupingMode: "separate",
      }),
      threads: [
        makeTemporaryThread({
          ...
          title: "Temp thread",
          projectId: ProjectId.make("project-temp"),
        }),
      ],
      pendingTasks: [],
      projectSortOrder: "updated_at",
    ...
    expect(scopes.map((scope) => scope.title)).toEqual(["Quiet", "Temp only"]);
  });

  it("builds a Temporary group newest-first, or null when none are alive", () => {
    const older = makeTemporaryThread({
      id: ThreadId.make("thread-temp-old"),
      title: "Older temp",
      updatedAt: "2026-06-27T00:00:00.000Z",
    });
    const newer = makeTemporaryThread({
      id: ThreadId.make("thread-temp-new"),
      title: "Newer temp",
      updatedAt: "2026-06-28T12:00:00.000Z",
    });
    const permanent = makeThread({
    ...
    const group = buildTemporaryHomeThreadGroup({
      threads: [older, permanent, newer],
      threadSortOrder: "updated_at",
    });
    expect(group?.key).toBe("temporary");
    expect(group?.title).toBe("Temporary");
    expect(group?.threads.map((thread) => thread.id)).toEqual([newer.id, older.id]);
    expect(group?.recentThreads.map((thread) => thread.id)).toEqual([newer.id, older.id]);
    expect(group?.representative).toBeNull();
  ...
  it("filters the Temporary group by search query like every other section", () => {
    const match = makeTemporaryThread({
      id: ThreadId.make("thread-temp-match"),
      title: "Debug flaky test",
    });
    const other = makeTemporaryThread({
      id: ThreadId.make("thread-temp-other"),
      title: "Scratch ideas",
    });
    ...
    expect(group?.threads.map((thread) => thread.id)).toEqual([match.id]);
