...
The launcher is the only runtime writer of `service-state.json`. `rune service install` and
`rune service update` may replace the launcher and state while the unit is stopped. Server children
...
1. The active server installs `rune@<target>` into a unique staging directory.
...
```sh
npx rune@<version> service update
```
