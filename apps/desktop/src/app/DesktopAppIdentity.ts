...
const AppPackageMetadata = Schema.Struct({
  runeCommitHash: Schema.optional(Schema.String),
});
...
>()("@rune/desktop/app/DesktopAppIdentity") {}
    ...
    return yield* Option.match(raw, {
      ...
      onSome: (value) =>
        decodeAppPackageMetadata(value).pipe(
          Effect.map((parsed) =>
            Option.fromNullishOr(parsed.runeCommitHash).pipe(Option.flatMap(normalizeCommitHash)),
          ),
