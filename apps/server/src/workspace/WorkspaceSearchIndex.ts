...
/**
 * Oversized result page for searches that must post-filter in the server
 * (image-only results): one big fetch instead of paging past rejected rows.
 */
const WORKSPACE_SEARCH_PAGE_SIZE = 25_002;
...
>()("@rune/server/workspace/WorkspaceSearchIndex") {}
...
export const make = Effect.fn("WorkspaceSearchIndex.make")(function* (
  ...
  const search: WorkspaceSearchIndex["Service"]["search"] = Effect.fn(
    ...
    const pageSize = imageOnly ? WORKSPACE_SEARCH_PAGE_SIZE : Math.max(1, limit + 1);
  ...
  return WorkspaceSearchIndex.of({ refresh, search, searchContents });
});
...
export class WorkspaceSearchIndexMap extends LayerMap.Service<WorkspaceSearchIndexMap>()(
  "@rune/server/workspace/WorkspaceSearchIndexMap",
  {
    lookup: layer,
