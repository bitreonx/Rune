...
This is a living glossary for RUNE. It explains what common terms mean in this codebase.
...
A conversation flagged to auto-delete after a day without activity. In [the contracts][1], a nullable `temporaryAt` marks the flag on both the thread and its shell (null = permanent). Clients arm it with the `thread.temporary.set` command and [the decider][8] stamps the timestamp from server time. The read model keeps temporary threads — clients hide them from lists, search, and activity ordering — and [TemporaryThreadSweeper.ts][25] purges them with `thread.delete` once the TTL passes. See [Temporary chats][26].
...
[24]: ./overview.md
[25]: ../../apps/server/src/orchestration/Layers/TemporaryThreadSweeper.ts
[26]: ../user/composer.md
