...
describe("streaming row projection", () => {
  ...
  it("owns checkpoint lookups across streaming and equal source snapshots", () => {
    ...
    const summary: TurnDiffSummary = {
      ...
      checkpointRef: CheckpointRef.make("refs/RUNE/checkpoints/history-turn"),
      status: "ready",
      files: [],
  ...
  it("reuses long-thread rows through detail events, selectors, and attachment previews", () => {
    ...
    const history = Array.from({ length: 250 }, (_, index) => {
      ...
      const checkpoint: TurnDiffSummary = {
        ...
        checkpointRef: CheckpointRef.make(`refs/RUNE/checkpoints/older-${index}`),
        status: "ready",
        files: [],
        get assistantMessageId() {
          checkpointLookupReads += 1;
...
describe("work entry labels", () => {
  ...
  it.each([
    ...
    const browserEntry = {
      ...entry,
      toolTitle: "RUNE-code.preview_click",
      detail: '{"ok":true}',
      toolLifecycleStatus,
    };
  ...
  it("uses the active summary state for legacy tools without a lifecycle status", () => {
    const browserEntry = { ...entry, toolTitle: "RUNE-code.preview_click" };
        ...
        timelineEntries: [
          {
            ...
            entry: {
              ...
              toolData: { server: "RUNE-code", tool },
            },
          },
        ],
        isWorking: false,
...
describe("deriveMessagesTimelineRows work-group diff stats", () => {
  const turnSummary = {
    turnId: "turn-1" as never,
    checkpointTurnCount: 3,
    checkpointRef: "checkpoint-1" as never,
    status: "ready" as const,
    files: [{ path: "src/app.ts", kind: "modified", additions: 12, deletions: 4 }],
    assistantMessageId: null,
    completedAt: "2026-01-01T00:00:30Z",
  };

  function editTimelineEntry(id: string, createdAt: string) {
    return {
      id,
      kind: "work" as const,
      createdAt,
  ...
  function deriveRows(
    workEntries: ReturnType<typeof editTimelineEntry>[],
    turnSummaries: readonly typeof turnSummary[],
  ) {
    const turnDiffSummaryByTurnId = new Map(turnSummaries.map((s) => [s.turnId, s]));
  ...
  it("attaches the group's aggregated diff stat to a collapsed tool group", () => {
    const rows = deriveRows([editTimelineEntry("work-1", "2026-01-01T00:00:20Z")], [turnSummary]);

    const toggle = rows.find((row) => row.kind === "work-toggle");
    expect(toggle?.diffStat).toEqual({ additions: 12, deletions: 4 });
  });

  it("omits the stat when the turn has no checkpoint summary", () => {
    const rows = deriveRows([editTimelineEntry("work-1", "2026-01-01T00:00:20Z")], []);

    const toggle = rows.find((row) => row.kind === "work-toggle");
    expect(toggle?.diffStat).toBeNull();
  });
});


describe("humanizeToolSummaryLabel", () => {
  it("phrases task updates as plain sentences instead of raw JSON", () => {
    expect(
      humanizeToolSummaryLabel('TaskUpdate: {"status":"completed","taskId":"3"}'),
    ).toBe("Updated task #3 · completed");
    expect(humanizeToolSummaryLabel('TaskUpdate: {"taskId":"12","status":"in_progress"}')).toBe(
      "Updated task #12 · in progress",
    );
  });

  it("phrases task creation with its subject", () => {
    expect(humanizeToolSummaryLabel('TaskCreate: {"subject":"Write design doc"}')).toBe(
      "Created task “Write design doc”",
    );
    expect(humanizeToolSummaryLabel('TaskCreate: {"activeForm":"Writing design doc"}')).toBe(
      "Created task for Writing design doc",
    );
  });

  it("phrases todo list rewrites", () => {
    expect(
      humanizeToolSummaryLabel('TodoWrite: {"todos":[{"content":"a","status":"completed"}]}'),
    ).toBe("Updated the todo list");
  });

  it("returns null for labels without parseable embedded JSON", () => {
    expect(humanizeToolSummaryLabel("Ran npm test")).toBeNull();
    expect(humanizeToolSummaryLabel("TaskUpdate: {oops")).toBeNull();
    expect(humanizeToolSummaryLabel("TaskUpdate: null")).toBeNull();
  });
});

describe("deriveTurnPlanDisplay", () => {
  function planSteps(statuses: Array<"pending" | "inProgress" | "completed">) {
    return statuses.map((status, index) => ({ key: `k${index}`, step: `Step ${index + 1}`, status }));
  }

  it("collapses a finished plan to a done summary", () => {
    const display = deriveTurnPlanDisplay(planSteps(["completed", "completed"]));
    expect(display.allDone).toBe(true);
    expect(display.completedCount).toBe(2);
    expect(display.label).toBe("All 2 steps complete");
  });

  it("leads with the active step while work remains", () => {
    const display = deriveTurnPlanDisplay(planSteps(["completed", "inProgress", "pending"]));
    expect(display.allDone).toBe(false);
    expect(display.completedCount).toBe(1);
    expect(display.label).toBe("Step 2");
  });

  it("falls back to the next pending step when nothing is active yet", () => {
    const display = deriveTurnPlanDisplay(planSteps(["pending", "pending", "pending"]));
