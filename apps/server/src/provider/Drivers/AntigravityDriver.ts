...
import { AntigravitySettings, ProviderDriverKind, ProviderSetupError } from "@rune/contracts";
    ...
    Effect.gen(function* () {
      ...
      const makeDisposableRuntime = Effect.fn("AntigravityDriver.makeDisposableRuntime")(function* (
        ...
        const cwd = yield* fileSystem
          .makeTempDirectoryScoped({ prefix: "rune-antigravity-setup-" })
          .pipe(
            Effect.mapError(
              () =>
                new ProviderSetupError({
        ...
        const runtime = yield* makeRuntime({
          cwd,
          clientInfo: { name: "rune-provider-setup", version: "0.0.0" },
          mcpServers: [],
          ...
          .withProcess(
            ...
            Effect.gen(function* () {
              const runtime = yield* makeRuntime({
                ...
                clientInfo: { name: "rune-provider-probe", version: "0.0.0" },
                mcpServers: [],
              });
      ...
      const textGeneration = yield* makeAntigravityTextGeneration({
        ...
        makeRuntime: (cwd) =>
          makeRuntime({
            cwd,
            clientInfo: { name: "rune-text", version: "0.0.0" },
            mcpServers: [],
          }),
      });
