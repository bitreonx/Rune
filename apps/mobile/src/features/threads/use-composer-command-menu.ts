import type { EnvironmentId, ProviderInteractionMode, ServerProvider } from "@rune/contracts";
import { USAGE_LIMITS_COMMAND } from "@rune/shared/usageLimits";
import {
...
export function buildComposerSlashCommandItems(input: {
  ...
  /** Whether RUNE itself offers /usage-limits for the selected provider. */
  ...
  // Providers expand commands only at the start of a message. RUNE commands
  ...
  for (const command of input.selectedProviderStatus?.slashCommands ?? []) {
    ...
    // RUNE's own limits command is answered by the thread composer; New Task has
