...
import { renderCodexDirectivesForCopy } from "@rune/client-runtime/codex-markdown-directives";
import { commandProgramName } from "@rune/client-runtime/work-log/command-label";
import {
...
} from "@rune/client-runtime/work-log/presentation";
export { normalizeCompactToolLabel, toolGroupAction };
import {
  formatDuration,
...
const TASK_STATUS_PHRASES: Record<string, string> = {
  completed: "completed",
  complete: "completed",
...
function quoteJsonString(value: unknown): string | null {
...
/**
 * Turns provider tool summaries that embed their raw JSON input
 * ("TaskUpdate: {…}") into a short phrase. Null when the label has no
 * parseable JSON payload to improve on — the raw text is then the best
 * available description.
 */
export function humanizeToolSummaryLabel(label: string): string | null {
  const separatorIndex = label.indexOf(": ");
  if (separatorIndex <= 0) return null;
  const toolName = label.slice(0, separatorIndex);
  if (!/^[\w .-]+$/u.test(toolName)) return null;

  const jsonPart = label.slice(separatorIndex + 2).trim();
...
export interface TurnPlanDisplay {
  readonly allDone: boolean;
  readonly completedCount: number;
  /** Collapsed-row label: the live step while working, a done summary when settled. */
  readonly label: string;
}

/** What a plan chip shows when folded: current step, or an all-done summary. */
export function deriveTurnPlanDisplay(
  steps: ReadonlyArray<{ step: string; status: "pending" | "inProgress" | "completed" }>,
): TurnPlanDisplay {
  ...
  const label = allDone
    ...
    : (steps.find((step) => step.status === "inProgress")?.step ??
      ...
      steps.at(-1)?.step ??
      "Plan");
  return { allDone, completedCount, label };
}
...
export function deriveMessagesTimelineRows(input: {
  ...
  turnDiffSummaryByAssistantMessageId: ReadonlyMap<MessageId, TurnDiffSummary>;
  /** Checkpoint summaries by turn, for diff stats on work-log rows. */
    ...
    if (timelineEntry.kind === "work") {
      ...
      if (visibleGroupedEntries.length > 0) {
        ...
        if (onlyToolEntries && activeInProgressToolEntries.length > 0) {
          ...
          const turnId = timelineEntry.entry.turnId;
          const turnFiles =
            (turnId != null ? input.turnDiffSummaryByTurnId?.get(turnId)?.files : undefined) ?? [];
          nextRows.push({
            kind: "work-toggle",
            ...
            diffStat: summarizeWorkGroupDiffStat(visibleGroupedEntries, turnFiles),
          });
          if (expanded) {
            for (const [entryIndex, workEntry] of visibleGroupedEntries.entries()) {
              nextRows.push({
          ...
          if (hiddenEntries.length > 0) {
            ...
            const toggleTurnId = timelineEntry.entry.turnId;
            const toggleFiles =
              (toggleTurnId != null
                ? input.turnDiffSummaryByTurnId?.get(toggleTurnId)?.files
                : undefined) ?? [];
