...
import { deepMerge } from "@rune/shared/Struct";
import { createModelCapabilities } from "@rune/shared/model";
import { applyServerSettingsPatch } from "@rune/shared/serverSettings";
...
import * as WorkspaceEntriesModule from "../../workspace/WorkspaceEntries.ts";
import * as WorkspaceFileSystemModule from "../../workspace/WorkspaceFileSystem.ts";
...
process.env.RUNE_CURSOR_ENABLED = "1";
...
// API drivers now require workspace services + ProcessRunner for the native
// agent loop; the ambient test layer satisfies that env for every registry
// build below. Tool execution never runs in these tests, so the VCS driver
// is an empty mock.
const workspaceEntriesLayer = WorkspaceEntriesModule.layer.pipe(
  Layer.provide(WorkspacePaths.layer),
);
const providerEnvLayer = Layer.empty.pipe(
  Layer.provideMerge(ServerSettingsModule.layerTest()),
  Layer.provideMerge(TestHttpClientLive),
  Layer.provideMerge(WorkspacePaths.layer),
  Layer.provideMerge(workspaceEntriesLayer),
  Layer.provideMerge(
    WorkspaceFileSystemModule.layer.pipe(
      Layer.provide(WorkspacePaths.layer),
      Layer.provide(workspaceEntriesLayer),
      Layer.provideMerge(Layer.mock(GitVcsDriverModule.GitVcsDriver)({})),
    ),
  ),
  Layer.provideMerge(ProcessRunnerModule.layer.pipe(Layer.provide(NodeServices.layer))),
  Layer.provideMerge(NodeServices.layer),
);

it.layer(providerEnvLayer)(
  "ProviderRegistry",
  (it) => {
    ...
    describe("ProviderRegistryLive", () => {
            ...
            ProviderRegistryLive.pipe(
              ...
              Layer.provideMerge(
                ServerConfig.layerTest(process.cwd(), {
                  prefix: "rune-provider-registry-background-refresh-",
                }),
              ),
              Layer.provideMerge(NodeServices.layer),
            ),
        ...
        Effect.gen(function* () {
          ...
          const runtimeServices = yield* Layer.build(
            ProviderRegistryLive.pipe(
              ...
              Layer.provideMerge(
                ServerConfig.layerTest(process.cwd(), {
                  prefix: "rune-provider-registry-merged-persist-",
                }),
              ),
          ...
          Effect.gen(function* () {
            ...
            const runtimeServices = yield* Layer.build(
              ProviderRegistryLive.pipe(
                ...
                Layer.provideMerge(
                  ServerConfig.layerTest(process.cwd(), {
                    prefix: "rune-provider-registry-opencode-authoritative-persist-",
                  }),
                ),
                Layer.provideMerge(NodeServices.layer),
              ),
        ...
        Effect.gen(function* () {
          ...
          const runtimeServices = yield* Layer.build(
            ProviderRegistryLive.pipe(
              ...
              Layer.provideMerge(
                ServerConfig.layerTest(process.cwd(), {
                  prefix: "rune-provider-registry-refresh-failure-",
                }),
              ),
              Layer.provideMerge(BackgroundPolicyAlwaysRunLayer),
              Layer.provideMerge(NodeServices.layer),
        ...
        Effect.gen(function* () {
          ...
          const runtimeServices = yield* Layer.build(
            ProviderRegistryLive.pipe(
              ...
              Layer.provideMerge(
                ServerConfig.layerTest(process.cwd(), {
                  prefix: "rune-provider-registry-sync-failure-",
                }),
              ),
              Layer.provideMerge(BackgroundPolicyAlwaysRunLayer),
              Layer.provideMerge(NodeServices.layer),
      ...
      it.effect("propagates real Codex probe failures to the aggregator at boot", () =>
        Effect.gen(function* () {
          const missingBinary = `rune_codex_missing_`;
          const serverSettings = yield* makeMutableServerSettingsService(
            decodeServerSettings(
              deepMerge(encodedDefaultServerSettings, {
                ...
                providerInstances: {
                  // Matches the shape the user had in `.rune/dev/settings.json`
      ...
      it.effect("re-probes when settings change the codex binaryPath", () =>
        Effect.gen(function* () {
          const firstMissing = `rune_codex_first_`;
          const secondMissing = `rune_codex_second_`;
          ...
          Effect.gen(function* () {
            ...
            yield* Effect.gen(function* () {
              ...
              assert.strictEqual(
                ...
                "Cursor is disabled in RUNE settings.",
              );
      ...
      it.effect("skips codex probes entirely when the provider is disabled", () =>
        Effect.gen(function* () {
          ...
          assert.strictEqual(status.installed, false);
          assert.strictEqual(status.message, "Codex is disabled in RUNE settings.");
        }),
      );
    });
    ...
    describe("checkClaudeProviderStatus", () => {
      ...
      it.effect("runs Claude status probes with the configured CLAUDE_CONFIG_DIR", () => {
        const claudeConfigDir = "/tmp/runecode-claude-home";
