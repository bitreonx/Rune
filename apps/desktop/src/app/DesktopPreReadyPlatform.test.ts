    ...
    it.effect(
      ...
      () => {
        ...
        writeFileSyncMock.mockImplementation((path: string, contents: string) => {
          if (path === "/xdg/applications/rune.desktop") desktopEntry = contents;
        });
        ...
        assert.equal(identity.desktopName, "rune.desktop");
            assert.include(identity.desktopEntry ?? "", 'Exec="/Applications/current.AppImage" %U');
            assert.include(identity.desktopEntry ?? "", "Name=RUNE Code (Alpha)");
            assert.include(identity.desktopEntry ?? "", "MimeType=x-scheme-handler/rune;");
          }),
    ...
    () =>
      Effect.gen(function* () {
        class ClerkShaped extends Context.Service<ClerkShaped, { readonly ready: true }>()(
          "@rune/desktop/app/DesktopPreReadyPlatform.test/ClerkShaped",
        ) {}
