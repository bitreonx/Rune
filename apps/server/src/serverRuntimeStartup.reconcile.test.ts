...
it.effect(
  "restores an Antigravity orphan from its durable cursor without the restart error",
  () => {
    const thread = makeThread("antigravity-restart", "running");
    const antigravity = ProviderDriverKind.make("antigravity");
    const antigravityInstanceId = ProviderInstanceId.make("antigravity");
    const antigravityThread = {
      ...thread,
      session: {
        ...thread.session,
    ...
    const resumeCursor = { schemaVersion: 1, conversationId: "agy-conversation-1" };
    const starts: Array<unknown> = [];
    const dispatched: OrchestrationCommand[] = [];

    return runReconciliation({
      threads: [antigravityThread as unknown as ReturnType<typeof makeThread>],
      providerService: makeProviderService([], (threadId, input) => {
      ...
      Effect.tap(() =>
        Effect.sync(() => {
          ...
          if (command?.type === "thread.session.set") {
            assert.equal(command.session.status, "starting");
...
it.effect("uses a provider-specific error when Antigravity recovery cannot start", () => {
  const thread = makeThread("antigravity-restart-failed", "running");
  const antigravity = ProviderDriverKind.make("antigravity");
  const antigravityInstanceId = ProviderInstanceId.make("antigravity");
  const antigravityThread = {
    ...thread,
  ...
  const dispatched: OrchestrationCommand[] = [];
  const genericRestartError =
    "Provider session did not survive a server restart. Send a new message to continue.";

  return runReconciliation({
    threads: [antigravityThread as unknown as ReturnType<typeof makeThread>],
    providerService: makeProviderService([], (threadId, _input) =>
      Effect.fail(
        new ProviderAdapterProcessError({
          provider: antigravity,
          threadId,
          detail: "agy executable is unavailable",
        }),
      ),
    ),
    directory: {
      getBinding: () =>
        Effect.succeed(
          Option.some({
            ...
            runtimeMode: "full-access" as const,
            status: "running" as const,
            resumeCursor: { schemaVersion: 1, conversationId: "agy-conversation-1" },
          }),
        ),
      upsert: () => Effect.void,
      getProvider: () => Effect.die("unused"),
    ...
    Effect.tap(() =>
      Effect.sync(() => {
        ...
        assert.equal(command?.type, "thread.session.set");
        if (command?.type === "thread.session.set") {
          assert.notEqual(command.session.lastError, genericRestartError);
          assert.match(command.session.lastError ?? "", /Antigravity could not be restored/);
        }
      }),
    ),
  );
