...
import { sanitizeBranchFragment } from "@rune/shared/git";
import {
  detectSourceControlProviderFromRemoteUrl,
  isSshRemoteUrl,
} from "@rune/shared/sourceControl";
...
const BitbucketApiEnvConfig = Config.all({
  baseUrl: Config.string("RUNE_BITBUCKET_API_BASE_URL").pipe(
    Config.withDefault(DEFAULT_API_BASE_URL),
  ),
  accessToken: Config.string("RUNE_BITBUCKET_ACCESS_TOKEN").pipe(Config.option),
  email: Config.string("RUNE_BITBUCKET_EMAIL").pipe(Config.option),
  apiToken: Config.string("RUNE_BITBUCKET_API_TOKEN").pipe(Config.option),
});
...
>()("@rune/server/sourceControl/BitbucketApi") {}
...
function checkoutBranchName(input: {
  ...
  return `rune/pr-${input.pullRequestId}/${sanitizeBranchFragment(input.headBranch)}`;
}
...
function authFromConfig(
  ...
  return {
    ...
    host: Option.some("bitbucket.org"),
    detail: Option.some(
      "Set RUNE_BITBUCKET_EMAIL and RUNE_BITBUCKET_API_TOKEN, or RUNE_BITBUCKET_ACCESS_TOKEN.",
    ),
  };
}
