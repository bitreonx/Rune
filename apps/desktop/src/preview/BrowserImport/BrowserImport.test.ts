...
const withImporter = Effect.fnUntraced(function* () {
  ...
  const home = yield* fileSystem.makeTempDirectoryScoped({ prefix: "rune-import-" });
  ...
  const importer = yield* BrowserImport.BrowserImport.pipe(
    Effect.provide(
      BrowserImport.layer.pipe(
        ...
        Layer.provide(Layer.succeed(HostProcessExecutablePath, "/Applications/RUNE Code.app")),
    ...
    Effect.gen(function* () {
      ...
      const error = yield* importer
        .importCookies({
          ...
          scope: "persist:rune-preview-test",
          persistent: true,
        })
        .pipe(Effect.flip);
