...
import type { CursorSettings } from "@rune/contracts";
...
const cursorAcpDiscoveryFailedMessage = [
  ...
  "Cursor CLI setup may be incomplete; install or enable the Cursor CLI, restart RUNE, and try again.",
  "See https://cursor.com/docs/cli/installation.",
  "Check server logs for ACP details.",
].join(" ");
const missingCursorBinaryPath = "/definitely/not/installed/rune-cursor-agent";
const cursorCliCommandMissingMessage = [
  `Cursor CLI command \`${missingCursorBinaryPath}\` was not found.`,
  `Install or enable the Cursor CLI, make sure \`${missingCursorBinaryPath}\` is on PATH, then restart RUNE.`,
  "See https://cursor.com/docs/cli/installation.",
].join(" ");
