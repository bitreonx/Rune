...
import { ThreadId, type EnvironmentId, type ScopedThreadRef } from "@rune/contracts";
...
export const PULL_REQUESTS_PANEL_REF: ScopedThreadRef = {
  environmentId: "pull-requests-panel" as EnvironmentId,
...
const RIGHT_PANEL_STORAGE_KEY = "rune:right-panel-state:v2";
...
interface RightPanelStoreState {
  ...
  /** Ephemeral focus for the activity inspector; never persisted as panel layout state. */
  focusedAgentIdByThreadKey: Record<string, string>;
  open: (
    ref: ScopedThreadRef,
    kind: Exclude<RightPanelKind, "file" | "terminal" | "pull-request">,
  ) => void;
  focusAgent: (ref: ScopedThreadRef, agentId: string) => void;
  clearAgentFocus: (ref: ScopedThreadRef) => void;
    ...
    (set, get) => ({
      ...
      removeThread: (ref) =>
        set((state) => {
          ...
          const hasAgentFocus = threadKey in state.focusedAgentIdByThreadKey;
          if (!hasPanelState && !hasAgentFocus) return state;
          const { [threadKey]: _removed, ...rest } = state.byThreadKey;
          const { [threadKey]: _removedFocus, ...restFocus } = state.focusedAgentIdByThreadKey;
          return { byThreadKey: rest, focusedAgentIdByThreadKey: restFocus };
        }),
    }),
    {
...
export function selectFocusedAgentId(
  focusedAgentIdByThreadKey: Record<string, string>,
  ref: ScopedThreadRef | null | undefined,
): string | null {
  if (!ref) return null;
  return focusedAgentIdByThreadKey[scopedThreadKey(ref)] ?? null;
}
