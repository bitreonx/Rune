  ...
  Layer.mergeAll(
    Layer.succeed(
      ...
      DesktopEnvironment.DesktopEnvironment.of({
        platform,
        stateDir: "/state",
        linuxDesktopEntryName: "rune.desktop",
        appRoot: "/repo",
...
it.effect.each(["win32", "darwin", "linux"] as const)(
  ...
  (platform) => {
    ...
    const RUNE = {
      id: 42,
      title: "RUNE Code",
      appIdentifier: "rune.desktop",
      owner: { name: "RUNE Code", processId: 123 },
      bounds,
    ...
    macCaptureMock.mockReset().mockImplementation(async () => {
      images.push(RUNE.png);
      return { source: { name: RUNE.title }, png: RUNE.png };
    });
    ...
    linuxCaptureMock.mockResolvedValueOnce({
      ...
      window: {
        title: RUNE.title,
        appName: RUNE.owner.name,
        appIdentifier: RUNE.appIdentifier,
        processId: RUNE.owner.processId,
        bounds,
      },
      feedback: {
    ...
    return Effect.scoped(
      Effect.gen(function* () {
        ...
        const saved = yield* decodePendingMetadata(metadata);
        assert.equal(saved.source.windowTitle, RUNE.title);
        assert.equal(saved.source.appName, RUNE.owner.name);
        assert.equal(saved.source.accessibleText, `Window from process ${RUNE.owner.processId}`);
        assert.deepEqual(images, [RUNE.png]);
        assert.equal(prepareCaptureRevealMock.mock.calls.length, platform === "win32" ? 1 : 0);
        if (platform === "linux") {
          assert.equal(saved.source.appIdentifier, RUNE.appIdentifier);
          assert.deepEqual(activate.mock.calls, [[RUNE.title]]);
        }
      }),
    ).pipe(
      Effect.provide(
      ...
      Effect.gen(function* () {
        ...
        const warning = logs.find(
          (message) =>
            ...
            message[0] === "The compositor could not activate RUNE Code after the snapshot",
        );
...
it.each([
  { names: ["⠙ rune"], expected: "Verified text" },
  { names: ["⠋ rune", "⠙ rune"], expected: undefined },
...
it.effect("flags revoked macOS permissions on read and re-registers once they return", () => {
  ...
  return Effect.scoped(
    Effect.gen(function* () {
      ...
      assert.equal(
        revoked.message,
        "Allow Screen Recording in System Settings, then restart RUNE Code.",
      );
...
for (const fails of [false, true]) {
  it.effect(`tests macOS capture without publishing it and cleans up, failure=${fails}`, () => {
    const active = {
      ...
      id: 42,
      title: "Setup",
      owner: { name: "RUNE Code", processId: 123, path: "/Applications/RUNE Code.app" },
      bounds: { x: 0, y: 0, width: 800, height: 600 },
    };
    activeWindowMock.mockReset().mockResolvedValue(active);
