...
export default mergeConfig(
  ...
  defineConfig({
    ...
    pack: {
      ...
      define: {
        __RUNE_BUILD_CHANNEL__: JSON.stringify(cliBuildChannel),
        __RUNE_BUILD_RELAY_URL__: JSON.stringify(repoEnv.RUNE_RELAY_URL?.trim() ?? ""),
        __RUNE_BUILD_CLERK_PUBLISHABLE_KEY__: JSON.stringify(
          repoEnv.RUNE_CLERK_PUBLISHABLE_KEY?.trim() ?? "",
        ),
        __RUNE_BUILD_CLERK_CLI_OAUTH_CLIENT_ID__: JSON.stringify(
          repoEnv.RUNE_CLERK_CLI_OAUTH_CLIENT_ID?.trim() ?? "",
        ),
        __RUNE_BUILD_RELAY_CLIENT_OTLP_TRACES_URL__: JSON.stringify(
          repoEnv.RUNE_RELAY_CLIENT_OTLP_TRACES_URL?.trim() ?? "",
        ),
        __RUNE_BUILD_RELAY_CLIENT_OTLP_TRACES_DATASET__: JSON.stringify(
          repoEnv.RUNE_RELAY_CLIENT_OTLP_TRACES_DATASET?.trim() ?? "",
        ),
        __RUNE_BUILD_RELAY_CLIENT_OTLP_TRACES_TOKEN__: JSON.stringify(
          repoEnv.RUNE_RELAY_CLIENT_OTLP_TRACES_TOKEN?.trim() ?? "",
        ),
      },
    },
    test: {
