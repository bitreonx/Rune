...
import { providerInstanceInitials } from "../../providerInstances";
...
export const ProviderInstanceIcon = memo(function ProviderInstanceIcon(props: {
  ...
  showBadge?: boolean;
  /**
   * `"initials"` renders the instance-name initials, `"logo"` renders the
   * artwork supplied via `badgeLogoUrl` (e.g. the OpenRouter mark for a
   * gateway-backed Claude), `"none"` renders an empty pill.
   */
  badgeContent?: "initials" | "logo" | "none";
      ...
      {props.showBadge ? (
        <span
          className={cn(
            ...
            badgeContent === "logo" || !props.accentColor
              ? "bg-card"
        ...
        >
          ...
          {badgeContent === "logo" && props.badgeLogoUrl ? (
            <img
              src={props.badgeLogoUrl}
              alt=""
              draggable={false}
              className="size-2.5 rounded-[3px] object-cover"
            />
          ) : null}
        </span>
      ) : null}
