...
describe("ProviderRuntimeIngestion", () => {
  ...
  it("flushes buffered assistant text on a cadence instead of holding it to completion", async () => {
    const harness = await createHarness();
    ...
    const emitDelta = (eventId: string, delta: string, offsetMs: number) =>
      harness.emit({
        ...
        threadId: asThreadId("thread-1"),
        turnId: asTurnId("turn-cadence"),
        itemId: asItemId("item-cadence"),
        payload: {
    ...
    emitDelta("evt-cadence-1", "hello", 0);
    await harness.drain();

    // First visible chunk lands immediately; the message is still streaming.
    const partialThread = await waitForThread(harness.readModel, (entry) =>
      entry.messages.some(
        (message: ProviderRuntimeTestMessage) =>
          message.id === "assistant:item-cadence" && message.text === "hello",
      ),
    );
    const partialMessage = partialThread.messages.find(
      (entry: ProviderRuntimeTestMessage) => entry.id === "assistant:item-cadence",
    );
    expect(partialMessage?.streaming).toBe(true);

    // Inside the cadence window nothing new is dispatched yet.
    emitDelta("evt-cadence-2", " world", 100);
    await harness.drain();
    const heldThread = await harness.readModel();
    const heldMessage = heldThread.threads
      .find((entry) => entry.id === asThreadId("thread-1"))
      ?.messages.find((entry: ProviderRuntimeTestMessage) => entry.id === "assistant:item-cadence");
    expect(heldMessage?.text).toBe("hello");

    // Past the window the accumulated text flushes.
    emitDelta("evt-cadence-3", "!", 600);
    await harness.drain();
    const grownThread = await waitForThread(harness.readModel, (entry) =>
      entry.messages.some(
        (message: ProviderRuntimeTestMessage) =>
          message.id === "assistant:item-cadence" && message.text === "hello world!",
      ),
    );
    expect(
      grownThread.messages.find(
        (entry: ProviderRuntimeTestMessage) => entry.id === "assistant:item-cadence",
      )?.streaming,
    ).toBe(true);

    // Completion still finalizes exactly once with no text loss.
    harness.emit({
      type: "item.completed",
      eventId: asEventId("evt-cadence-completed"),
      provider: ProviderDriverKind.make("codex"),
      createdAt: at(700),
      threadId: asThreadId("thread-1"),
      turnId: asTurnId("turn-cadence"),
    ...
    const finalThread = await waitForThread(harness.readModel, (entry) =>
      entry.messages.some(
        (message: ProviderRuntimeTestMessage) =>
          message.id === "assistant:item-cadence" && !message.streaming,
      ),
    );
  ...
  it("shows the first buffered delta immediately while the turn is still running", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";
    ...
    await harness.drain();
    const midThread = await waitForThread(harness.readModel, (entry) =>
      entry.messages.some(
        (message: ProviderRuntimeTestMessage) =>
          message.id === "assistant:item-buffered" && message.text === "buffer me",
      ),
    );
