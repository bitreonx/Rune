...
import { HostProcessEnvironment, HostProcessPlatform } from "@rune/shared/hostProcess";
...
import {
  ...
  type DpopPublicJwk,
} from "@rune/shared/dpop";
import { RELAY_HEALTH_REQUEST_TYP, RELAY_MINT_REQUEST_TYP } from "@rune/shared/relayJwt";
...
import * as SkillRegistry from "./provider/SkillRegistry.ts";
  ...
  Effect.gen(function* () {
    ...
    yield* Layer.build(
      OtlpTracer.layer({
        ...
        resource: {
          serviceName: "rune-web",
          attributes: {
            "service.runtime": "rune-web",
...
const buildAppUnderTest = (options?: {
  ...
  layers?: {
    ...
    >;
    projectActionExecutor?: Partial<ProjectActionExecutor.ProjectActionExecutor["Service"]>;
    pocketStore?: Partial<PocketStore.PocketStore["Service"]>;
    promptQueue?: Partial<PromptQueueService.PromptQueueService["Service"]>;
    actionRegistry?: Partial<ActionRegistry.ActionRegistry["Service"]>;
    chatMutationLedger?: Partial<ChatMutationLedger.ChatMutationLedger["Service"]>;
    planSession?: Partial<PlanSession.PlanSession["Service"]>;
  ...
  Effect.gen(function* () {
    ...
    const tempBaseDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "rune-router-test-" });
      ...
      .pipe(
        ...
        Layer.provide(
          Layer.mergeAll(
            ...
            Layer.mock(PocketStore.PocketStore)({
              snapshot: () =>
                Effect.succeed({
                  revision: 0,
                  pockets: [],
              ...
              ...options?.layers?.pocketStore,
            }),
            Layer.mock(PromptQueueService.PromptQueueService)({
              snapshot: () => Effect.die("Prompt queue is not stubbed in this test"),
              dispatch: () => Effect.die("Prompt queue is not stubbed in this test"),
              ...options?.layers?.promptQueue,
            }),
            Layer.mock(ActionRegistry.ActionRegistry)({
              list: () => Effect.succeed({ actions: [] }),
              create: () => Effect.die("Action registry create is not stubbed in this test"),
              version: () => Effect.die("Action registry version is not stubbed in this test"),
              getVersion: () => Effect.succeed(null),
              createProposal: () =>
                Effect.die("Action proposal creation is not stubbed in this test"),
              listProposals: () => Effect.succeed({ proposals: [] }),
              approveProposal: () =>
                Effect.die("Action proposal approval is not stubbed in this test"),
              rejectProposal: () =>
                Effect.die("Action proposal rejection is not stubbed in this test"),
              dismissProposal: () =>
                Effect.die("Action proposal dismissal is not stubbed in this test"),
              recordRun: () => Effect.void,
              settleRun: () => Effect.void,
              listRunHistory: () => Effect.succeed({ runs: [] }),
              ...options?.layers?.actionRegistry,
            }),
            Layer.mock(ChatMutationLedger.ChatMutationLedger)({
              append: () => Effect.die("Mutation append is not stubbed in this test"),
              list: () => Effect.succeed({ operations: [] }),
              settle: () => Effect.die("Mutation settle is not stubbed in this test"),
              ...options?.layers?.chatMutationLedger,
            }),
            Layer.mock(PlanSession.PlanSession)({
              create: () => Effect.die("Plan session create is not stubbed in this test"),
              get: () => Effect.die("Plan session get is not stubbed in this test"),
              update: () => Effect.die("Plan session update is not stubbed in this test"),
              transition: () => Effect.die("Plan session transition is not stubbed in this test"),
              resume: () => Effect.die("Plan session resume is not stubbed in this test"),
              ...options?.layers?.planSession,
            }),
          ),
        ),
        Layer.provide(
        ...
        Layer.provide(
          Layer.mock(ProjectActionExecutor.ProjectActionExecutor)({
            runForThread: (_input) =>
              Effect.succeed({
                status: "no-script" as const,
                actionId: ActionId.make("test-action"),
                actionVersion: 1,
              }),
            ...options?.layers?.projectActionExecutor,
          }),
        ),
        Layer.provide(
          Layer.mock(SkillRegistry.SkillRegistry)({
            list: Effect.succeed({ version: 0, skills: [] }),
            refresh: Effect.succeed({ version: 0, skills: [] }),
            getBody: () => Effect.die("SkillRegistry body lookup is not stubbed in this test"),
            bridge: {
              compile: () =>
                Effect.succeed({ skills: [], sourceDirectories: [], compiledPrompt: "" }),
            },
          }),
        ),
        Layer.provide(
          Layer.mock(WorkspaceFileWatcher.WorkspaceFileWatcher)({
    ...
    const appLayer = servedRoutesLayer.pipe(
      ...
      Layer.provide(
        Layer.mock(CloudCliTokenManager.CloudCliTokenManager)({
          get: Effect.die(new Error("Unexpected RUNE Connect CLI authorization request.")),
...
const makeCloudMintCredentialRequest = (input: {
  ...
  const payload = {
    ...
    aud: input.audience ?? `rune-env:${input.environmentId}`,
...
it.layer(NodeServices.layer)("server router seam", (it) => {
  it.effect("parks HTTP ingress until command readiness", () =>
    Effect.gen(function* () {
      ...
      const staticDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "rune-router-gate-" });
  ...
  it.effect("revalidates static files without sending unchanged bodies", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const staticDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "RUNE-static-cache-" });
  ...
  it.effect("serves changed HTML with the same size and timestamp", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const staticDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "RUNE-static-html-" });
      const indexPath = path.join(staticDir, "index.html");
  ...
  it.effect("caches hashed static assets without freezing mutable files or SPA fallbacks", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const staticDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "RUNE-static-hashes-" });
  ...
  it.effect("binds static metadata and bytes to one file across atomic replacement", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const staticDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "RUNE-static-replace-" });
  ...
  it.effect("closes static file handles after GET, HEAD, 304, and request cancellation", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const staticDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "RUNE-static-close-" });
  ...
  it.effect("serves the public environment descriptor without requiring auth", () =>
    Effect.gen(function* () {
      ...
      const url = yield* getHttpServerUrl("/.well-known/rune/environment");
      ...
      Effect.gen(function* () {
        ...
        const tokenResponse = yield* fetchEffect(tokenUrl, {
          ...
          body: new URLSearchParams({
            ...
            subject_token_type: "urn:rune:params:oauth:token-type:environment-bootstrap",
      ...
      const linkProofResponse = yield* fetchEffect(linkProofUrl, {
        ...
        body: jsonRequestBody({
          ...
          endpoint: {
            ...
            // "manual" and "cloudflare_tunnel" are supported; "rune_relay" is not.
            providerKind: "rune_relay",
          },
          origin: {
    ...
    () =>
      Effect.gen(function* () {
        const installedRelayClient = {
          ...
          executablePath: "/tmp/rune/tools/cloudflared",
  ...
  it.effect("serves the documented RUNE Connect mint credential endpoint", () =>
    Effect.gen(function* () {
      yield* buildAppUnderTest();
      ...
      const mintUrl = yield* getHttpServerUrl("/api/rune-connect/mint-credential");
  ...
  it.effect("serves signed RUNE Connect environment health checks", () =>
    Effect.gen(function* () {
      yield* buildAppUnderTest();
      ...
      const healthUrl = yield* getHttpServerUrl("/api/rune-connect/health");
    ...
    Effect.gen(function* () {
      ...
      const wrongAudience = yield* postMint(
        makeCloudMintCredentialRequest({
          ...
          environmentId: testEnvironmentDescriptor.environmentId,
          clientProofKeyThumbprint: "client-proof-key-thumbprint",
          audience: "rune-env:other-environment",
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        assert.equal(record.resourceAttributes["service.name"], "rune-web");
  ...
  it.effect("routes actions.run through the canonical project action executor", () =>
    Effect.gen(function* () {
      const runForThread = vi.fn((input: ProjectActionExecutor.ProjectActionExecutorInput) =>
        Effect.succeed({
              ...
              Effect.succeed(
                Option.some(
                  makeDefaultOrchestrationThreadShell({
                    worktreePath: "/tmp/default-worktree",
                  }),
                ),
              ),
      ...
      const executorInput = runForThread.mock.calls[0]?.[0];
      assert.equal(executorInput?.threadId, "thread-default");
      assert.equal(executorInput?.projectId, "project-default");
      assert.equal(executorInput?.projectCwd, "/tmp/default-project");
      assert.equal(executorInput?.worktreePath, "/tmp/default-worktree");
      assert.equal(executorInput?.action.scope, "project");
      assert.equal(executorInput?.action.steps[0]?.command, "pnpm test");
    }).pipe(Effect.provide(NodeHttpServer.layerTest)),
  );

  it.effect("returns a structured action error for an unregistered project action", () =>
    Effect.gen(function* () {
      yield* buildAppUnderTest({
        layers: {
          projectionSnapshotQuery: {
      ...
      assert.equal(error._tag, "ActionRunError");
      if (error._tag === "ActionRunError") {
        assert.equal(error.code, "action-not-found");
        assert.equal(error.actionId, "action.missing");
      }
    }).pipe(Effect.provide(NodeHttpServer.layerTest)),
  );
  ...
  it.effect("rejects websocket rpc handshake when session authentication is missing", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({ prefix: "rune-ws-auth-required-" });
  ...
  it.effect("routes websocket rpc projects.searchEntries", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({ prefix: "rune-ws-project-search-" });
      yield* fs.writeFileString(
        path.join(workspaceDir, "needle-file.ts"),
        "export const needle = 1;",
      );
  ...
  it.effect("routes websocket rpc projects.listEntries and projects.readFile", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({ prefix: "rune-ws-project-files-" });
  ...
  it.effect("routes websocket rpc projects.searchEntries excludes gitignored files", () =>
    Effect.gen(function* () {
      ...
      const workspaceDir = yield* fs.makeTempDirectoryScoped({
        prefix: "rune-ws-project-search-gitignored-",
      });
  ...
  it.effect.skipIf(!symlinksSupported)("preserves structured workspace rpc failures", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({
        prefix: "rune-ws-workspace-errors-",
      });
      const outsideDir = yield* fs.makeTempDirectoryScoped({
        prefix: "rune-ws-workspace-errors-outside-",
      });
  ...
  it.effect("reports workspace root stat failures without relabeling them as missing", () =>
    Effect.gen(function* () {
      ...
      const blockedRoot = yield* fs.makeTempDirectoryScoped({
        prefix: "rune-ws-workspace-stat-error-",
      });
  ...
  it.effect("routes websocket rpc projects.writeFile", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({ prefix: "rune-ws-project-write-" });
  ...
  it.effect("creates a missing workspace root during websocket project.create dispatch", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const parentDir = yield* fs.makeTempDirectoryScoped({ prefix: "rune-ws-project-create-" });
      ...
      Effect.gen(function* () {
        ...
        const refreshStatus = vi.fn((_: string) =>
          Effect.succeed({
            ...
            hasPrimaryRemote: true,
            isDefaultRef: false,
            refName: "rune/bootstrap-refName",
            hasWorkingTreeChanges: false,
            workingTree: {
              files: [],
              insertions: 0,
        ...
        assert.deepEqual(createWorktree.mock.calls[0]?.[0], {
          ...
          newRefName: "rune/bootstrap-refName",
          baseRefName: "main",
          path: null,
        });
...
it.live(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const reportPath = yield* Config.string("RUNE_TRANSFER_BUDGET_REPORT_PATH").pipe(
        Config.option,
      );
      ...
      const resultPath = yield* Config.string("RUNE_TRANSFER_BUDGET_RESULT_PATH").pipe(
        Config.option,
      );
