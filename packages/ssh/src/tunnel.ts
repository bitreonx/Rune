import type { DesktopSshEnvironmentBootstrap, DesktopSshEnvironmentTarget } from "@rune/contracts";
import {
  describeReadinessCause,
  waitForHttpReady as waitForHttpReadyShared,
} from "@rune/shared/httpReadiness";
import * as NetService from "@rune/shared/Net";
import { extractJsonObject, fromLenientJson } from "@rune/shared/schemaJson";
import { satisfiesSemverRange } from "@rune/shared/semver";
...
export interface RemoteRuneRunnerOptions {
...
export interface SshEnvironmentManagerOptions {
  ...
  readonly resolveCliRunner?: Effect.Effect<RemoteRuneRunnerOptions>;
}
...
function sshRunnerLogFields(runner: RemoteRuneRunnerOptions | undefined) {
...
remote_node_satisfies_engine() {
  RUNE_NODE_ENGINE_RANGE=@@RUNE_NODE_ENGINE_RANGE@@
  if [ -z "$RUNE_NODE_ENGINE_RANGE" ]; then
    return 0
  fi
  node - "$RUNE_NODE_ENGINE_RANGE" <<'NODE'
@@RUNE_NODE_ENGINE_CHECK_SCRIPT@@
NODE
}
...
ensure_remote_node_path() {
  ...
  if ! command -v node >/dev/null 2>&1 && [ -d "$NVM_DIR/versions/node" ]; then
    for RUNE_NODE_BIN in "$NVM_DIR"/versions/node/*/bin; do
      if [ -x "$RUNE_NODE_BIN/node" ]; then
        PATH="$RUNE_NODE_BIN:$PATH"
        export PATH
      fi
    done
  fi
...
set -eu
@@RUNE_NODE_ENV_SCRIPT@@
ensure_remote_node_path || true
RUNE_NODE_SCRIPT_PATH=@@RUNE_NODE_SCRIPT_PATH@@
if [ -n "$RUNE_NODE_SCRIPT_PATH" ]; then
  ...
  exec node "$RUNE_NODE_SCRIPT_PATH" "$@"
fi
if command -v rune >/dev/null 2>&1; then
  exec rune "$@"
fi
# npm extracts a package before it runs the native builds of its dependencies,
# so a failed build (rune depends on node-pty, which needs a C toolchain) leaves
# the npx cache without a rune executable. \`npx --yes\` then exits 0 without
...
require_installed_rune_cli() {
  RUNE_CLI_PATH="$("$@" -- sh -c 'command -v rune' || true)"
  if [ -n "$RUNE_CLI_PATH" ]; then
    return 0
  fi
  printf 'Remote host installed %s but npm produced no rune executable, which usually means a native dependency (node-pty) failed to build. Install a C toolchain on the remote host (Debian/Ubuntu: build-essential, Fedora/RHEL: gcc-c++ make, macOS: xcode-select --install) and try again.\\n' @@RUNE_PACKAGE_SPEC@@ >&2
  return 1
}
if command -v npx >/dev/null 2>&1; then
  require_installed_rune_cli npx --yes --package @@RUNE_PACKAGE_SPEC@@ || exit 1
  exec npx --yes @@RUNE_PACKAGE_SPEC@@ "$@"
fi
if command -v npm >/dev/null 2>&1; then
  require_installed_rune_cli npm exec --yes --package @@RUNE_PACKAGE_SPEC@@ || exit 1
  exec npm exec --yes @@RUNE_PACKAGE_SPEC@@ -- "$@"
fi
printf 'Remote host is missing the rune CLI and could not install @@RUNE_PACKAGE_SPEC@@ because node/npm/npx are unavailable on PATH. Install Node or configure a supported version manager for non-interactive shells.\\n' >&2
exit 1
`;
...
@@RUNE_NODE_ENV_SCRIPT@@
STATE_KEY="$1"
STATE_DIR="$HOME/.rune/ssh-launch/$STATE_KEY"
DEFAULT_SERVER_HOME="$HOME/.rune"
DEFAULT_RUNTIME_FILE="$DEFAULT_SERVER_HOME/userdata/server-runtime.json"
PORT_FILE="$STATE_DIR/port"
PID_FILE="$STATE_DIR/pid"
MANAGED_FILE="$STATE_DIR/managed"
LOG_FILE="$STATE_DIR/server.log"
RUNNER_FILE="$STATE_DIR/run-rune.sh"
RUNNER_NEXT="$STATE_DIR/run-rune.next.$$"
mkdir -p "$STATE_DIR"
cleanup_runner_next() {
  rm -f "$RUNNER_NEXT"
}
...
pick_port() {
  node - "$PORT_FILE" "@@RUNE_DEFAULT_REMOTE_PORT@@" "@@RUNE_REMOTE_PORT_SCAN_WINDOW@@" <<'NODE'
@@RUNE_PICK_PORT_SCRIPT@@
NODE
}
wait_ready() {
  node - "$REMOTE_PORT" "$1" "@@RUNE_READY_PROBE_TIMEOUT_MS@@" <<'NODE'
@@RUNE_WAIT_READY_SCRIPT@@
NODE
}
wait_for_pid_exit() {
  PID_TO_WAIT="$1"
...
if [ -n "$DEFAULT_REMOTE_PORT" ]; then
  ...
  if wait_ready "@@RUNE_REUSE_READY_TIMEOUT_MS@@"; then
...
if [ "$REMOTE_MANAGED" = "external" ]; then
  if [ -z "$REMOTE_PORT" ] || ! wait_ready "@@RUNE_REUSE_READY_TIMEOUT_MS@@"; then
    REMOTE_PID=""
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  fi
  ...
  if [ "$RUNNER_CHANGED" -eq 1 ]; then
    ...
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  elif ! wait_ready "@@RUNE_REUSE_READY_TIMEOUT_MS@@"; then
    kill "$REMOTE_PID" 2>/dev/null || true
    wait_for_pid_exit "$REMOTE_PID"
    REMOTE_PID=""
    REMOTE_PORT=""
...
if [ -z "$REMOTE_PORT" ]; then
  ...
  nohup env RUNE_NO_BROWSER=1 "$RUNNER_FILE" serve --host 127.0.0.1 --port "$REMOTE_PORT" --base-dir "$DEFAULT_SERVER_HOME" >>"$LOG_FILE" 2>&1 < /dev/null &
  REMOTE_PID="$!"
  printf '%s\\n' "$REMOTE_PID" >"$PID_FILE"
  printf '%s\\n' "$REMOTE_PORT" >"$PORT_FILE"
  printf 'managed\\n' >"$MANAGED_FILE"
  if ! wait_ready "@@RUNE_READY_TIMEOUT_MS@@"; then
    printf 'Remote RUNE server did not become ready on 127.0.0.1:%s.\\n' "$REMOTE_PORT" >&2
    if [ -s "$LOG_FILE" ]; then
...
STATE_DIR="$HOME/.rune/ssh-launch/@@RUNE_STATE_KEY@@"
DEFAULT_SERVER_HOME="$HOME/.rune"
RUNNER_FILE="$STATE_DIR/run-rune.sh"
mkdir -p "$STATE_DIR"
cat >"$RUNNER_FILE" <<'SH'
...
if [ "$REMOTE_MANAGED" != "external" ] && [ -n "$REMOTE_PID" ] && kill -0 "$REMOTE_PID" 2>/dev/null; then
  ...
  if kill -0 "$REMOTE_PID" 2>/dev/null; then
    printf 'Remote RUNE server with PID %s did not stop within 2 seconds. Its ownership files were kept.\\n' "$REMOTE_PID" >&2
    exit 1
  fi
fi
rm -f "$PID_FILE" "$PORT_FILE" "$MANAGED_FILE"
...
export function buildRemoteRuneRunnerScript(input?: RemoteRuneRunnerOptions): string {
  const packageSpec = shellSingleQuote(input?.packageSpec?.trim() || "rune@latest");
  const nodeScriptPath = input?.nodeScriptPath?.trim() || "";
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_RUNNER_SCRIPT, {
      RUNE_PACKAGE_SPEC: packageSpec,
      RUNE_NODE_SCRIPT_PATH: shellSingleQuote(nodeScriptPath),
      RUNE_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    }),
  );
}

export function buildRemoteNodeEnvScript(input?: RemoteRuneRunnerOptions): string {
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_NODE_ENV_SCRIPT, {
      RUNE_NODE_ENGINE_RANGE: shellSingleQuote(input?.nodeEngineRange?.trim() || ""),
      RUNE_NODE_ENGINE_CHECK_SCRIPT: stripTrailingNewlines(buildRemoteNodeEngineCheckScript()),
    }),
  );
}

export function buildRemoteLaunchScript(input?: RemoteRuneRunnerOptions): string {
  return applyScriptPlaceholders(REMOTE_LAUNCH_SCRIPT, {
    RUNE_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    RUNE_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteRuneRunnerScript(input)),
    RUNE_PICK_PORT_SCRIPT: stripTrailingNewlines(REMOTE_PICK_PORT_SCRIPT),
    RUNE_WAIT_READY_SCRIPT: stripTrailingNewlines(REMOTE_WAIT_READY_SCRIPT),
    RUNE_DEFAULT_REMOTE_PORT: String(DEFAULT_REMOTE_PORT),
    RUNE_REMOTE_PORT_SCAN_WINDOW: String(REMOTE_PORT_SCAN_WINDOW),
    RUNE_READY_TIMEOUT_MS: String(REMOTE_READY_TIMEOUT_MS),
    RUNE_REUSE_READY_TIMEOUT_MS: String(REMOTE_REUSE_READY_TIMEOUT_MS),
    RUNE_READY_PROBE_TIMEOUT_MS: String(SSH_READY_PROBE_TIMEOUT_MS),
  });
}

export function buildRemotePairingScript(
  target: DesktopSshEnvironmentTarget,
  input?: RemoteRuneRunnerOptions,
): string {
  return applyScriptPlaceholders(REMOTE_PAIRING_SCRIPT, {
    RUNE_STATE_KEY: remoteStateKey(target),
    RUNE_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteRuneRunnerScript(input)),
  });
}
...
export const launchOrReuseRemoteServer = Effect.fn("ssh/tunnel.launchOrReuseRemoteServer")(
  function* (
    ...
    runner?: RemoteRuneRunnerOptions,
  ): Effect.fn.Return<
...
const makeSshEnvironmentManager = Effect.fn("ssh/tunnel.SshEnvironmentManager.make")(function* (
  ...
  const createTunnelEntry = Effect.fn("ssh/tunnel.ensureTunnelEntry.create")(function* (input: {
    readonly key: string;
    readonly resolvedTarget: DesktopSshEnvironmentTarget;
    readonly runner?: RemoteRuneRunnerOptions;
...
>()("@rune/ssh/tunnel/SshEnvironmentManager") {
