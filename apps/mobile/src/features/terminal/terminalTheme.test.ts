...
describe("getMobileTerminalTheme", () => {
  it("preserves the default light terminal palette", () => {
    expect(getMobileTerminalTheme("RUNE-code", "light")).toMatchObject({
      background: "#f2f2f7",
      foreground: "#6C6C71",
  ...
  it("preserves the default dark terminal palette", () => {
    expect(getMobileTerminalTheme("RUNE-code", "dark")).toMatchObject({
      background: "#0a0a0a",
      foreground: "#adadb1",
      cursorForeground: "#009fff",
  ...
  it("applies the selected palette without replacing ANSI status colors", () => {
    const standard = getMobileTerminalTheme("RUNE-code", "dark");
...
describe("getMobileTerminalTheme", () => {
  it("preserves the Pierre terminal for the default theme", () => {
    for (const scheme of ["light", "dark"] as const) {
      expect(getMobileTerminalTheme("rune", scheme)).toEqual(getPierreTerminalTheme(scheme));
    }
  });

  it("applies the selected palette without replacing ANSI status colors", () => {
    const standard = getMobileTerminalTheme("rune", "dark");
    const ocean = getMobileTerminalTheme("ocean", "dark");
...
describe("buildGhosttyThemeConfig", () => {
  it("serializes theme colors into a ghostty config file", () => {
    const config = buildGhosttyThemeConfig(getMobileTerminalTheme("RUNE-code", "dark"));
