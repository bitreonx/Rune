...
describe("mobileThemeRuntime", () => {
  it("keeps the default palette on Uniwind's built-in appearance themes", () => {
    expect(getMobileUniwindThemeName("RUNE-code", "light")).toBe("light");
    expect(getMobileUniwindThemeName("RUNE-code", "dark")).toBe("dark");
  });

  it("maps custom palettes and appearances to registered themes", () => {
    expect(getMobileUniwindThemeName("RUNE-chat", "dark")).toBe("RUNE-chat-dark");
  });
