...
const appId = "com.runetools.Rune";
const metadata = {
  title: "Editor",
  appName: "Text Editor",
...
beforeEach(async () => {
  ...
  directory = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "RUNE-portal-test-"));
...
it("retains the authenticated connection until the compositor flight has landed", async () => {
  ...
  await feedback.activate("RUNE Code");
  ...
  expect(
    bus.calls
      ...
      .map((call) => call.body),
  ).toEqual([[false, true], ["RUNE Code"], [0.1, 0.8, 0.2, 0.1]]);
  expect(bus.disconnect).toHaveBeenCalledOnce();
  feedback.close();
  expect(bus.disconnect).toHaveBeenCalledOnce();
});
...
it("can activate without animation, and expires a renderer-abandoned capture", async () => {
  ...
  await expect(snapshot!.feedback!.activate("RUNE Code")).rejects.toThrow("No window");
