...
it("activates only a window of the process authenticated by D-Bus", () => {
  const target = window(42, "RUNE Code");
  expect(findCaptureDestination([window(99, "RUNE Code"), target], 42, "RUNE Code")).toBe(target);
  expect(findCaptureDestination([window(99, "RUNE Code")], 42, "RUNE Code")).toBeUndefined();
...
it("does not guess between ambiguous windows from the same process", () => {
  expect(findCaptureDestination([window(42, "RUNE"), window(42, "RUNE")], 42, "RUNE")).toBeUndefined();
