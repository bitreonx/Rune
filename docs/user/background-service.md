# Running RUNE in the Background

On Linux and macOS, RUNE can run as a background service for your user, so it is ready without
keeping a terminal open.

## Manage the Service

Install it with the latest RUNE release:

```sh
npx rune@latest service install
```

Check whether it is installed:

```sh
npx rune@latest service status
```

Update or repair it:

```sh
npx rune@latest service update
```

Stop it and remove it from startup:

```sh
npx rune@latest service uninstall
```

Updating restarts RUNE briefly. Let active agent work and terminal commands finish first.
If a remote update is already in progress, wait for it to finish before retrying a local update.

The service runs a small stable launcher. Exact RUNE versions are installed separately, so a
...
**Linux** uses a systemd user unit at `~/.config/systemd/user/rune.service`. The service starts
when the machine boots and keeps running after you log out (lingering is enabled during install).

**macOS** uses a launch agent at `~/Library/LaunchAgents/com.runetools.rune.service.plist`. It
...
## Using It with RUNE Connect

RUNE Connect may offer to install the service during setup so the host stays reachable in the
background. This is only an onboarding shortcut: the service and RUNE Connect are managed separately.

Signing out of RUNE Connect does not remove the service. Use `rune service uninstall` when you no longer
want RUNE to start in the background.
