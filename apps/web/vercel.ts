...
const ROUTER_HOST = "app.rune.dev";
const HOSTED_WEB_CHANNEL_COOKIE = "rune_web_channel";
const LATEST_ORIGIN = "https://latest.app.rune.dev";
const NIGHTLY_ORIGIN = "https://nightly.app.rune.dev";
...
export const config: VercelConfig = {
  buildCommand:
    'vp run --filter @rune/web build && node ../../scripts/apply-web-brand-assets.ts --channel "${VITE_HOSTED_APP_CHANNEL:-latest}"',
  git: {
    deploymentEnabled: false,
  },
  installCommand:
    "npm install -g vite-plus && vp install --ignore-scripts --filter '@rune/scripts...' --filter '@rune/web...'",
  routes: [
    {
      src: "/__rune/channel",
