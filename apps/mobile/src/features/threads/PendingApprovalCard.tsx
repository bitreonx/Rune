...
export function PendingApprovalCard(props: PendingApprovalCardProps) {
  ...
  return (
    <View className="gap-2.5 rounded-[20px] border border-neutral-200 bg-neutral-100 p-4 dark:border-white/6 dark:bg-neutral-900">
      <Text className="font-rune-bold text-2xs uppercase tracking-[1.1px] text-sky-700 dark:text-sky-300">
        Approval needed
      </Text>
        ...
        {options.map((option) => (
          ...
          >
            <Text
              className={`text-sm ${
                option.decision === "accept"
                  ? "font-rune-extrabold text-white"
                  : option.decision === "decline"
                    ? "font-rune-bold text-rose-700 dark:text-rose-300"
                    : "font-rune-bold text-neutral-950 dark:text-neutral-50"
              }`}
            >
              {option.label}
            </Text>
