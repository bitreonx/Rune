...
> {
  switch (command.type) {
    ...
    case "thread.temporary.set": {
      ...
      // Idempotent by re-emission (see thread.unpin): flagging an already
      // temporary thread keeps its original temporaryAt, and keeping a
      // permanent thread lands on the same null state — neither churns
      // updatedAt.
      const alreadyTemporary = thread.temporaryAt != null;
      ...
      return {
        ...
        payload: {
          threadId: command.threadId,
          temporaryAt: clearing ? null : (thread.temporaryAt ?? occurredAt),
    ...
    case "thread.turn.start": {
      ...
      const userMessageEvent: Omit<OrchestrationEvent, "sequence"> = {
        ...
        payload: {
          ...
          turnId: null,
          streaming: false,
          ...(command.message.hidden !== undefined ? { hidden: command.message.hidden } : {}),
          createdAt: command.createdAt,
          updatedAt: command.createdAt,
        },
      };
