...
describe("ThreadErrorBanner", () => {
  ...
  it("offers Continue only for the orphaned provider session error", () => {
    expect(threadErrorOffersContinue(ORPHANED_PROVIDER_SESSION_ERROR)).toBe(true);
    expect(threadErrorOffersContinue("Provider crashed")).toBe(false);
    expect(threadErrorOffersContinue(null)).toBe(false);
  });

  it("renders a Continue action when one is provided", () => {
    const markup = renderToStaticMarkup(
      <ThreadErrorBanner
        error={ORPHANED_PROVIDER_SESSION_ERROR}
        onDismiss={() => {}}
        onContinue={() => {}}
      />,
    );
  ...
  it("omits the Continue action when none is provided", () => {
    const markup = renderToStaticMarkup(
      <ThreadErrorBanner error="Provider crashed" onDismiss={() => {}} />,
    );
