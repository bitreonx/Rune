...
import { DEV_PROXIED_PATH_PREFIXES } from "@rune/shared/devProxy";
...
const isSingleOriginDev = process.env.RUNE_SINGLE_ORIGIN_DEV === "1";
...
const configuredRelayUrl = repoEnv.VITE_RUNE_RELAY_URL?.trim() || "";
...
const sourcemapEnv = process.env.RUNE_WEB_SOURCEMAP?.trim().toLowerCase();
...
// as hot patches. Opt-in while experimental: RUNE_BUNDLED_DEV=1 pnpm dev:web
// The dev runner defaults this on for --share runs (remote browsers pay a
// round trip per import level in unbundled dev); RUNE_BUNDLED_DEV=0 opts out.
const bundledDevEnv = process.env.RUNE_BUNDLED_DEV?.trim().toLowerCase();
...
function resolveDevProxyTarget(
  ...
  // RUNE_PORT is set by scripts/dev-runner.ts for every non-desktop mode.
...
const devProxyTarget = resolveDevProxyTarget(process.env.RUNE_PORT, configuredWsUrl);
...
const configuredAllowedHosts = (process.env.RUNE_DEV_ALLOWED_HOSTS ?? "")
  .split(",")
...
export default defineConfig(() => {
  return {
    ...
    define: {
      ...
      "import.meta.env.VITE_RUNE_RELAY_URL": JSON.stringify(configuredRelayUrl),
