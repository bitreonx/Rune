...
import type { DesktopAppActivationRequest } from "@rune/contracts";
import { resolveDesktopAppControlAddress } from "@rune/shared/desktopAppControl";
import {
  HostProcessPlatform,
  HostProcessUserId,
...
describe("RUNE app", () => {
  it.effect("rejects SSH before it tries to reach a desktop app", () =>
    withTempDirectory("RUNE-app-ssh-test-", (root) =>
      Effect.gen(function* () {
        const baseDir = NodePath.join(root, "missing-RUNE-home");
        ...
        expect(error).toMatchObject({
          ...
          message:
            "`RUNE app` only controls a desktop app on the same machine. It cannot run over SSH.",
        });
  ...
  it.effect("rejects unsupported platforms without creating state", () =>
    withTempDirectory("RUNE-app-platform-test-", (root) =>
      Effect.gen(function* () {
        const baseDir = NodePath.join(root, "missing-RUNE-home");
        ...
        expect(error).toMatchObject({
          ...
          platform: "freebsd",
          message: "`RUNE app` is not supported on freebsd.",
        });
        expect(yield* pathExists(baseDir)).toBe(false);
      }),
    ),
  ...
  it.effect("does not create state when only a server or no desktop app is running", () =>
    withTempDirectory("RUNE-app-missing-test-", (root) =>
      Effect.gen(function* () {
        const baseDir = NodePath.join(root, "missing-RUNE-home");
        ...
        expect(error).toMatchObject({
          ...
          message: expect.stringContaining("Could not reach the RUNE Code desktop app."),
          cause: { code: "ENOENT" },
        });
        expect(yield* pathExists(baseDir)).toBe(false);
      }),
  ...
  it.effect("uses RUNE_HOME or --base-dir and sends the default or explicit path", () =>
    withTempDirectory("RUNE-app-command-test-", (root) =>
      Effect.gen(function* () {
        const baseDir = NodePath.join(root, "RUNE-home");
        ...
        yield* runCli(["app"], { RUNE_HOME: baseDir });
  ...
  it.effect("prefers the installed desktop app when a dev desktop is also running", () =>
    withTempDirectory("RUNE-app-preferred-test-", (root) =>
      Effect.gen(function* () {
        vi.mocked(NodeOS.homedir).mockReturnValue(root);
        const baseDir = NodePath.join(root, ".rune");
        const desktop = yield* fakeDesktop({ baseDir });
  ...
  it.effect("finds the dev desktop when the default desktop socket is absent", () =>
    withTempDirectory("RUNE-app-dev-test-", (root) =>
      Effect.gen(function* () {
        vi.mocked(NodeOS.homedir).mockReturnValue(root);
        const baseDir = NodePath.join(root, ".rune");
        const development = yield* fakeDesktop({ baseDir, stateSubdirectory: "dev" });
  ...
  it.effect("never searches a dev state directory for an explicit RUNE home", () =>
    withTempDirectory("RUNE-app-explicit-test-", (root) =>
      Effect.gen(function* () {
        vi.mocked(NodeOS.homedir).mockReturnValue(root);
        const baseDir = NodePath.join(root, ".rune");
        const development = yield* fakeDesktop({ baseDir, stateSubdirectory: "dev" });
      ...
      const envError = yield* runCli(["app"], { RUNE_HOME: baseDir }).pipe(Effect.flip);
  ...
  for (const responseKind of ["failure", "invalid"] as const) {
    it.effect(`never falls back after the default desktop sends a ${responseKind} response`, () =>
      withTempDirectory("RUNE-app-response-test-", (root) =>
        Effect.gen(function* () {
          vi.mocked(NodeOS.homedir).mockReturnValue(root);
          const baseDir = NodePath.join(root, ".rune");
