import type { ProviderUserInputAnswers, UserInputQuestion } from "@rune/contracts";
...
/**
 * Client captured the plan for RUNE's proposed-plan card. Abandon the native
 * Grok plan-approval gate so the turn unblocks; the user implements via RUNE UI.
 */
 ...
 * Used so RUNE can show the plan while plan mode is still active (before exit).
 */
...
export const makeXAiPromptCompletionRuntime = Effect.fn("makeXAiPromptCompletionRuntime")(
  function* (runtime: AcpSessionRuntime.AcpSessionRuntime["Service"]) {
    ...
    const allocatePromptFallbackId = Effect.sync(() => {
      ...
      return `rune-xai-prompt-${nextPromptFallbackId}`;
    });
