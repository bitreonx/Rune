...
export class FileSaveCoordinator<A = unknown, E = unknown> {
  ...
  /**
   * Explicit save (⌘S / save button): persists now instead of waiting out the
   * debounce. A no-op when nothing was edited or a write is already in flight
   * — the in-flight path already picks up the newest contents when it lands.
   */
  flush(): void {
