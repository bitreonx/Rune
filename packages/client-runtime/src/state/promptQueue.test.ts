...
import { createPromptQueueItem, createPromptQueueThreadState, nextPromptQueueItem, reducePromptQueue } from "./promptQueue.ts";

const threadId = "thread-1" as never;
const turnId = "turn-1" as never;
const item = (text: string, id: string) => ({ ...createPromptQueueItem(threadId, text, "2026-08-26T00:00:00.000Z"), id });
...
describe("prompt queue state machine", () => {
  it("preserves FIFO and claims each item once", () => {
    ...
    state = reducePromptQueue(state, { type: "claim-next", turnId, now: "2026-08-26T00:01:00.000Z" });
    expect(nextPromptQueueItem(state)?.text).toBe("C");
    state = reducePromptQueue(state, { type: "claim-next", turnId: "turn-2" as never, now: "2026-08-26T00:02:00.000Z" });
  ...
  it("does not execute a settled item twice", () => {
    ...
    state = reducePromptQueue(state, { type: "settle", itemId: "b", status: "completed", now: "y" });
    const unchanged = reducePromptQueue(state, { type: "settle", itemId: "b", status: "completed", now: "z" });
