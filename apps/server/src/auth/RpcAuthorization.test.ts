...
describe("RPC authorization scopes", () => {
  ...
  it("keeps skill discovery and body reads under the environment read scope", () => {
    expect(requiredScopeForRpcMethod(WS_METHODS.skillsList)).toBe(AuthOrchestrationReadScope);
    expect(requiredScopeForRpcMethod(WS_METHODS.skillsRefresh)).toBe(AuthOrchestrationReadScope);
    expect(requiredScopeForRpcMethod(WS_METHODS.skillsGetBody)).toBe(AuthOrchestrationReadScope);
  });
