...
vi.mock("@pierre/diffs/react", () => {
  return { CodeView: () => null, FileDiff: MockFileDiff };
});

// The inline diff's query layer drags the environment RPC stack into this
// file's import graph; the timeline tests never expand a diff.
...
describe("MessagesTimeline", () => {
  ...
  it("offers edit and delete actions on a user message with checkpoint history", () => {
    const markup = renderToStaticMarkup(
      <MessagesTimeline
        {...buildProps()}
        timelineEntries={[buildUserTimelineEntry("First prompt.")]}
        revertTurnCountByUserMessageId={new Map([[MessageId.make("message-1"), 0]])}
      />,
    );
  ...
  it("hides edit and delete actions without checkpoint history", () => {
    const markup = renderToStaticMarkup(
      <MessagesTimeline
        {...buildProps()}
        timelineEntries={[buildUserTimelineEntry("First prompt.")]}
  ...
  it("spins the working row's status circle", () => {
    const markup = renderToStaticMarkup(
      <MessagesTimeline
        {...buildProps()}
        isWorking
    ...
    expect(markup).toContain("Working for");
    expect(markup).toContain("working-spin");
  });
