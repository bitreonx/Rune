    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        ServerConfig.layerTest(
          ...
          input.configBaseDir ?? { prefix: "rune-scanner-config-" },
        ),
...
it.layer(NodeServices.layer)("AgentSessionScanner", (it) => {
  describe("scan", () => {
    it.effect("reads Claude project cwds from transcripts, newest first", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const claudeHomePath = yield* makeTempDir("rune-claude-home-");
        const codexHomePath = yield* makeTempDir("rune-codex-home-");
        const olderWorkspace = yield* makeTempDir("rune-workspace-older-");
        const newerWorkspace = yield* makeTempDir("rune-workspace-newer-");
    ...
    it.effect("groups Codex rollouts by cwd across date directories", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("rune-claude-home-");
        const codexHomePath = yield* makeTempDir("rune-codex-home-");
        const workspace = yield* makeTempDir("rune-workspace-");
        const otherWorkspace = yield* makeTempDir("rune-workspace-other-");
    ...
    it.effect("returns the imported project ID through a realpath alias", () =>
      Effect.gen(function* () {
        ...
        const codexHomePath = yield* makeTempDir("rune-codex-home-");
        const workspace = yield* makeTempDir("rune-workspace-");
        const linkParent = yield* makeTempDir("rune-scanner-links-");
    ...
    it.effect("keeps case variants distinct when the filesystem identities differ", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("rune-claude-home-");
        const codexHomePath = yield* makeTempDir("rune-codex-home-");
        const backingUpper = yield* makeTempDir("rune-backing-upper-");
        const backingLower = yield* makeTempDir("rune-backing-lower-");
        const aliasParent = yield* makeTempDir("rune-case-aliases-");
    ...
    it.effect("uses explicit provider instance homes instead of overridden legacy homes", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const claudeHomePath = yield* makeTempDir("rune-claude-legacy-");
        const codexHomePath = yield* makeTempDir("rune-codex-legacy-");
        const claudeInstanceHome = yield* makeTempDir("rune-claude-instance-");
        const codexInstanceHome = yield* makeTempDir("rune-codex-instance-");
        const legacyWorkspace = yield* makeTempDir("rune-workspace-legacy-");
        const claudeWorkspace = yield* makeTempDir("rune-workspace-claude-");
        const codexWorkspace = yield* makeTempDir("rune-workspace-codex-");
    ...
    it.effect("scans each distinct home across multiple instances once", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("rune-claude-home-");
        const codexHomePath = yield* makeTempDir("rune-codex-home-");
        const otherCodexHome = yield* makeTempDir("rune-codex-other-");
        const workspace = yield* makeTempDir("rune-workspace-");
        const otherWorkspace = yield* makeTempDir("rune-workspace-other-");
    ...
    it.effect("honors provider instance home directory environment variables", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("rune-claude-legacy-");
        const codexHomePath = yield* makeTempDir("rune-codex-legacy-");
        const claudeEnvironmentHome = yield* makeTempDir("rune-claude-env-");
        const codexEnvironmentHome = yield* makeTempDir("rune-codex-env-");
        const claudeWorkspace = yield* makeTempDir("rune-workspace-claude-");
        const codexWorkspace = yield* makeTempDir("rune-workspace-codex-");
    ...
    it.effect("does not scan provider instances disabled by the envelope or config", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("rune-claude-home-");
        const codexHomePath = yield* makeTempDir("rune-codex-home-");
        const envelopeDisabledHome = yield* makeTempDir("rune-codex-disabled-envelope-");
        const configDisabledHome = yield* makeTempDir("rune-codex-disabled-config-");
        const envelopeWorkspace = yield* makeTempDir("rune-workspace-disabled-envelope-");
        const configWorkspace = yield* makeTempDir("rune-workspace-disabled-config-");
    ...
    it.effect("excludes the home directory, temporary root, and RUNE data directory", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const claudeHomePath = yield* makeTempDir("rune-claude-home-");
        const codexHomePath = yield* makeTempDir("rune-codex-home-");
        const configBaseDir = yield* makeTempDir("rune-scanner-base-");
        const workspace = yield* makeTempDir("rune-workspace-");
    ...
    it.effect("excludes RUNE-managed worktree sandboxes", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const claudeHomePath = yield* makeTempDir("rune-claude-home-");
        const codexHomePath = yield* makeTempDir("rune-codex-home-");
        ...
        const worktreeCwd = path.join(claudeHomePath, ".rune", "worktrees", "t3code", "wt-1");
    ...
    it.effect("excludes Codex scratch directories and Downloads", () =>
      Effect.gen(function* () {
        ...
        const runId = path.basename(yield* makeTempDir("rune-scanner-test-"));
        ...
        const keep = yield* makeTempDir("rune-workspace-keep-");
    ...
    it.effect("skips linked git worktrees and reports the origin of real checkouts", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("rune-claude-home-");
        const codexHomePath = yield* makeTempDir("rune-codex-home-");
        const repo = yield* makeTempDir("rune-workspace-repo-");
        const worktree = yield* makeTempDir("rune-workspace-worktree-");
        const plain = yield* makeTempDir("rune-workspace-plain-");
        const noRemote = yield* makeTempDir("rune-workspace-noremote-");
        const submodule = yield* makeTempDir("rune-workspace-submodule-");
    ...
    it.effect("excludes sandboxes under the configured worktrees dir without .RUNE in the path", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const claudeHomePath = yield* makeTempDir("rune-claude-home-");
        const codexHomePath = yield* makeTempDir("rune-codex-home-");
        ...
        // dir contains no `.RUNE` segment — only the config-based prefix match
    ...
    it.effect.each([64, 65])("shares metadata bytes across homes for %s one-MiB files", (count) =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const fileSystem = yield* FileSystem.FileSystem;
        const claudeHomePath = yield* makeTempDir("rune-metadata-home-");
        const secondHome = yield* makeTempDir("rune-metadata-second-");
        const codexHomePath = yield* makeTempDir("rune-metadata-codex-");
        const firstWorkspace = yield* makeTempDir("rune-metadata-first-project-");
        const secondWorkspace = yield* makeTempDir("rune-metadata-second-project-");
        const directories = [
    ...
    it.effect.each([50, 51])("bounds metadata open/read calls for %s short-read files", (count) =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const fileSystem = yield* FileSystem.FileSystem;
        const claudeHomePath = yield* makeTempDir("rune-short-metadata-home-");
        const codexHomePath = yield* makeTempDir("rune-short-metadata-codex-");
        const workspace = yield* makeTempDir("rune-short-metadata-project-");
    ...
    it.effect("bounds malformed metadata records without excluding another account", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const fileSystem = yield* FileSystem.FileSystem;
        const claudeHomePath = yield* makeTempDir("rune-record-metadata-home-");
        const secondHome = yield* makeTempDir("rune-record-metadata-second-");
        const codexHomePath = yield* makeTempDir("rune-record-metadata-codex-");
        const workspace = yield* makeTempDir("rune-record-metadata-project-");
        const directory = path.join(claudeHomePath, "projects", "p");
        const template = path.join(directory, "template.jsonl");
        yield* writeTranscript({
          filePath: template,
    ...
    it.effect.each([19_999, 20_000])(
      ...
      (count) =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const fileSystem = yield* FileSystem.FileSystem;
          const claudeHomePath = yield* makeTempDir("rune-directory-budget-home-");
          const codexHomePath = yield* makeTempDir("rune-directory-budget-codex-");
    ...
    it.effect("returns an empty result when neither home directory exists", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const root = yield* makeTempDir("rune-missing-homes-");
  ...
  describe("recentThreads", () => {
    it.effect.each([false, true])(
      ...
      (overflow) =>
        Effect.gen(function* () {
          ...
          const claudeHomePath = yield* makeTempDir("rune-record-limit-claude-");
          const codexHomePath = yield* makeTempDir("rune-record-limit-codex-");
          const workspace = yield* makeTempDir("rune-record-limit-project-");
    ...
    it.effect("keeps the provider instance that owns a custom session home", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("rune-claude-home-");
        const codexHomePath = yield* makeTempDir("rune-codex-home-");
        const customHome = yield* makeTempDir("rune-codex-custom-");
        const workspace = yield* makeTempDir("rune-workspace-");
    ...
    it.effect("streams large transcripts across providers without hiding projects", () =>
      Effect.gen(function* () {
        ...
        const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
        yield* TestClock.setTime(nowMs);
        const claudeHomePath = yield* makeTempDir("rune-budget-claude-");
        const codexHomePath = yield* makeTempDir("rune-budget-codex-");
        const workspace = yield* makeTempDir("rune-budget-workspace-");
    ...
    it.effect("skips excessive records without blocking an older valid transcript", () =>
      Effect.gen(function* () {
        ...
        const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
        yield* TestClock.setTime(nowMs);
        const claudeHomePath = yield* makeTempDir("rune-record-budget-claude-");
        const codexHomePath = yield* makeTempDir("rune-record-budget-codex-");
        const workspace = yield* makeTempDir("rune-record-budget-workspace-");
        ...
        it.effect.skipIf(replacement === "symlink alias" && !symlinksSupported)(
          ...
          () =>
            Effect.gen(function* () {
              ...
              const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
              yield* TestClock.setTime(nowMs);
              const fixture = yield* makeTempDir("rune-replaced-cwd-");
    ...
    it.effect("checks file identity and provider before skipping completed history", () =>
      Effect.gen(function* () {
        ...
        const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
        yield* TestClock.setTime(nowMs);
        const claudeHomePath = yield* makeTempDir("rune-completed-claude-");
        const codexHomePath = yield* makeTempDir("rune-completed-codex-");
        const workspace = yield* makeTempDir("rune-completed-workspace-");
    ...
    it.effect("does not reopen a transcript that becomes a non-file after discovery", () =>
      Effect.gen(function* () {
        ...
        const codexHomePath = yield* makeTempDir("rune-codex-home-");
        const workspace = yield* makeTempDir("rune-workspace-");
        const nonFilePath = yield* makeTempDir("rune-non-file-");
    ...
    it.effect("does not import sessions from a RUNE-managed worktree", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const fileSystem = yield* FileSystem.FileSystem;
        const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
    ...
    it.effect("uses one deterministic provider instance for a shared session home", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("rune-claude-home-");
        const codexHomePath = yield* makeTempDir("rune-codex-home-");
        const sharedHome = yield* makeTempDir("rune-codex-shared-");
        const workspace = yield* makeTempDir("rune-workspace-");
    ...
    it.effect("keeps a second account when the first has 5000 newer files", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("rune-claude-home-");
        const codexHomePath = yield* makeTempDir("rune-codex-home-");
        const oldWorkspace = yield* makeTempDir("rune-workspace-old-");
        const recentWorkspace = yield* makeTempDir("rune-workspace-recent-");
        const recentHome = yield* makeTempDir("rune-claude-recent-home-");
