...
The session directory also stores provider-specific resume cursors. Startup reconciliation restores
Antigravity sessions from their persisted `conversation_id` via `agy --conversation` before settling
the thread projection, so a normal server restart does not surface the generic orphaned-session
message for that provider. When another provider's session does not survive a restart, the web
banner offers a Continue action that dispatches `thread.turn.start` with a message flagged
`hidden`; providers cannot resume without input, so the nudge text reaches the CLI while clients
filter it out of timelines and the projector skips it when building SQLite projections. Because the
projections cannot serve hidden messages, the provider reactor bridges them from the domain event
stream to satisfy its turn-start message lookup.
...
[openai-api]: ../../apps/server/src/provider/Drivers/OpenAiApiDriver.ts
