...
import { HostProcessPlatform, HostProcessUserId } from "@rune/shared/hostProcess";
...
describe("desktop app control server", () => {
  it.effect("roundtrips a request and removes its socket on shutdown", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(async () => {
        const root = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "RUNE-app-control-test-"));
  ...
  it.effect("cancels a queued request when the client disconnects", () =>
    Effect.gen(function* () {
      ...
      const userId = yield* HostProcessUserId;
      yield* Effect.promise(async () => {
        const root = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "RUNE-app-cancel-test-"));
        const target = makeTarget(NodePath.join(root, "userdata"), platform, userId);
