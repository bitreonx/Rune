...
it.layer(NodeServices.layer)("providerMaintenance", (it) => {
  ...
  it.effect.skipIf(!symlinksSupported)(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("rune-vite-plus-capabilities");
  ...
  it.effect("proves Windows npm ownership from the package manifest beside the shim", () =>
    Effect.gen(function* () {
      const tempDir = yield* makeTempDir("RUNE-npm-windows-capabilities");
  ...
  it.effect.skipIf(!symlinksSupported)(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("RUNE-pnpm-capabilities");
  ...
  it.effect.skipIf(windowsHost)(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("rune-bun-capabilities");
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("rune-native-package-tool-native-capabilities");
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("rune-scoped-package-tool-native-capabilities");
  ...
  it.effect("keeps npm updates for binaries symlinked into npm's global node_modules tree", () =>
    Effect.gen(function* () {
      const tempDir = yield* makeTempDir("rune-npm-capabilities");
  ...
  it.effect.skipIf(windowsHost)("runs an explicit native updater outside PATH", () =>
    Effect.gen(function* () {
      const tempDir = yield* makeTempDir("rune-pnpm-realpath-capabilities");
      const binDir = NodePath.join(tempDir, "bin");
      const packageBinDir = NodePath.join(
        tempDir,
        "with spaces",
  ...
  it.effect.skipIf(windowsHost)("carries the native updater's environment into the action", () =>
    Effect.gen(function* () {
      const tempDir = yield* makeTempDir("RUNE-native-env");
  ...
  it.effect.skipIf(windowsHost)(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("RUNE-missing-native-capabilities");
  ...
  it.effect.each([
    ...
    (fixture) =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("RUNE-homebrew-capabilities");
  ...
  it.effect.skipIf(windowsHost)(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("RUNE-homebrew-foreign-prefix");
        const brewBinDir = NodePath.join(tempDir, "brew-bin");
  ...
  it.effect.skipIf(windowsHost)(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("RUNE-custom-capabilities");
