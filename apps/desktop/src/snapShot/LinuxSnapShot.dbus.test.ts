...
it.runIf(hasDbus)("captures through real D-Bus marshalling on a private bus", async () => {
  const directory = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "RUNE-private-dbus-"));
  ...
  try {
    ...
    await server.requestName("org.gnome.Shell.Extensions.RUNESnapShot", NameFlag.DO_NOT_QUEUE);
    ...
    expect(await portal.backend("com.runetools.Rune")).toBe("screenshot-portal");
    expect(await portal.capturePortal()).toEqual({ png });
    expect(target).toBe(8);
    portalVersion = 2;
    const extension = connect();
    expect(await extension.backend("com.runetools.Rune")).toBe("gnome-extension");
    expect(await extension.captureExtension("com.runetools.Rune")).toMatchObject({
      png,
      window: { processId: 42 },
    });
    const owner = await server.call(
      new Message({
        ...
        member: "GetNameOwner",
        signature: "s",
        body: ["com.runetools.Rune.SnapShot"],
      }),
    );
    ...
    expect(await updated.backend("com.runetools.Rune")).toBe("gnome-extension");
    const snapshot = await updated.captureExtension("com.runetools.Rune", {
      flash: true,
      animate: true,
    });
    expect(snapshot.feedback?.animationStarted).toBe(true);
    await snapshot.feedback!.activate("RUNE Code");
    ...
    expect(activateTitle).toBe("RUNE Code");
    expect(animateFrame).toEqual([0.1, 0.8, 0.2, 0.1]);
    vi.stubEnv("XDG_CURRENT_DESKTOP", "KDE");
    const kde = connect();
    expect(await kde.backend("com.runetools.Rune")).toBe("kde");
    ...
    const command = [
      ...
      "--session",
      "--dest",
      "com.runetools.Rune.NiriTest.SnapShot",
      "--object-path",
      "/com/runetools/SnapShot",
      "--method",
      "com.runetools.SnapShot.Capture",
    ];
    ...
    const invalid = server.call(
      new Message({
        destination: "com.runetools.Rune.NiriTest.SnapShot",
        path: "/com/runetools/SnapShot",
        interface: "com.runetools.SnapShot",
        member: "Capture",
        signature: "s",
        body: ["not allowed"],
      }),
