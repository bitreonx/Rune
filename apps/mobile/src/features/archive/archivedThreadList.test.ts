import type { ArchivedSnapshotEntry } from "@rune/client-runtime/state/threads";
import type { OrchestrationProjectShell, OrchestrationThreadShell } from "@rune/contracts";
...
describe("buildArchivedThreadGroups", () => {
  it("groups archived threads by project and sorts newest first", () => {
    const project = makeProject({ id: ProjectId.make("project-1"), title: "RUNE" });
  ...
  it("filters by environment and matches project, thread, and branch text", () => {
    ...
    const firstProject = makeProject({ id: ProjectId.make("project-1"), title: "RUNE" });
