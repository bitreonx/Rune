...
describe("composerDraftStore temporary chats", () => {
  const threadId = ThreadId.make("thread-temporary");
  const threadRef = scopeThreadRef(TEST_ENVIRONMENT_ID, threadId);
  ...
  it("stores the temporary flag on the draft", () => {
    useComposerDraftStore.getState().setTemporary(threadRef, true);

    expect(draftFor(threadId, TEST_ENVIRONMENT_ID)?.temporary).toBe(true);
  });

  it("keeps an armed empty draft instead of pruning it", () => {
    useComposerDraftStore.getState().setTemporary(threadRef, true);

    expect(draftFor(threadId, TEST_ENVIRONMENT_ID)).toMatchObject({ temporary: true });
  });

  it("removes an empty draft when the flag is cleared", () => {
    useComposerDraftStore.getState().setTemporary(threadRef, true);

    useComposerDraftStore.getState().setTemporary(threadRef, false);

    expect(draftFor(threadId, TEST_ENVIRONMENT_ID)).toBeUndefined();
  });

  it("preserves the armed flag when clearing composer content for a retry", () => {
    const store = useComposerDraftStore.getState();
    store.setPrompt(threadRef, "retry me as temporary");
    store.setTemporary(threadRef, true);
  ...
  it("persists temporary only while armed and drops the disarmed empty draft", () => {
    const persistApi = useComposerDraftStore.persist as unknown as {
      getOptions: () => {
        partialize: (state: ReturnType<typeof useComposerDraftStore.getState>) => unknown;
      };
    };
    const readPersistedTemporary = () => {
      const persisted = persistApi.getOptions().partialize(useComposerDraftStore.getState()) as {
        draftsByThreadKey?: Record<string, { temporary?: boolean }>;
      };
      return persisted.draftsByThreadKey?.[threadKeyFor(threadId, TEST_ENVIRONMENT_ID)]?.temporary;
    };

    useComposerDraftStore.getState().setTemporary(threadRef, true);
    expect(readPersistedTemporary()).toBe(true);

    useComposerDraftStore.getState().setTemporary(threadRef, false);
    expect(readPersistedTemporary()).toBeUndefined();
  });

  it("hydrates persisted drafts without a temporary flag as permanent", () => {
    const persistApi = useComposerDraftStore.persist as unknown as {
      getOptions: () => {
        merge: (
          persistedState: unknown,
    ...
    expect(mergedState.draftsByThreadKey[threadKeyFor(threadId)]?.temporary).toBe(false);
  });
});
