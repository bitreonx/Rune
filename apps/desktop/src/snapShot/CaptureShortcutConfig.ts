...
export class CaptureShortcutConfig {
  ...
  async apply(id: string, desktop: Target["desktop"]): Promise<DesktopCaptureConfigApplied> {
    ...
    const temporary = NodePath.join(directory, `.RUNE-capture-${NodeCrypto.randomUUID()}.tmp`);
    let staged = false;
    try {
      ...
      await unchanged();
      const backupPath = `${root.resolvedPath}.RUNE-capture-backup-${NodeCrypto.randomUUID()}`;
