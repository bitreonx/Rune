...
it.layer(NodeServices.layer)("SessionStore.layer", (it) => {
  it.effect("keys remote cookies by environment identity instead of state directory", () =>
    Effect.gen(function* () {
      ...
      const original = yield* cookieName("/srv/RUNE-one", EnvironmentId.make("environment-one"));
      const moved = yield* cookieName("/srv/RUNE-moved", EnvironmentId.make("environment-one"));
      const other = yield* cookieName("/srv/RUNE-one", EnvironmentId.make("environment-two"));
