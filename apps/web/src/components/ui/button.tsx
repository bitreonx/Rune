...
function Button({ className, variant, size, render, ...props }: ButtonProps) {
  ...
  const defaultProps = {
    ...
    "data-slot": "button",
    type: typeValue,
    // Press, not click: the tick should land at contact like a physical
    // key. mergeProps chains this with any consumer-supplied handler.
    onPointerDown: (event: React.PointerEvent<HTMLButtonElement>) => {
      if (event.button === 0) playSoundEffect("click");
    },
  };
