...
import {
  ...
  signRelayJwt,
  verifyRelayJwt,
} from "@rune/shared/relayJwt";
import { isSecureRelayUrl } from "@rune/shared/relayUrl";
  ...
  Effect.mapError(
    () =>
      new EnvironmentHttpInternalServerError({
        message: "RUNE_RELAY_URL must be configured as a secure absolute HTTPS origin.",
      }),
  ),
);
...
const makeCloudLinkProof = Effect.fn("environment.cloud.makeLinkProof")(function* (
  ...
  const payload = {
    iss: `rune-env:${descriptor.environmentId}`,
...
const relayClientRequest = <A>(
  ...
  HttpClientRequest.post(input.url).pipe(
    ...
    Effect.mapError(
      (cause) =>
        new EnvironmentHttpInternalServerError({
          message: `RUNE Connect relay request failed: ${String(cause)}`,
        }),
    ),
...
const reconcileDesiredCloudLinkWith = Effect.fn("environment.cloud.reconcileDesiredLinkWith")(
          ...
          onNone: () =>
            Effect.fail(
              new EnvironmentHttpUnauthorizedError({
                message: "Run `rune connect link` to authorize this environment.",
              }),
            ),
  ...
  Effect.catchIf(
    ServerSecretStore.isSecretStoreError,
    failEnvironmentCloudInternalError("Could not persist desired RUNE Connect link state."),
  ),
  Effect.catchTags({
...
// explicit launcher stop (`rune service uninstall`, `systemctl stop`,
...
export const releaseManagedTunnelOnShutdown = Effect.fn(
  ...
  // persisted URL: RUNE_RELAY_URL may have changed since the link was made.
...
const cloudEnvironmentHealthHandler = Effect.fn("environment.cloud.health")(
  function* (dependencies: CloudHttpDependencies, request: RelayCloudEnvironmentHealthRequest) {
    ...
    const proofOption = yield* verifyRelayJwt({
      ...
      audience: `rune-env:${environmentId}`,
...
export const connectHttpApiLayer = HttpApiBuilder.group(
  ...
  Effect.fnUntraced(function* (handlers) {
    ...
    return handlers
      ...
      .handle("runeMintCredential", ({ payload }) =>
