...
import { GitCommandError } from "@rune/contracts";
    ...
    Layer.provide(
      ConfigProvider.layer(
        ConfigProvider.fromEnv({
          env: {
            RUNE_BITBUCKET_API_BASE_URL: "https://api.test.local/2.0",
            RUNE_BITBUCKET_EMAIL: "user@example.com",
            RUNE_BITBUCKET_API_TOKEN: "token",
          },
        }),
      ),
    ),
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(JSON.parse(new TextDecoder().decode(rawBody)), {
      ...
      source: {
        ...
        repository: { full_name: "owner/rune" },
      },
      destination: {
        branch: { name: "main" },
      },
...
it.effect("checks out fork pull requests through an ensured fork remote", () => {
  const { git, layer } = makeLayer({
    response: (request) => {
      if (request.url.endsWith("/repositories/octocat/rune")) {
        return Response.json({
          ...repositoryJson,
          full_name: "octocat/rune",
          links: {
            html: { href: "https://bitbucket.org/octocat/rune" },
            clone: [
              { name: "https", href: "https://bitbucket.org/octocat/rune.git" },
              { name: "ssh", href: "git@bitbucket.org:octocat/rune.git" },
            ],
          },
        });
      }
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(git.ensureRemote.mock.calls[0]?.[0], {
      ...
      url: "git@bitbucket.org:octocat/rune.git",
    });
    assert.deepStrictEqual(git.fetchRemoteBranch.mock.calls[0]?.[0], {
      cwd: "/repo",
      remoteName: "octocat",
      remoteBranch: "main",
      localBranch: "rune/pr-42/main",
    });
    assert.deepStrictEqual(git.setBranchUpstream.mock.calls[0]?.[0], {
      cwd: "/repo",
      branch: "rune/pr-42/main",
      remoteName: "octocat",
      remoteBranch: "main",
    });
    assert.deepStrictEqual(git.switchRef.mock.calls[0]?.[0], {
      cwd: "/repo",
      refName: "rune/pr-42/main",
    });
  }).pipe(Effect.provide(layer));
});
