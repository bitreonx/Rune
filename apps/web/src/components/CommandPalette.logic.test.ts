...
describe("buildThreadActionItems", () => {
  it("excludes temporary threads from the palette thread list", () => {
    const items = buildThreadActionItems({
      threads: [
        makeThread({ id: ThreadId.make("thread-permanent"), title: "Permanent thread" }),
        makeThread({
          id: ThreadId.make("thread-temp"),
          title: "Temporary thread",
          temporaryAt: "2026-03-09T10:00:00.000Z",
        }),
      ],
    ...
    expect(items.map((item) => item.value)).toEqual(["thread:thread-permanent"]);
  });
  ...
  it("keeps message excerpts searchable without replacing thread metadata", () => {
    const [item] = buildThreadActionItems({
      ...
      projectTitleById: new Map([[PROJECT_ID, "RUNE"]]),
      sortOrder: "updated_at",
      icon: null,
      getContentMatch: () => ({
        source: "assistant",
    ...
    expect(item?.description).toBe("RUNE · #feat/search");
  });
