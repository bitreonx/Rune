...
import type { SnapShotKeyChord } from "@rune/contracts";
...
export class PortalCaptureShortcut {
  ...
  async configure() {
    ...
    if (!this.hasSession || this.version < 2)
      throw new Error(
        "Open your desktop's shortcut settings and allow RUNE Code's capture shortcut.",
      );
    await this.call({
  ...
  private failed = (error: unknown) => {
    ...
    this.update({
      ...
      shortcutMessage: this.managedByHyprland
        ? "Couldn't connect to Hyprland shortcuts. Make sure xdg-desktop-portal-hyprland is running, then restart RUNE Code."
  ...
  private async request(
    ...
    const token = `rune_${NodeCrypto.randomUUID().replaceAll("-", "")}`;
          ...
          this.update({
            ...
            shortcutMessage:
              this.version >= 2
                ...
                : "Shortcut permission wasn't granted. Allow RUNE Code in your desktop's shortcut settings.",
          });
          return undefined;
        }
    ...
    if (this.managedByHyprland) {
      this.update({
        ...
        shortcutMessage: shortcut
          ...
          : "Hyprland did not register the capture action. Check that xdg-desktop-portal-hyprland is running, then restart RUNE Code.",
      });
      return;
    }
  ...
  private async initialize(appId: string, shortcut: SnapShotKeyChord) {
    ...
    const created = await this.request("CreateSession", "", [], {
      session_handle_token: new Variant(
        "s",
        `rune_capture_${NodeCrypto.randomUUID().replaceAll("-", "")}`,
      ),
    });
    ...
    this.shortcutId = this.managedByHyprland
      ...
      : `RUNE-snap-shot-${NodeCrypto.createHash("sha256").update(trigger).digest("hex").slice(0, 16)}`;
