...
/**
 * Drives the primary environment's RUNE Connect link. RUNE Connect (managed
...
export function useCloudLinkController() {
  ...
  const reportUpdateFailure = (cause: unknown) => {
    const message = cause instanceof Error ? cause.message : "Could not update RUNE Connect access.";
    const traceId = findErrorTraceId(cause);
    console.error("[rune-connect] Could not update RUNE Connect", { message, traceId, cause });
    setOperationError(traceId ? `${message} Trace ID: ${traceId}` : message);
    toastManager.add({
      type: "error",
      title: "Could not update RUNE Connect",
      description: message,
      data: traceId
        ? {
  ...
  const reconcileCloudState = async (desired: CloudLinkDesiredState): Promise<boolean> => {
    ...
    if (!wantsLink) {
      ...
      // leave the user unable to turn RUNE Connect off.
      ...
      if (!clerkToken) {
        reportUpdateFailure(new Error("Sign in to RUNE Connect before enabling this."));
        return false;
      }
