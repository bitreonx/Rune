...
it("activates the caller, flies its frozen capture, and retains the landed image until disconnect", async () => {
  ...
  await feedback.activate(":1.23", "RUNE");
...
it("waits for RUNE to remap and cancels that wait when disabled", async () => {
  const { feedback } = begin();
  const activation = feedback.activate(":1.23", "RUNE");
  ...
  const cancelled = next.activate(":1.23", "RUNE");
  next.dispose();
  await expect(cancelled).rejects.toThrow("No active capture");
  expect(shell.mapped.size).toBe(0);
});

it("does not animate under reduced motion but still activates RUNE", async () => {
...
it("rejects another sender and locked-session activation", async () => {
  shell.windows = [target()];
  const { feedback } = begin();
  await expect(feedback.activate(":1.99", "RUNE")).rejects.toThrow("No active capture");
  shell.locked = true;
  await expect(feedback.activate(":1.23", "RUNE")).rejects.toThrow("No active capture");
  expect(shell.activate).not.toHaveBeenCalled();
  feedback.dispose();
});
