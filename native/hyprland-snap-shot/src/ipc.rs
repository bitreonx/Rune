...
pub fn destination(windows: Vec<Window>, pid: u32, title: &str) -> Result<Option<Window>> {
    ...
    if matches.next().is_some() {
        return Err("More than one RUNE Code window matches the capture destination.".into());
    }
    Ok(found)
}

pub fn activate(pid: u32, title: &str) -> Result<()> {
    // Subscribe before looking up a newly mapped RUNE window, so no map/title event is missed.
    ...
    loop {
        ...
        if let Some(window) = destination(windows()?, pid, title)? {
            ...
            {
                return Err("Hyprland could not focus RUNE Code.".into());
            }
            return Ok(());
        }
        events.get_ref().set_read_timeout(Some(
            deadline
                ...
                .ok_or("RUNE Code did not become visible.")?,
        ))?;
...
mod tests {
    ...
    fn destination_requires_unique_process_and_title() {
        assert!(
            destination(vec![window(2, "RUNE")], 1, "RUNE")
                .unwrap()
                .is_none()
        );
        assert!(
            destination(vec![window(1, "other")], 1, "RUNE")
                .unwrap()
                .is_none()
        );
        assert!(destination(vec![window(1, "RUNE"), window(1, "RUNE")], 1, "RUNE").is_err());
        assert!(
            destination(vec![window(2, "RUNE"), window(1, "RUNE")], 1, "RUNE")
                .unwrap()
                .is_some()
        );
    }
