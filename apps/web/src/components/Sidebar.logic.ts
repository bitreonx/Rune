...
import type { SidebarProjectSortOrder, SidebarThreadSortOrder } from "@rune/contracts/settings";
import {
...
export { sortActiveThreadsByOrderKey as sortThreadsForSidebar } from "@rune/client-runtime/state/thread-sort";

/** Temporary chats stay out of the main sidebar stream: the partition memo
    sends everything this splits off to the Temporary shelf, and nothing else
    (counts, search, jump lists) may see it again. */
export function splitTemporarySidebarThreads<
  T extends { readonly temporaryAt: string | null | undefined },
>(threads: readonly T[]): { permanent: T[]; temporary: T[] } {
  const permanent: T[] = [];
  const temporary: T[] = [];
  for (const thread of threads) {
    if (thread.temporaryAt == null) {
      permanent.push(thread);
    } else {
...
export {
  ...
  pinOrderKeyBetween,
  planPinnedReorder,
} from "@rune/client-runtime/state/thread-sort";
export { sortPinnedThreadsByOrderKey as sortPinnedThreadsForSidebar } from "@rune/client-runtime/state/thread-sort";
...
/** The live plan step a working row can show next to its duration, or null
    when the turn carries no plan progress (row falls back to duration only).
    The server clears planProgress when the turn settles, so this never goes
    stale on a settled row. */
export function resolveWorkingStepLabel(
  thread: Pick<SidebarThreadSummary, "planProgress">,
): string | null {
  return thread.planProgress?.step ?? null;
}
...
>(
  ...
  for (const thread of threads) {
    ...
    // Hidden threads must not make a project look recently active either.
    if (thread.temporaryAt != null) continue;
