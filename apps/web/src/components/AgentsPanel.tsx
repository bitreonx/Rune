...
function findAgentById(model: AgentPanelModel, agentId: string): RuntimeSubagent | null {
  if (model.directAgents.some((agent) => agent.id === agentId)) {
  ...
  for (const group of model.workflows) {
    ...
    const members = workflowMembers(group);
    const match = members.find((agent) => agent.id === agentId);
        ...
        {activity.length > 0 ? (
          activity.map((entry) => (
            <div key={`${entry.at}:${entry.summary}`} className="flex gap-2 text-[11px] leading-relaxed">
              ...
              <p className="min-w-0 rounded-lg rounded-tl-sm bg-background/45 px-2 py-1 text-foreground/85">
                {entry.summary}
              </p>
            </div>
          ))
        ) : latestSummary ? (
          <p className="rounded-lg rounded-tl-sm bg-background/45 px-2 py-1 text-[11px] leading-relaxed text-foreground/85">
            {latestSummary}
          </p>
        ) : (
          <p className="text-[11px] text-muted-foreground">No activity has been recorded yet.</p>
        )}
      </div>
      <div className="flex flex-wrap items-center gap-x-3 gap-y-1 border-t border-border/45 px-3 py-2 font-mono text-[9px] text-muted-foreground/70">
        <span>{formatSubagentTokenCount(agent.usage?.totalTokens ?? 0)} tok</span>
        {agent.outputFile ? <span className="truncate">{agent.outputFile}</span> : null}
        {agent.runHandles?.sessionUrl ? (
          <a
            href={agent.runHandles.sessionUrl}
            target="_blank"
            rel="noreferrer"
            className="ml-auto inline-flex items-center gap-1 text-[var(--rune-violet-strong)] hover:underline"
          >
            Provider session
        ...
        <div className="flex flex-col gap-2 p-2">
          {model.workflows.map((group) => (
            <WorkflowSection
              ...
              threadId={threadId}
              focusedAgentId={focusedAgentId}
              onFocusAgent={(agentId) => onFocusAgent?.(agentId)}
            />
          ))}
