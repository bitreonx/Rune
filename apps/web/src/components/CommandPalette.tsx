...
import { scopeProjectRef, scopeThreadRef } from "@rune/client-runtime/environment";
import {
...
function OpenCommandPaletteDialog(props: {
  ...
  const keybindings = useAtomValue(primaryServerKeybindingsAtom);
  const { theme, themeHalves, resolvedTheme } = useTheme();
  const soundEffectsEnabled = useSoundPreferencesStore((state) => state.enabled);
  ...
  actionItems.push({
    kind: "action",
    value: "action:toggle-sound-effects",
    searchTerms: ["sound", "audio", "mute", "unmute", "effects", "sfx"],
    title: soundEffectsEnabled ? "Disable sound effects" : "Enable sound effects",
    description: soundEffectsEnabled ? "Sounds are on" : "Sounds are muted",
    icon:
      soundEffectsEnabled === false ? (
        <VolumeXIcon className={ITEM_ICON_CLASS} />
      ) : (
        <Volume2Icon className={ITEM_ICON_CLASS} />
      ),
    run: async () => {
      const store = useSoundPreferencesStore.getState();
  ...
  actionItems.push({
    ...
    searchTerms: ["schedules", "scheduled", "automations", "recurring", "tasks"],
    title: "Open schedules",
    description: "Manage RUNE scheduled tasks",
