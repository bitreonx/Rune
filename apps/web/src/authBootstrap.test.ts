...
function installTestBrowser(url: string) {
  ...
  vi.stubGlobal("document", { title: "RUNE" });
...
describe("resolveInitialServerAuthGateState", () => {
  ...
  it("retries transport failures while the desktop server is starting", async () => {
    vi.useFakeTimers();
    let attempts = 0;
    const request = HttpClientRequest.get("http://localhost/api/auth/session");
    const runner: PrimaryHttpEffectRunner = async <A>() => {
  ...
  it("retries when desktop bearer bootstrap wraps a transport failure", async () => {
    vi.useFakeTimers();
    let attempts = 0;
    const request = HttpClientRequest.get("http://localhost/api/auth/bootstrap/bearer");
    const runner: PrimaryHttpEffectRunner = async <A>() => {
      attempts += 1;
      if (attempts < 2) {
        throw {
          _tag: "DesktopLocalEnvironmentAuthSessionBootstrapError",
