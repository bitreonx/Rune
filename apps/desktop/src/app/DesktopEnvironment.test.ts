...
describe("DesktopEnvironment", () => {
  it.effect("derives state paths and development identity inside Effect", () =>
    Effect.gen(function* () {
      const environment = yield* makeEnvironment(
        ...
        {
          RUNE_HOME: " /tmp/rune ",
          RUNE_COMMIT_HASH: " 0123456789abcdef ",
          RUNE_PORT: "4949",
          VITE_DEV_SERVER_URL: "http://localhost:5173",
          RUNE_DEV_REMOTE_SERVER_ENTRY_PATH: " /remote/server.mjs ",
          RUNE_OTLP_TRACES_URL: " http://127.0.0.1:4318/v1/traces ",
          RUNE_OTLP_EXPORT_INTERVAL_MS: "2500",
        },
      );

      assert.equal(environment.isDevelopment, true);
      assert.equal(environment.appDataDirectory, "/Users/alice/Library/Application Support");
      assert.equal(environment.baseDir, "/tmp/rune");
      assert.equal(environment.stateDir, "/tmp/rune/userdata");
      assert.equal(environment.desktopSettingsPath, "/tmp/rune/userdata/desktop-settings.json");
      assert.equal(environment.clientSettingsPath, "/tmp/rune/userdata/client-settings.json");
      assert.equal(
        environment.savedEnvironmentRegistryPath,
        "/tmp/rune/userdata/saved-environments.json",
      );
      assert.equal(environment.serverSettingsPath, "/tmp/rune/userdata/settings.json");
      assert.equal(environment.logDir, "/tmp/rune/userdata/logs");
      assert.equal(environment.browserArtifactsDir, "/tmp/rune/userdata/browser-artifacts");
      assert.equal(environment.rootDir, "/repo");
      assert.equal(environment.appRoot, "/repo");
      assert.equal(environment.serverRoot, "/repo");
      assert.equal(environment.backendEntryPath, "/repo/apps/server/dist/bin.mjs");
      assert.equal(environment.backendCwd, "/repo");
      assert.equal(environment.appUserModelId, "dev.rune.desktop.dev");
      assert.equal(environment.linuxWmClass, "rune-dev");
      assert.deepEqual(
      ...
      assert.deepEqual(environment.branding, {
        baseName: "RUNE",
        stageLabel: "Dev",
        displayName: "RUNE (Dev)",
      });
      assert.equal(environment.displayName, "RUNE (Dev)");
    }),
  );
  ...
  it.effect("uses the stable desktop entry as the packaged Linux portal identity", () =>
    Effect.gen(function* () {
      const environment = yield* makeEnvironment({
        platform: "linux",
        isPackaged: true,
        appPath: "/tmp/.mount_rune/resources/app.asar",
        resourcesPath: "/tmp/.mount_rune/resources",
      });

      assert.equal(environment.linuxDesktopEntryName, "rune.desktop");
    }),
  );
  ...
  it.effect("keeps implicit development state separate from production state", () =>
    Effect.gen(function* () {
      ...
      assert.equal(development.stateDir, "/Users/alice/.rune/dev");
      assert.equal(production.stateDir, "/Users/alice/.rune/userdata");
    }),
  );
  ...
  it.effect("uses a configured app user model id override", () =>
    Effect.gen(function* () {
      const environment = yield* makeEnvironment(
        {},
        {
          RUNE_DESKTOP_APP_USER_MODEL_ID: " dev.rune.desktop.dev.local ",
          VITE_DEV_SERVER_URL: "http://localhost:5173",
        },
      );

      assert.equal(environment.appUserModelId, "dev.rune.desktop.dev.local");
    }),
  );
