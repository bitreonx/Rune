import type { PreviewViewportSetting } from "@rune/contracts";
...
/** Compatibility helper for callers that only have the currently rendered box. */
export function clampPreviewMiniPlayerSize(
  ...
  const aspectRatio =
    Number.isFinite(size.width) && Number.isFinite(size.height) && size.width > 0 && size.height > 0
      ...
      : PREVIEW_MINI_PLAYER_DEFAULT_SIZE.width / PREVIEW_MINI_PLAYER_DEFAULT_SIZE.height;
  const width = Math.min(
    Math.max(size.width, PREVIEW_MINI_PLAYER_MIN_SIZE.width),
    Math.max(1, available.width),
    Math.max(1, available.height * aspectRatio),
  );
  return { width: Math.round(width), height: Math.round(width / aspectRatio) };
}
