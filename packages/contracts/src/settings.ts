...
import { UsageLimitSourceConfig } from "./providerUsageLimits.ts";
...
/** Bounds for user-configurable workspace panel transitions. */
...
export const DEFAULT_PANEL_ANIMATION_DURATION_MS: PanelAnimationDurationMs = 200;
/**
...
export const ClientSettingsSchema = Schema.Struct({
  ...
  autoContinueAfterRestart: Schema.Boolean.pipe(Schema.withDecodingDefault(Effect.succeed(true))),
  ...
  // Activity presentation is client-local so the same server can support
  // technical and simplified viewers independently across devices.
  simplifiedActivity: Schema.Boolean.pipe(Schema.withDecodingDefault(Effect.succeed(true))),
  showDeveloperTrace: Schema.Boolean.pipe(Schema.withDecodingDefault(Effect.succeed(false))),
...
export const CodexSettings = makeProviderSettingsSchema(
  {
    ...
    shadowHomePath: TrimmedString.pipe(
      ...
      Schema.annotateKey({
        ...
        providerSettingsForm: {
          placeholder: "~/.codex-rune/personal",
          clearWhenEmpty: "omit",
        },
      }),
    ),
...
export const OpenCodeSettings = makeProviderSettingsSchema(
  {
    ...
    serverUrl: TrimmedString.pipe(
      ...
      Schema.annotateKey({
        title: "Server URL",
        description: "Leave blank to let RUNE spawn the server when needed.",
        providerSettingsForm: {
...
export const OpenAiApiSettings = makeProviderSettingsSchema(
  {
    enabled: Schema.Boolean.pipe(
      Schema.withDecodingDefault(Effect.succeed(true)),
      Schema.annotateKey({ providerSettingsForm: { hidden: true } }),
    ...
    baseUrl: TrimmedString.pipe(
      ...
      Schema.annotateKey({
        title: "API base URL",
        description: "OpenAI-compatible endpoint. Keep the /v1 path for the official API.",
        providerSettingsForm: {
  ...
  { order: ["baseUrl", "organization", "project"] },
);
export type OpenAiApiSettings = typeof OpenAiApiSettings.Type;
...
export const OpenRouterSettings = makeProviderSettingsSchema(
  {
    ...
    baseUrl: TrimmedString.pipe(
      ...
      Schema.annotateKey({
        title: "API base URL",
        description: "OpenRouter or another OpenAI-compatible gateway endpoint.",
        providerSettingsForm: {
    ...
    siteUrl: TrimmedString.pipe(
      Schema.withDecodingDefault(Effect.succeed("")),
      Schema.annotateKey({
        title: "Site URL",
        description: "Optional URL used by OpenRouter attribution headers.",
        providerSettingsForm: { clearWhenEmpty: "omit" },
      }),
    ),
    appName: TrimmedString.pipe(
      Schema.withDecodingDefault(Effect.succeed("RUNE")),
      Schema.annotateKey({
        title: "App name",
        description: "Optional application label shown in OpenRouter analytics.",
        providerSettingsForm: { clearWhenEmpty: "omit" },
      }),
    ),
    customModels: Schema.Array(Schema.String).pipe(
...
export const AntigravitySettings = makeProviderSettingsSchema(
  {
    ...
    gcpProject: TrimmedString.pipe(
      ...
      Schema.annotateKey({
        title: "GCP project",
        description: "Required for Gemini Enterprise. Agent Platform uses it when no API key is set.",
...
export const ServerSettings = Schema.Struct({
   ...
   * withholds the MCP credential, so the `rune-code` server (and with it every
  ...
  defaultTheme: Schema.String.pipe(Schema.withDecodingDefault(Effect.succeed(""))),
  /**
   * When the environment's theme was last set, so clients can tell a re-set
   * of the same value from one they already applied: `RUNE theme set` must act
  ...
  harnesses: HarnessesSettings.pipe(
    Schema.withDecodingDefault(Effect.succeed({ profiles: {}, services: {} })),
  ),
...
export const ServerSettingsPatch = Schema.Struct({
  ...
  harnesses: Schema.optionalKey(
    Schema.Struct({
      profiles: Schema.optionalKey(Schema.Record(ProfileId, HarnessProfileConfig)),
...
export const ClientSettingsPatch = Schema.Struct({
  ...
  autoContinueAfterRestart: Schema.optionalKey(Schema.Boolean),
  ...
  simplifiedActivity: Schema.optionalKey(Schema.Boolean),
  showDeveloperTrace: Schema.optionalKey(Schema.Boolean),
