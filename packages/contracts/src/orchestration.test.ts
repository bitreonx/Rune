...
it.effect("accepts a temporary flag on bootstrap.createThread and defaults it to absent", () =>
  Effect.gen(function* () {
    const baseBootstrap = {
      projectId: "project-1",
      title: "Temp thread",
    ...
    const temporary = yield* decodeThreadTurnStartCommand({
      type: "thread.turn.start",
      commandId: "cmd-turn-temp",
      threadId: "thread-1",
      message: {
        messageId: "msg-temp",
        role: "user",
      ...
      bootstrap: { createThread: { ...baseBootstrap, temporary: true } },
      createdAt: "2026-01-01T00:00:00.000Z",
    });
    const permanent = yield* decodeThreadTurnStartCommand({
      type: "thread.turn.start",
      commandId: "cmd-turn-permanent",
      threadId: "thread-1",
      message: {
      ...
      bootstrap: { createThread: baseBootstrap },
      createdAt: "2026-01-01T00:00:00.000Z",
    });

    assert.strictEqual(temporary.bootstrap?.createThread?.temporary, true);
    assert.strictEqual(permanent.bootstrap?.createThread?.temporary, undefined);
  }),
);

it.effect("decodes thread.temporary.set commands and rejects missing fields", () =>
  Effect.gen(function* () {
    const keep = yield* decodeClientOrchestrationCommand({
      type: "thread.temporary.set",
      commandId: "cmd-temp-keep",
      threadId: "thread-1",
      temporary: false,
    });

    assert.strictEqual(keep.type, "thread.temporary.set");

    const missingTemporary = yield* Effect.exit(
      decodeClientOrchestrationCommand({
        type: "thread.temporary.set",
        commandId: "cmd-temp-missing",
        threadId: "thread-1",
      }),
    );
    assert.strictEqual(missingTemporary._tag, "Failure");
  }),
);

it.effect("defaults temporaryAt to null when decoding historical thread data", () =>
  Effect.gen(function* () {
    const common = {
      id: "thread-1",
      projectId: "project-1",
    ...
    assert.strictEqual(thread.temporaryAt, null);
    assert.strictEqual(shell.temporaryAt, null);
  }),
);

it.effect("decodes thread.temporary-set events with a nullable temporaryAt", () =>
  Effect.gen(function* () {
    const flagged = yield* decodeOrchestrationEvent({
      sequence: 1,
      eventId: "event-temporary-1",
      aggregateKind: "thread",
      aggregateId: "thread-1",
      type: "thread.temporary-set",
      occurredAt: "2026-01-01T00:00:00.000Z",
      commandId: "cmd-temporary-1",
      causationEventId: null,
      correlationId: "cmd-temporary-1",
      metadata: {},
      payload: {
        threadId: "thread-1",
        temporaryAt: "2026-01-01T00:00:00.000Z",
        updatedAt: "2026-01-01T00:00:00.000Z",
      },
    });
    const cleared = yield* decodeOrchestrationEvent({
      sequence: 2,
      eventId: "event-temporary-2",
      aggregateKind: "thread",
      aggregateId: "thread-1",
      type: "thread.temporary-set",
      occurredAt: "2026-01-02T00:00:00.000Z",
      commandId: "cmd-temporary-2",
      causationEventId: null,
      correlationId: "cmd-temporary-2",
      metadata: {},
      payload: {
        threadId: "thread-1",
        temporaryAt: null,
    ...
    if (flagged.type !== "thread.temporary-set" || cleared.type !== "thread.temporary-set") {
      assert.fail("Expected thread.temporary-set events.");
    }
    assert.strictEqual(flagged.payload.temporaryAt, "2026-01-01T00:00:00.000Z");
    assert.strictEqual(cleared.payload.temporaryAt, null);
  }),
);
