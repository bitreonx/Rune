...
const remoteDebuggingPort = process.env.RUNE_DESKTOP_REMOTE_DEBUGGING_PORT?.trim();
// oxlint-disable-next-line rune/no-global-process-runtime -- Standalone dev script has no Effect runtime.
...
if (devProtocolClient) {
  childEnv.RUNE_DESKTOP_APP_USER_MODEL_ID = devProtocolClient.appBundleId;
  childEnv.RUNE_DESKTOP_PROTOCOL_REGISTRATION_MANAGED = "1";
}
...
function cleanupStaleDevApps() {
  ...
  NodeChildProcess.spawnSync("pkill", ["-f", "--", `--rune-dev-root=${desktopDir}`], {
    stdio: "ignore",
  });
}
...
function startApp() {
  ...
  const launchArgs = devProtocolClient
    ? electronArgs
    : [...electronArgs, `--rune-dev-root=${desktopDir}`, "dist-electron/main.cjs"];
