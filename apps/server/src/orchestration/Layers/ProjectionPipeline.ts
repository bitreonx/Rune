...
} from "@rune/shared/threadPullRequests";
  ...
  function* () {
    ...
    const applyThreadMessagesProjection: ProjectorDefinition["apply"] = Effect.fn(
      ...
      switch (event.type) {
        ...
        case "thread.message-sent": {
          // Hidden messages exist to drive a provider turn without ever being
          // user-visible, so they stay out of the SQLite projections that
          // power search and shell summaries.
          if (event.payload.hidden === true) {
            return;
          }
