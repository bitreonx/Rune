import { AuthStandardClientScopes, EnvironmentId } from "@rune/contracts";
...
function pairingHttpLayer(
  ...
  const fetchFn = ((input, init = {}) => {
    ...
    if (url.endsWith("/.well-known/rune/environment")) {
  ...
  it.effect("prepares a persisted bearer registration from pairing details", () =>
    Effect.gen(function* () {
      ...
      expect(calls.map((call) => call.url)).toEqual([
        "https://remote.example.test/.well-known/rune/environment",
      ...
      expect(tokenParams.get("client_label")).toBe("RUNE Test");
    }),
  );
