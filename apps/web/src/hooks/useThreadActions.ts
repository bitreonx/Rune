...
export function useThreadActions() {
  ...
  /** Converts a temporary thread to permanent: it re-enters the main sidebar
   *  lists and stops counting down toward auto-delete. Only reachable on
   *  threads the server already flagged, so no version-skew gate is needed. */
  const keepThread = useCallback(
    async (target: ScopedThreadRef) => {
      return setTemporaryThreadMutation({
        environmentId: target.environmentId,
        input: { threadId: target.threadId, temporary: false },
      });
    },
