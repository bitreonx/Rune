...
} from "@rune/client-runtime/state/providerInstanceDisplay";
export {
  normalizeProviderAccentColor,
  providerInstanceInitials,
  resolveProviderInstanceDisplayName,
...
export interface ProviderInstanceEntry {
  ...
  /**
   * Which upstream gateway service this Claude instance targets, derived from
   * its explicit instance environment (see `claudeServices.ts`). Absent for
   * plain Anthropic instances and non-Claude drivers.
   */
  readonly serviceBadge?: ClaudeServiceBadgeKind | undefined;
...
export interface InstanceBadgePresentation {
  readonly show: boolean;
  readonly content: "initials" | "logo";
  readonly logoUrl?: string | undefined;
}

/**
 * Shared badge decision for every surface that renders an instance icon —
 * composer trigger, picker rail, settings cards, edit page header. Instances
 * backed by OpenRouter show its artwork; everything else keeps the existing
 * initials decision from `shouldShowInstanceBadge`.
 */
export function instanceBadgePresentation(
  entry: ProviderInstanceEntry,
  entries: Iterable<ProviderInstanceEntry>,
): InstanceBadgePresentation {
  if (entry.serviceBadge === "openrouter") {
    return { show: true, content: "logo", logoUrl: OPENROUTER_LOGO_URL };
  }
  return { show: shouldShowInstanceBadge(entry, entries), content: "initials" };
}
