...
RUNE uses [Vite+](https://viteplus.dev/guide/). Install the global `vp` command, install
...
- `vp run dev --share`: Also publishes the web port over HTTPS on this machine's tailnet. The
  ...
  Shared runs default to Vite's bundled dev mode (`RUNE_BUNDLED_DEV=1`): a remote browser pays a
  network round trip per import level in unbundled dev, which turns a cold module graph into
  minutes of waterfall. Set `RUNE_BUNDLED_DEV=0` to opt a shared run back out.
- `vp run dev --browser`: Auto-opens a browser. Off by default. The dev runner writes
  `RUNE_NO_BROWSER` itself from this flag, so setting `RUNE_NO_BROWSER=0` in your environment has
...
- Pass dev-runner flags directly after the root task name, for example:
  `vp run dev --home-dir /tmp/rune-dev`
...
- From the **main checkout**, dev commands implicitly use `~/.rune/dev`, keeping development state
  separate from `~/.rune/userdata`. An explicit `--home-dir <path>` stores state under
  `<path>/userdata`; the base directory remains available for caches, worktrees, and other shared
  data.

## Build, check, test

- `vp run build`: Fans out over `apps/*`, `packages/*`, `oxlint-plugin-rune`, and `scripts`.
...
- Signed macOS builds also require `RUNE_APPLE_TEAM_ID` and
  `RUNE_MACOS_PROVISIONING_PROFILE`. The passkey RP domain is derived from
  `RUNE_CLERK_PUBLISHABLE_KEY` unless `RUNE_CLERK_PASSKEY_RP_DOMAINS` overrides it.
...
- Example: `RUNE_DEV_INSTANCE=branch-a vp run dev:desktop`

Offset resolution, in order:

1. `RUNE_PORT_OFFSET`, which must be a non-negative integer. Negative values are rejected.
2. `RUNE_DEV_INSTANCE`. An all-digit value is used directly as the offset; any other non-empty
