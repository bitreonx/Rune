 ...
 * The state directory is hashed so custom RUNE homes cannot exceed Unix socket
 * path limits.
 */
export function resolveDesktopAppControlAddress(input: {
  ...
  if (input.platform === "win32") {
    return {
      address: `\\\\.\\pipe\\rune-app-${stateHash}`,
      directory: null,
    };
  }
  ...
  const directory = input.joinPath(input.tempDir, `rune-${userKey}`);
  return {
