import { projectQuestionToolInput } from "@rune/shared/toolActivity";
import type {
  OrchestrationEvent,
...
function projectPreviewToolMetadata(data: Record<string, unknown>, status: unknown) {
  ...
  if (
    typeof name !== "string" ||
    !/^(?:mcp__)?(?:RUNE-code|rune_code|t3code)_{1,2}preview_(?:open|navigate|status|snapshot|click|type|press|scroll|resize|set_appearance|evaluate|wait_for|recording_start|recording_stop)$/.test(
      name,
    )
  )
    return {};
