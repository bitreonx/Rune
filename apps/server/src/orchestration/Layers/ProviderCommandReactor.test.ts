...
import { TextGenerationError } from "@rune/contracts";
...
describe("ProviderCommandReactor", () => {
  ...
  async function createHarness(input?: {
    ...
    const baseDir =
      input?.baseDir ?? NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "rune-reactor-"));
  ...
  it("starts the provider turn for a hidden message resolved from the command read model", async () => {
    // Hidden messages (the restart-recovery nudge) never reach the SQLite
    // projections, so the reactor must still find them in the in-memory
    // command read model to resume the provider session.
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        ...
        threadId: ThreadId.make("thread-1"),
        message: {
          messageId: asMessageId("user-message-hidden"),
          role: "user",
          text: "Continue.",
          attachments: [],
          hidden: true,
  ...
  it("pins the first user message when regeneration context is truncated", async () => {
    ...
    expect(message).not.toContain("rune-citation://");
  ...
  it("matches the client-seeded title even when the outgoing prompt is reformatted", async () => {
    ...
    expect(harness.generateThreadTitle.mock.calls[0]?.[0].message).not.toContain("rune-citation://");
    const readModel = await harness.readModel();
    const thread = readModel.threads.find((entry) => entry.id === ThreadId.make("thread-1"));
  ...
  it("generates a worktree branch name for the first turn", async () => {
    ...
    expect(harness.generateBranchName.mock.calls[0]?.[0].message).not.toContain("rune-citation://");
