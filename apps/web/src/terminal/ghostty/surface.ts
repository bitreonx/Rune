...
export interface GhosttyTerminalSurfaceOptions {
  ...
  /** Fired after any search-state transition: new query, navigation, or clear. */
  readonly onSearchChange?: (status: TerminalSearchStatus) => void;
  /** Fired when the viewport pins to or leaves the last page of scrollback. */
...
export class GhosttyTerminalSurface {
  ...
  /** Last value reported through onAtBottomChange; null until first computed. */
  private atBottom: boolean | null = null;
  ...
  static async create(
    ...
    input.className = "rune-ghostty-input";
  ...
  resetAndWrite(data: string): void {
    ...
    this.synchronizeMouseTrackingState();
    // A replayed buffer invalidates every absolute match position, so stale
    // highlights must not survive into the new content.
  ...
  /**
   * Scans the whole scrollback for the query and highlights every match. The
   * viewport pages through history from the top while the scrollbar offset is
   * saved, then settles back exactly where it was, so the scan is invisible to
   * the user apart from one synchronous pause proportional to buffer size.
   */
  search(query: string): number {
    ...
    const originalOffset = state?.offset ?? 0;
    const len = Math.max(1, state?.len ?? this.rows);
    const maxOffset = Math.max(0, (state?.total ?? this.rows) - len);
    const matches: TerminalSearchMatch[] = [];
    if (originalOffset > 0) this.core.scroll(-originalOffset);
    try {
      for (const offset of searchScanOffsets(state, len)) {
        const snapshot = this.core.snapshot();
        matches.push(
          ...findBufferMatches(
            snapshot.rowData.map((row) => row.cells),
            offset,
            query,
          ),
        );
        if (offset + len > maxOffset) break;
        this.core.scroll(len);
      }
    } finally {
      const settled = this.readScrollbarState();
      const restoreDelta = originalOffset - (settled?.offset ?? originalOffset);
      if (restoreDelta !== 0) this.core.scroll(restoreDelta);
    }
    this.searchMatches = matches;
    this.searchActiveIndex = initialActiveMatch(matches, originalOffset);
    this.forceFullRender = true;
    this.scrollbarDirty = true;
    this.requestRender();
  ...
  clearSearch(): void {
    if (this.disposed) return;
    const hadQuery = this.searchQuery.length > 0;
    this.searchQuery = "";
    this.resetSearchMatches();
    if (!hadQuery) return;
    this.forceFullRender = true;
  ...
  private resetSearchMatches(): void {
    const hadState =
      this.searchMatches.length > 0 || this.searchActiveIndex >= 0 || this.searchQuery.length > 0;
    this.searchMatches = [];
    this.searchActiveIndex = -1;
    if (hadState) this.emitSearchChange();
  }

  private stepSearch(direction: 1 | -1): boolean {
    if (this.disposed || this.searchMatches.length === 0) return false;
    const nextIndex = wrapMatchIndex(this.searchActiveIndex, this.searchMatches.length, direction);
    ...
    if (match) {
      ...
      const len = Math.max(1, state?.len ?? this.rows);
      const maxOffset = Math.max(0, (state?.total ?? this.rows) - len);
      const target = scrollOffsetForMatch({
        matchRow: match.row,
        offset: state?.offset ?? 0,
        viewportRows: len,
        maxOffset,
      });
      if (state && target !== state.offset) {
        this.core.scroll(target - state.offset);
        this.scrollbarState = { ...state, offset: target };
        this.forceFullRender = true;
        this.scrollbarDirty = true;
      }
    }
  ...
  /** Reports at-bottom transitions once, not per frame. */
  private emitAtBottom(atBottom: boolean): void {
    if (this.atBottom === atBottom) return;
    this.atBottom = atBottom;
    this.options.onAtBottomChange?.(atBottom);
  }
  ...
  private renderFrame(): void {
    ...
    this.emitAtBottom(scrollbarAtBottom(scrollState));
    ...
    renderGhosttySnapshot({
      ...
      ...(this.searchMatches.length > 0
        ? {
            searchRanges: viewportSearchRanges(
              this.searchMatches,
