...
import { buildRemoteNodeEnvScript } from "@rune/ssh/tunnel";
...
>()("@rune/desktop/wsl/DesktopWslEnvironment") {}
...
// that install wrote.
const WSL_RUNTIME_READY_MARKER = ".rune-wsl-runtime-ready";
const WSL_RUNTIME_SELECTED_MARKER = ".rune-wsl-runtime-selected";
...
export const buildWslRuntimeInstallScript = (
  ...
  return [
    "set -eu",
    'runtime_parent="$HOME/.rune/wsl-runtime"',
    ...
    '    [ -f "${candidate%/*}/rune-wsl-node-pty.json" ] || continue',
    "    return 0",
    "  done",
    "  return 1",
    "}",
...
export const buildWslRuntimeInvalidateScript = (runtimeId: string): string => {
  ...
  return [
    "set -eu",
    `rm -f "$HOME/.rune/wsl-runtime/${safeRuntimeId}/${WSL_RUNTIME_READY_MARKER}"`,
  ].join("\n");
};
...
const formatNodePtyProbeFailureReason = (exitCode: number): string | null =>
  exitCode === NODE_PTY_PREBUILD_MISSING_EXIT_CODE
    ? "WSL support is missing from this RUNE build: the packaged Linux node-pty binary was not included. Rebuild the Windows artifact with `--wsl-prebuild <path-to-linux-pty.node>` or install a build that includes WSL support."
    : null;
...
const marker = path.join(prebuildDir, "rune-wsl-node-pty.json");
...
const NODE_PTY_BUILD_SCRIPT = (linuxServerDir: string) =>
  [
    ...
    `printf '{"arch":"%s","modules":"%s","nodePtyVersion":"%s"}\\n' "$arch" "$modules" "$node_pty_version" > "$prebuild_dir/rune-wsl-node-pty.json"`,
