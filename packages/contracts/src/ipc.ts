...
import { SnapShotShortcut, type ClientSettings } from "./settings.ts";
...
export interface DesktopPreviewWebviewConfig {
  /** `persist:rune-preview` (or whatever the desktop chose). */
  partition: string;
  /**
...
export interface DesktopBridge {
  ...
  invalidateLocalEnvironmentBearerToken?: () => Promise<void>;
  ...
  /**
   * Absolute filesystem path for a renderer-picked File (webUtils wrapper).
   * Optional: older desktop builds lack it, and web callers have no local
  ...
  /** Present when the desktop shell accepts `RUNE app` activation requests. */
  appActivation?: {
...
export const DESKTOP_PREVIEW_RECORDING_CAPTURE_TRIGGER = "__runeDesktopPreviewRecordingCapture";
