...
export function buildThreadFeed(
  ...
  const allMessages = options?.localMessages
    ? [...loadedMessages, ...options.localMessages]
    : loadedMessages;
  // Hidden messages drive provider turns without ever being user-visible
  // (e.g. the restart-recovery nudge), so the feed skips them.
