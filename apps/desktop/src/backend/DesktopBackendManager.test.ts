...
describe("DesktopBackendManager", () => {
  it("uses bounded RUNE startup readiness deadlines", () => {
    assert.equal(DesktopBackendManager.DEFAULT_BACKEND_READINESS_TIMEOUT_MS, 30_000);
    assert.equal(DesktopBackendManager.DEFAULT_BACKEND_READINESS_PROBE_TIMEOUT_MS, 2_000);
  });
    ...
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        const readiness = yield* DesktopBackendManager.waitForHttpReady({
          ...
          probeTimeout: Duration.millis(25),
