...
The same configuration lives in `~/.rune/userdata/keybindings.json` on the machine running the
server, if you prefer editing it directly. RUNE writes the built-in defaults into that file on
...
`terminal.search` opens find-in-buffer for the focused terminal and defaults to `mod+f`; see
[Terminal](./terminal.md) for the search controls.
