import { EnvironmentId, SkillId, type SkillRegistrySnapshot, WS_METHODS } from "@rune/contracts";
  ...
  Effect.gen(function* () {
    ...
    const client = {
      [WS_METHODS.skillsList]: () => {
        calls.push(WS_METHODS.skillsList);
        return Effect.succeed(snapshot);
      },
      [WS_METHODS.skillsRefresh]: () => {
        calls.push(WS_METHODS.skillsRefresh);
        return Effect.succeed(snapshot);
      },
    ...
    const list = yield* listSkills().pipe(
      Effect.provideService(EnvironmentSupervisor.EnvironmentSupervisor, supervisor),
    );
    const refreshed = yield* refreshSkills().pipe(
      Effect.provideService(EnvironmentSupervisor.EnvironmentSupervisor, supervisor),
    );
    ...
    expect(calls).toEqual([
      WS_METHODS.skillsList,
      WS_METHODS.skillsRefresh,
      `${WS_METHODS.skillsGetBody}:hash:review`,
    ]);
  }),
);
...
it("preserves explicit-only semantics in the client projection", () => {
  expect(
    getAutoInvocableSkills([
      ...snapshot.skills,
      { ...snapshot.skills[0]!, id: SkillId.make("hash:explicit"), explicitOnly: true },
      { ...snapshot.skills[0]!, id: SkillId.make("hash:disabled"), enabled: false },
    ]).map((skill) => skill.id),
  ).toEqual(["hash:review"]);
});
