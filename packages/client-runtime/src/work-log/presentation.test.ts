...
describe("resolveWorkEntryToolPresentation", () => {
  it.each([
    "mcp__rune-code__preview_click",
    "mcp__rune_code__preview_click",
    "mcp__t3code__preview_click",
    "RUNE-code.preview_click",
    "RUNE-code · preview_click completed",
    "rune_code/preview_click",
    "preview_click",
  ...
  it("labels device tools with the device icon", () => {
    expect(
      resolveWorkEntryToolPresentation({
        label: "mcp__rune-code__device_open",
        toolLifecycleStatus: "completed",
      }),
    ).toEqual({ displayName: "Opened a device in the Device panel", icon: "device" });
    expect(resolveWorkEntryToolPresentation({ label: "RUNE-code · device_screenshot" })).toEqual({
  ...
  it("uses structured MCP identity when the provider supplies a custom title", () => {
    expect(
      resolveWorkEntryToolPresentation({
        ...
        toolData: { server: "RUNE-code", tool: "preview_snapshot", result: { title: "Example" } },
      }),
  ...
  it("uses the summary's state only when the provider omitted a lifecycle status", () => {
    const entry = { label: "RUNE-code.preview_click" };
  ...
  it.each([
    ...
    ["t3_thread_read", "Reading a RUNE thread", "Read a RUNE thread"],
    ["t3_thread_send", "Sending to a RUNE thread", "Sent to a RUNE thread"],
    [
      "t3_worktree_handoff",
    ...
    const entry = { label: `RUNE-code.${tool}` };
    expect(
  ...
  it("keeps RUNE branding for non-browser tools and falls back to the original tool label", () => {
    expect(
      resolveWorkEntryToolPresentation({
        label: "mcp__rune_code__task_status",
        toolTitle: "Check the child task",
      }),
    ).toEqual({ displayName: "Getting delegated task status", icon: "RUNE-code" });
  });

  it("does not brand unknown tools or another server's matching tool name", () => {
    for (const label of [
      "mcp__github__preview_click",
      "RUNE-code.unknown_tool",
      "RUNE-code.toString",
      "Search files",
    ]) {
...
describe("browser group summaries", () => {
  const browserEntry: WorkLogPresentationEntry = {
    label: "MCP tool call",
    toolData: { server: "RUNE-code", tool: "preview_click" },
  ...
  it("preserves first-seen action ordering alongside non-browser tools", () => {
    expect(
      summarizeToolGroup([
        ...
        {
          ...browserEntry,
          toolData: { server: "RUNE-code", tool: "task_status" },
        },
      ]),
      ...
      summarizeToolGroup([
        {
          ...
          toolData: { toolName: "mcp__rune_code__preview_evaluate" },
        },
      ]),
...
describe("resolveViewedImageAsset", () => {
  ...
  it("serves RUNE attachment paths in place like any other host path", () => {
    const path = "/Users/demo/.rune/dev/attachments/11111111-1111-4111-8111-111111111111.png";
...
describe("pull request tool presentation", () => {
  it.each([
    "mcp__rune-code__link_pull_request",
    "mcp__rune_code__link_pull_request",
    "RUNE-code · link_pull_request",
  ...
  it("recognizes unlink targets supplied as repository and number", () => {
    expect(
      resolveWorkEntryToolPresentation({
        ...
        toolLifecycleStatus: "completed",
        toolData: {
          toolName: "mcp__rune-code__unlink_pull_request",
  ...
  it("summarizes native PR work separately from ordinary tools and integration metadata", () => {
    const link: WorkLogPresentationEntry = {
      label: "RUNE-code · link_pull_request",
      tone: "tool",
      itemType: "mcp_tool_call",
      toolLifecycleStatus: "completed",
      toolSource: { key: "RUNE-code", name: "RUNE Code", kind: "integration" },
    };
    const list: WorkLogPresentationEntry = {
      ...link,
      label: "RUNE-code · list_thread_pull_requests",
    };
    expect(summarizeToolGroup([link, link, list])).toBe(
      "Linked 2 pull requests and checked linked pull requests",
    );
    expect(summarizeToolGroup([{ ...link, label: "RUNE-code · unlink_pull_request" }])).toBe(
...
describe("device group summaries", () => {
  ...
  it("recognizes Claude tool names and preserves screenshot previews", () => {
    const entry = {
      ...
      toolData: { toolName: "mcp__rune_code__device_screenshot" },
  ...
  it("does not classify another server's tools as RUNE device controls", () => {
    expect(
      summarizeToolGroup([
        {
          ...deviceEntry("device_open"),
