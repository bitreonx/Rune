...
provider instance, so it applies to every project using that provider on the same T3 Code server.

Choose one of these services:

- **Anthropic**: leave the base URL blank and use Claude Code's normal login, or enter an
  `ANTHROPIC_API_KEY`.
- **OpenRouter**: T3 Code fills in `https://openrouter.ai/api` and uses your OpenRouter key as the
  compatible service token.
- **Custom compatible service**: enter the gateway's base URL and API key or token.

T3 Code stores credentials as server-side secrets. The key is never written to the project
...
Add or edit a Claude provider in T3 Code Settings, then expand its card:
...
When the selected model is a custom model, T3 Code pins Claude Code's `opus`, `sonnet`, and `haiku`
