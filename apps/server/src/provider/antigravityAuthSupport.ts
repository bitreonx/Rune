...
import type { AntigravityAuthMethod, ProviderInstanceId } from "@rune/contracts";
import { HostProcessExecutablePath, HostProcessPlatform } from "@rune/shared/hostProcess";
...
const browserHelperSource =
  ...
  `()=>process.exit(0))`;
const browserPreflightUrl = "https://example.invalid/RUNE-antigravity-browser-preflight";
...
/** Recognizes native auth failures and interactive login blocked by RUNE. */
...
/**
 * The agent reads its user-global skills under `GEMINI_HOME`, which RUNE points
...
export const prepareAntigravityProfile = Effect.fn("prepareAntigravityProfile")(function* (input: {
  ...
  if (
    ...
    return yield* authSupportError(
      "The RUNE runtime path cannot be used to suppress Antigravity browser launches.",
    );
  }
...
/** Receives native 1.1.1 sign-in URLs and RUNE browser-helper URLs without logging stderr. */
