...
describe("FileSaveCoordinator", () => {
  ...
  it("flush() persists immediately instead of waiting out the debounce", async () => {
    vi.useFakeTimers();
    const persist = vi
      .fn<(contents: string) => Promise<AtomCommandResult<void, never>>>()
      .mockResolvedValue(AsyncResult.success(undefined));
    ...
    coordinator.change("urgent");
    coordinator.flush();

    expect(persist).toHaveBeenCalledOnce();
    expect(persist).toHaveBeenCalledWith("urgent");

    // The scheduled autosave must not fire a second write afterwards.
