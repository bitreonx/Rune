...
} from "@rune/client-runtime/state/attachments";
import { runAtomCommand, squashAtomCommandFailure } from "@rune/client-runtime/state/runtime";
import type {
  ChatFileAttachment,
  ChatImageAttachment,
  EnvironmentId,
...
import { PROVIDER_SEND_TURN_SUPPORTED_IMAGE_MIME_TYPES } from "@rune/contracts";
...
async function uploadFileBytes(
  ...
  const file =
    fileUri === undefined
      ? new File(Paths.cache, `RUNE-upload-${uuidv4()}`)
