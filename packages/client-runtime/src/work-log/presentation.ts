...
import { parseChangeRequestUrl } from "@rune/shared/changeRequestUrl";
...
const RUNE_MCP_TOOL_LABELS: Record<
  string,
...
> = {
  ...
  create_threads: ["Create", "Creating", "Created", "RUNE threads"],
  t3_thread_start: ["Start", "Starting", "Started", "a RUNE thread"],
  t3_thread_list: ["List", "Listing", "Listed", "RUNE threads"],
  t3_thread_read: ["Read", "Reading", "Read", "a RUNE thread"],
  t3_thread_send: ["Send", "Sending", "Sent", "to a RUNE thread"],
  t3_thread_wait: ["Wait", "Waiting", "Waited", "for a RUNE thread"],
  t3_thread_interrupt: ["Interrupt", "Interrupting", "Interrupted", "a RUNE thread"],
...
function resolveRuneMcpToolPresentation(
  ...
  const name = normalizeCompactToolLabel(value).replace(
    // Accept the legacy T3 identifier so imported historical work logs still
    // render correctly, while all newly emitted RUNE entries use RUNE-code.
    /^(?:mcp__(?:RUNE-code|rune_code|t3code)__|(?:RUNE-code|rune_code|t3code)(?:[.:/]|\s*·\s*))/i,
    "",
  );
  if (!Object.hasOwn(RUNE_MCP_TOOL_LABELS, name)) return null;

  const [action, running, completed, detail] = RUNE_MCP_TOOL_LABELS[name]!;
  const verb =
    status === "inProgress"
      ? running
      : status === "completed"
...
export function resolveWorkEntryToolPresentation(
  ...
  if (data !== null && typeof data === "object") {
    if (
      ...
      return resolveRuneMcpToolPresentation(`${data.server}.${data.tool}`, status, data);
    }
    if ("toolName" in data && typeof data.toolName === "string") {
      return resolveRuneMcpToolPresentation(data.toolName, status, data);
    }
  }

  return (
    resolveRuneMcpToolPresentation(entry.toolTitle, status, data) ??
    resolveRuneMcpToolPresentation(entry.label, status, data)
  );
}
