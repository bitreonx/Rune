...
describe("ElectronWindow", () => {
    ...
    Effect.gen(function* () {
      ...
      const options = {
        ...
        webPreferences: {
          ...
          partition: "persist:rune-preview-test",
          sandbox: true,
      ...
      assert.equal(error.message, 'Failed to create Electron BrowserWindow "RUNE" (1100x780).');
  ...
  it.effect("focuses the exact RUNE window before activating from a shell-hosted app", () =>
    Effect.gen(function* () {
      const operations: Array<string> = [];
  ...
  it.effect("prepares the exact RUNE window before a capture overlay", () =>
    Effect.gen(function* () {
