...
describe("fileBreadcrumbs", () => {
  it("builds project, directory, and file crumbs", () => {
    expect(fileBreadcrumbs("rune", "apps/web/src/main.tsx")).toEqual([
      { label: "rune", path: "", kind: "project" },
  ...
  it("starts host paths outside the workspace at the filesystem root", () => {
    expect(fileBreadcrumbs("t3code", "/tmp/RUNE-cleanup/report.md")).toEqual([
      { label: "tmp", path: "/tmp", kind: "directory" },
      { label: "RUNE-cleanup", path: "/tmp/RUNE-cleanup", kind: "directory" },
      { label: "report.md", path: "/tmp/RUNE-cleanup/report.md", kind: "file" },
    ]);
