...
describe("grokAcpSpawnArgs", () => {
  it("inherits the Grok CLI config when no RUNE runtime mode is set", () => {
    expect(grokAcpSpawnArgs()).toEqual(["agent", "stdio"]);
  });

  it("forces Grok to ask when RUNE is Supervised", () => {
...
describe("buildGrokAcpSpawnInput", () => {
  it("passes the RUNE referrer through Grok OAuth env", () => {
