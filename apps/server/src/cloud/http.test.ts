...
import { RelayClientTracer } from "@rune/shared/relayTracing";
...
import type { RelayLinkProofRequest } from "@rune/contracts/relay";
  ...
  it.effect("does not accept an unauthenticated request trace parent", () =>
    Effect.gen(function* () {
      ...
      const request = HttpServerRequest.fromWeb(
        new Request("https://environment.example.test/api/rune-connect/mint-credential", {
          headers: {
...
describe("releaseManagedTunnelOnShutdown", () => {
  ...
  const provideReleaseHarness =
    ...
    <A, E, R>(effect: Effect.Effect<A, E, R>) =>
      effect.pipe(
        ...
        Effect.provide(
          ServerConfigModule.layerTest("/", { prefix: "rune-http-release-test-" }).pipe(
  ...
  it.effect("still releases a pending update when the launcher is stopping", () => {
    // `rune service uninstall` or `systemctl stop` during the pending window:
...
describe("link proof provider kinds", () => {
  ...
  it("accepts managed and manual endpoints but not rune_relay", () => {
    ...
    expect(isSupportedLinkProviderKind(proofRequest("rune_relay"))).toBe(false);
  });
