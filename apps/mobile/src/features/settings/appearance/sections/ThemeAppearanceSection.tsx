...
import { mixThemePreviewBase, THEME_PREVIEW_RENDER_SPECS } from "@rune/shared/themePreview";
  ...
  return (
    <View className="min-w-36 flex-1 basis-[47%] gap-3 rounded-[24px] border border-border bg-card px-2 py-4">
      ...
      <View className="min-h-8 flex-row items-center" pointerEvents="none">
        <Text className="min-w-0 flex-1 text-lg font-rune-medium" numberOfLines={1}>
          {props.label}
        </Text>
      </View>
    </View>
  ...
  return (
    ...
    >
      ...
      <Text
        className={
          props.selected
            ? "text-center text-base font-rune-bold text-foreground"
...
function SectionLabel({ children }: { readonly children: string }) {
  return <Text className="px-2 text-sm font-rune-medium text-foreground-muted">{children}</Text>;
}
