...
import { dedupeRemoteBranchesWithLocalMatches, normalizeGitRemoteUrl } from "@rune/shared/git";
import { compactTraceAttributes } from "@rune/shared/observability";
import { decodeJsonResult } from "@rune/shared/schemaJson";
...
> {
  ...
  const traceFilePath = yield* fs.makeTempFileScoped({
    prefix: `rune-git-trace2-${process.pid}-`,
    suffix: ".json",
  });
    ...
    Effect.fn("refreshCheckedOutBranch")(function* (input) {
      ...
      if (!isAncestor) {
        ...
        yield* executeGit(
          ...
          input.cwd,
          ["update-ref", "refs/rune/pre-refresh", headCommit],
