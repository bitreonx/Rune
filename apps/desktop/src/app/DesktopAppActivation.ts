...
import { HostProcessUserId } from "@rune/shared/hostProcess";
  ...
  const server = NodeNet.createServer((socket) => {
    ...
    socket.on("data", (chunk) => {
      ...
      void input.handle(parsed).then(finish, () => {
        finish(
          invalidResponse(parsed.requestId, "RUNE Code could not process the desktop app request."),
        );
      });
    });
...
>()("@rune/desktop/app/DesktopAppActivation") {}
