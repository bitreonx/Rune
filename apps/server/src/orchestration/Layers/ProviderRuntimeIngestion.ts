...
const make = Effect.gen(function* () {
  ...
  const bufferedAssistantTextByMessageId = yield* Cache.make<
    MessageId,
    { text: string; lastFlushAtMs: number }
  >({
    capacity: BUFFERED_MESSAGE_TEXT_BY_MESSAGE_ID_CACHE_CAPACITY,
    timeToLive: BUFFERED_MESSAGE_TEXT_BY_MESSAGE_ID_TTL,
    lookup: () => Effect.succeed({ text: "", lastFlushAtMs: 0 }),
  });
  ...
  // Buffered mode keeps the dispatch rate low without hiding progress: text
  // becomes visible immediately, then flushes at most once per interval until
  // the message finalizes. Timestamps come from provider event time so the
  // cadence is reproducible.
  const appendBufferedAssistantText = (messageId: MessageId, delta: string, nowMs: number) =>
    Cache.getOption(bufferedAssistantTextByMessageId, messageId).pipe(
      Effect.flatMap((existingEntry) =>
        Effect.gen(function* () {
          ...
          // Whitespace-only buffers stay held back so they never materialize
          // as their own assistant segment; they ride along with the next
          // renderable flush instead.
          if (
            Number.isFinite(nowMs) &&
            nowMs - lastFlushAtMs >= BUFFERED_ASSISTANT_FLUSH_INTERVAL_MS &&
    ...
    Cache.getOption(bufferedAssistantTextByMessageId, messageId).pipe(
      Effect.flatMap((existingEntry) =>
        Cache.invalidate(bufferedAssistantTextByMessageId, messageId).pipe(
          Effect.as(Option.getOrElse(existingEntry, () => ({ text: "", lastFlushAtMs: 0 })).text),
        ),
      ),
    );
    ...
    Effect.gen(function* () {
      ...
      if (assistantDelta && assistantDelta.length > 0) {
        ...
        if (assistantDeliveryMode === "buffered") {
          const visibleChunk = yield* appendBufferedAssistantText(
            assistantMessageId,
            assistantDelta,
            Date.parse(now),
          );
