...
export const HYPRLAND_CAPTURE_EXECUTABLE = "RUNE-hyprland-snap-shot";
...
export function hyprlandCaptureExecutable(paths: HyprlandCapturePaths) {
  return NodePath.join(paths.dataHome, "rune", "hyprland-capture", HYPRLAND_CAPTURE_EXECUTABLE);
}
...
export class HyprlandCaptureSetup {
  ...
  async state(): Promise<DesktopCaptureHelperState> {
    try {
      ...
      if (!bundle)
        throw new Error(
          "The Hyprland capture helper is missing from this build. Update or reinstall RUNE Code.",
        );
...
export async function captureHyprlandWindow(
  ...
  const directory = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "RUNE-hyprland-capture-"));
