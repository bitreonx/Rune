...
describe("ComposerBannerStack", () => {
  it("keeps expanded banners in layout flow so surrounding content moves out of their way", () => {
    ...
    // Tight rhythm between stacked banners so the pile stays close to the seam.
    const innerStack = markup.match(/class="([^"]*\binvisible pointer-events-none[^"]*)"/);
    expect(innerStack?.[1]).toContain("space-y-1.5 pb-1.5");
  });
