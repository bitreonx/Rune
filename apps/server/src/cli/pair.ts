/**
 * `rune pair` - mint a pairing token for an already-running server and print it
...
const WELL_KNOWN_ENVIRONMENT_PATH = "/.well-known/rune/environment";
...
export class NoRunningServerError extends Schema.TaggedErrorClass<NoRunningServerError>()(
  ...
  override get message(): string {
    return [
      "No running RUNE server found.",
      ...this.checkedStatePaths.map((statePath) => `  checked ${statePath}`),
      "Start one with `npx rune serve`, or connect this machine with RUNE Connect: `npx rune connect`.",
    ].join("\n");
  }
}
...
export class ServesOtherEnvironmentError extends Schema.TaggedErrorClass<ServesOtherEnvironmentError>()(
  ...
  override get message(): string {
    return `Tailscale Serve on HTTPS port ${String(this.servePort)} already fronts a different RUNE server. Pass --tailscale-serve-port to publish this one on another port.`;
  }
}
...
export class ServePortOccupiedError extends Schema.TaggedErrorClass<ServePortOccupiedError>()(
  ...
  override get message(): string {
    return `HTTPS port ${String(this.servePort)} on the tailnet already serves something that is not a RUNE server. Pass --tailscale-serve-port to publish this one on another port.`;
  }
}
...
/**
 * Three outcomes, because they drive different decisions: a RUNE descriptor
 * (pair with it), nothing answering (safe to configure Tailscale Serve), or
 * something answering that is not a RUNE server (do NOT overwrite its mapping).
 */
type EnvironmentProbeResult =
  | { readonly _tag: "descriptor"; readonly descriptor: ExecutionEnvironmentDescriptor }
  | { readonly _tag: "unreachable" }
  | { readonly _tag: "not-a-rune-server" };
...
const probeEnvironmentDescriptor = (
  ...
  Effect.gen(function* () {
    ...
    // it as unreachable lets `rune pair --tailscale` repair its own mapping
    ...
    const descriptor = yield* HttpClientResponse.filterStatusOk(response).pipe(
      ...
      Effect.mapError(() => ({ _tag: "not-a-rune-server" }) as const),
    );
...
const discoverPairTarget = Effect.fn("pair.discoverPairTarget")(function* (
  ...
  if (explicitBaseDir !== undefined && explicitBaseDir.trim().length > 0) {
    ...
    const envHome = yield* Config.string("RUNE_HOME").pipe(Config.option);
...
const resolveTailscalePairingBase = Effect.fn("pair.resolveTailscalePairingBase")(
  function* (input: { readonly target: DiscoveredPairTarget; readonly servePort: number }) {
    ...
    // environment, is safe to (re)configure. Any other responder — RUNE or not
    ...
    if (existing._tag === "not-a-rune-server") {
...
const mintPairingLink = Effect.fn("pair.mintPairingLink")(function* (input: {
  ...
  return yield* Effect.gen(function* () {
    ...
    return yield* environmentAuth.createPairingLink({
      ...
      label: Option.getOrElse(input.label, () => "rune pair"),
...
export const pairCommand = Command.make("pair", {
  ...
  Command.withDescription(
    "Mint a pairing token for a running RUNE server and print it as a QR code.",
  ),
