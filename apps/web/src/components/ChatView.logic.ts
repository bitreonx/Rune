...
export const LAST_INVOKED_SCRIPT_BY_PROJECT_KEY = "rune:last-invoked-script-by-project";
 ...
 * conversation are restored to before the prompt, and edit additionally hands
 * the prompt text back to the composer. "turn" is the same pipeline triggered
 * from a turn's changed-files card.
 */
export function buildUserMessageRewindConfirmation(
  mode: UserMessageRewindMode,
  turnCount: number,
): { message: string; options: { variant: "destructive" } } {
  if (mode === "turn") {
    return buildTurnRevertConfirmation(turnCount);
  }
  const header = `Rewind this thread to before this prompt (checkpoint ${turnCount})?`;
  const message =
    mode === "edit"
      ? [
          header,
...
/**
 * Reverting from a turn's changed-files card reuses the checkpoint revert
 * pipeline: files and conversation are restored to before the turn.
 */
export function buildTurnRevertConfirmation(turnCount: number): {
  message: string;
  options: { variant: "destructive" };
} {
  const message = [
    `Undo this turn (checkpoint ${turnCount})?`,
