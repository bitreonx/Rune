...
> =>
  Effect.flatMap(Effect.service(FileSystem.FileSystem), (fs) =>
    Effect.gen(function* () {
      ...
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "rune-terminal-" });
...
it.layer(
  ...
  it.effect("attaches to exited sessions without restarting them", () =>
    Effect.gen(function* () {
      ...
      const unsubscribe = yield* manager.attachStream(
        openInput({
          env: {
            RUNE_WORKTREE_PATH: "/tmp/should-not-restart",
          },
      ...
      const unsubscribe = yield* manager.attachStream(
        {
          ...openInput({
            env: {
              RUNE_WORKTREE_PATH: "/tmp/restart-requested",
            },
  ...
  it.effect("filters app runtime env variables from terminal sessions", () =>
    Effect.gen(function* () {
      ...
      expect(spawnInput.env.RUNE_PORT).toBeUndefined();
  ...
  it.effect("injects runtime env overrides into spawned terminals", () =>
    Effect.gen(function* () {
      ...
      yield* manager.open(
        openInput({
          env: {
            RUNE_PROJECT_ROOT: "/repo",
            RUNE_WORKTREE_PATH: "/repo/worktree-a",
            CUSTOM_FLAG: "1",
          },
        }),
      );
      ...
      assert.equal(spawnInput.env.RUNE_PROJECT_ROOT, "/repo");
      assert.equal(spawnInput.env.RUNE_WORKTREE_PATH, "/repo/worktree-a");
    ...
    Effect.gen(function* () {
      ...
      Effect.provide(
        ServerSettings.layer.pipe(
          ...
          Layer.provide(
            ServerConfig.layerTest(process.cwd(), { prefix: "rune-terminal-provider-restart-" }),
          ),
        ),
      ),
    ),
