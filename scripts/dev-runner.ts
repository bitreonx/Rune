...
export const DEFAULT_RUNE_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(NodeOS.homedir(), ".rune"),
);
...
/**
 * `role` matters because only the backend honours `--host`/`RUNE_HOST`; the
...
export class DevRunnerInvalidPortOffsetError extends Schema.TaggedErrorClass<DevRunnerInvalidPortOffsetError>()(
  ...
  {
    configKey: Schema.Literal("RUNE_PORT_OFFSET"),
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("RUNE_PORT_OFFSET"),
  devInstance: optionalStringConfig("RUNE_DEV_INSTANCE"),
});
...
> {
  if (config.portOffset !== undefined) {
    ...
    return Effect.succeed({
      ...
      source: `RUNE_PORT_OFFSET=${config.portOffset}`,
    });
  }
  ...
  if (seed) {
    if (/^\d+$/.test(seed)) {
      return Effect.succeed({
        offset: Number(seed),
        source: `numeric RUNE_DEV_INSTANCE=${seed}`,
      });
    }

    const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
    return Effect.succeed({ offset, source: `hashed RUNE_DEV_INSTANCE=${seed}` });
  }
...
function resolveBaseDir(baseDir: string | undefined): Effect.Effect<string, never, Path.Path> {
  return Effect.gen(function* () {
    ...
    return yield* DEFAULT_RUNE_HOME;
  });
}
...
interface CreateDevRunnerEnvInput {
  ...
  readonly runeHome: string | undefined;
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    // by the caller; an unset runeHome here genuinely means "use the default".
    const configuredBaseDir = runeHome?.trim() || undefined;
    ...
    if (!isDesktopMode) {
      ...
      if (mode === "dev" || mode === "dev:web") {
        ...
        output.RUNE_SINGLE_ORIGIN_DEV = "1";
      } else {
        output.VITE_HTTP_URL = `http://localhost:${serverPort}`;
        output.VITE_WS_URL = `ws://localhost:${serverPort}`;
        delete output.RUNE_SINGLE_ORIGIN_DEV;
      }
    } else {
      output.RUNE_PORT = String(serverPort);
      ...
      delete output.RUNE_SINGLE_ORIGIN_DEV;
      delete output.RUNE_MODE;
      delete output.RUNE_NO_BROWSER;
      delete output.RUNE_HOST;
    }

    if (!isDesktopMode && host !== undefined) {
      output.RUNE_HOST = host;
    }

    if (!isDesktopMode) {
      output.RUNE_NO_BROWSER = browser === true ? "0" : "1";
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.RUNE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.RUNE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.RUNE_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.RUNE_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.RUNE_MODE = "web";
      delete output.RUNE_DESKTOP_WS_URL;
    }
 ...
 * target reach it there. When `--host`/`RUNE_HOST` moves the backend onto
  ...
  return Effect.gen(function* () {
      ...
      Effect.mapError(
        (cause) =>
          new DevRunnerConfigurationError({
            configKeys: ["RUNE_PORT_OFFSET", "RUNE_DEV_INSTANCE"],
            cause,
          }),
      ),
    );
    ...
    const selectionSuffix =
      serverOffset !== offset || webOffset !== offset
        ...
        : "";
    const baseDir = env.RUNE_HOME ?? (yield* DEFAULT_RUNE_HOME);

    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.RUNE_PORT)} webPort=${String(env.PORT)} baseDir=${baseDir}`,
    );
    ...
    if (input.share) {
      if (input.mode === "dev:server") {
        ...
        if (shared) {
          ...
          // a few chunk requests. Only defaulted, so RUNE_BUNDLED_DEV=0
          // still opts a --share run back out.
          if (env.RUNE_BUNDLED_DEV === undefined) {
            env.RUNE_BUNDLED_DEV = "1";
          }
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  runeHome: Flag.string("home-dir").pipe(
    Flag.withDescription(
      "Explicit RUNE data directory; runtime state is stored under userdata (equivalent to RUNE_HOME). Inside a git worktree this defaults to that worktree's own .rune so dev state stays off the shared home.",
    ),
    Flag.optional,
  ...
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to RUNE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("RUNE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to RUNE_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("RUNE_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to RUNE_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("RUNE_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to RUNE_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("RUNE_PORT")),
  ),
