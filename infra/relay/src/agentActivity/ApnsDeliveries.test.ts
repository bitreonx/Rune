    ...
    return Effect.gen(function* () {
      ...
      Effect.provide(
        makeLayer({
          attempts,
          currentTargets: [
            { ...target, bundle_id: "com.runetools.t3code.preview", aps_environment: "sandbox" },
          ],
    ...
    for (const changed of ["bundle", "environment", "legacy"] as const) {
      it.effect(`routes ${kind} using current registration with ${changed} job metadata`, () => {
        ...
        const payload = makeApnsDeliveryJobPayload({
          ...
          ...(changed === "legacy"
            ? {}
            : { bundleId: "com.runetools.t3code.dev", apsEnvironment: "sandbox" as const }),
        ...
        return Effect.gen(function* () {
          ...
          expect(requests[0]?.headers["apns-topic"]).toBe(
            `${changed === "bundle" ? "com.runetools.t3code.preview" : "com.runetools.t3code.dev"}${kind === "live_activity_update" ? ".push-type.liveactivity" : ""}`,
          );
        }).pipe(
          Effect.provide(
            makeLayer({
              ...
              currentTargets: [
                {
                  ...
                  bundle_id:
                    changed === "bundle" ? "com.runetools.t3code.preview" : "com.runetools.t3code.dev",
