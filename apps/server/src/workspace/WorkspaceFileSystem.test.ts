...
const TestLayer = Layer.empty.pipe(
  ...
  Layer.provide(
    ServerConfig.ServerConfig.layerTest(process.cwd(), {
      prefix: "rune-workspace-files-test-",
    }),
  ),
...
const runGit = Effect.fn("runGit")(function* (cwd: string, args: ReadonlyArray<string>) {
  // A failed setup command is a broken test, not a handled outcome.
  yield* Effect.promise(() => execFile("git", [...args], { cwd, windowsHide: true }));
});

const commitAll = Effect.fn("commitAll")(function* (cwd: string) {
  yield* runGit(cwd, ["add", "."]);
  yield* runGit(cwd, ["commit", "-m", "test snapshot"]);
});
...
it.layer(TestLayer, { excludeTestServices: true })("WorkspaceFileSystemLive", (it) => {
  ...
  describe("readFileAtHead", () => {
    it.effect("reads committed contents for a tracked file modified in place", () =>
      Effect.gen(function* () {
        const workspaceFileSystem = yield* WorkspaceFileSystem.WorkspaceFileSystem;
        const cwd = yield* makeTempDir;
        ...
        const result = yield* workspaceFileSystem.readFileAtHead({
          cwd,
          relativePath: "src/index.ts",
        });
    ...
    it.effect("reports untracked files as absent from HEAD", () =>
      Effect.gen(function* () {
        const workspaceFileSystem = yield* WorkspaceFileSystem.WorkspaceFileSystem;
        const cwd = yield* makeTempDir;
        yield* runGit(cwd, ["init"]);
    ...
    it.effect("reports an unborn repository as having no HEAD contents", () =>
      Effect.gen(function* () {
        const workspaceFileSystem = yield* WorkspaceFileSystem.WorkspaceFileSystem;
        const cwd = yield* makeTempDir;
        yield* runGit(cwd, ["init"]);
    ...
    it.effect("fails when the workspace root is not a git repository", () =>
      Effect.gen(function* () {
        const workspaceFileSystem = yield* WorkspaceFileSystem.WorkspaceFileSystem;
        const cwd = yield* makeTempDir;
        yield* writeTextFile(cwd, "src/index.ts", "export {};\n");
        ...
        const error = yield* workspaceFileSystem
          .readFileAtHead({ cwd, relativePath: "src/index.ts" })
          .pipe(Effect.flip);
        if (error._tag !== "WorkspaceHeadReadError") {
          return yield* Effect.die(`Expected WorkspaceHeadReadError, got ${error._tag}`);
        }

        expect(error.reason).toBe("not_git_repository");
      }),
    );

    it.effect("rejects binary blobs stored in HEAD", () =>
      Effect.gen(function* () {
        const workspaceFileSystem = yield* WorkspaceFileSystem.WorkspaceFileSystem;
        const fileSystem = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        ...
        const error = yield* workspaceFileSystem
          .readFileAtHead({ cwd, relativePath: "blob.bin" })
          .pipe(Effect.flip);
    ...
    it.effect("marks oversized HEAD blobs as truncated", () =>
      Effect.gen(function* () {
        const workspaceFileSystem = yield* WorkspaceFileSystem.WorkspaceFileSystem;
        const cwd = yield* makeTempDir;
        yield* runGit(cwd, ["init"]);
        ...
        const result = yield* workspaceFileSystem.readFileAtHead({ cwd, relativePath: "big.txt" });

        expect(result.truncated).toBe(true);
        expect(result.presentInHead).toBe(true);
        expect(result.contents.length).toBeLessThan(blobSize);
      }),
    );
  });
});
