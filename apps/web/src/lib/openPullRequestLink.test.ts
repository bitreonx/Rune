...
describe("openPullRequestLink", () => {
  it("opens the requested pull request URL", async () => {
    ...
    const targetUrl = "https://github.com/rune-dev/rune/pull/123";
  ...
  it("reports bridge failures with a safe target origin", async () => {
    ...
    const targetUrl = "https://github.com/rune-dev/rune/pull/123?token=secret";
...
describe("parseChangeRequestUrl", () => {
  ...
  it("reads a GitLab merge request, nested groups and all", () => {
    expect(
      parseChangeRequestUrl("https://gitlab.com/rune-dev/platform/rune/-/merge_requests/42"),
    ).toEqual({
      host: "gitlab.com",
      repository: "rune-dev/platform/rune",
      number: 42,
    });
  });
  ...
  it("reads both Azure DevOps URL forms, keeping `_git` in the repository path", () => {
    expect(
      parseChangeRequestUrl("https://dev.azure.com/acme/platform/_git/rune/pullrequest/17"),
    ).toEqual({
      host: "dev.azure.com",
      repository: "acme/platform/_git/rune",
      number: 17,
    });
    expect(
      parseChangeRequestUrl("https://acme.visualstudio.com/platform/_git/rune/pullrequest/17"),
    ).toEqual({
      host: "acme.visualstudio.com",
      repository: "platform/_git/rune",
      number: 17,
    });
  });

  it("survives trailing segments, a trailing slash and a query string", () => {
    expect(parseChangeRequestUrl("https://github.com/rune-dev/rune/pull/123/files?w=1")).toEqual({
      host: "github.com",
      repository: "rune-dev/rune",
      number: 123,
    });
    ...
    expect(parseChangeRequestUrl("https://github.com/rune-dev/rune/pull/123/")).toEqual({
      host: "github.com",
      repository: "rune-dev/rune",
      number: 123,
    });
  ...
  it("claims nothing it cannot be sure of, so the link goes to the browser", () => {
    for (const link of [
      "https://github.com/rune-dev/rune/issues/123",
      "https://github.com/rune-dev/rune/commit/0a1b2c3",
      "https://github.com/rune-dev/rune",
      "https://github.com/rune-dev/rune/pull/abc",
      "https://gitlab.com/rune-dev/rune/-/snippets/12",
      "https://gitlab.com/rune-dev/rune/-/issues/12",
      ...
      "javascript:alert(1)//github.com/rune-dev/rune/pull/1",
      "not a url",
    ]) {
...
describe("findProjectForChangeRequest", () => {
  ...
  it("matches a nested GitLab group by the whole path below the host", () => {
    ...
    // two-segment owner/name form would look for `rune-dev/rune` and find nothing.
    const projects = [
      project({
        canonicalKey: "gitlab.com/rune-dev/platform/rune",
        provider: "gitlab",
        displayName: "rune-dev/platform/rune",
        owner: "rune-dev",
        name: "rune",
      }),
    ];
