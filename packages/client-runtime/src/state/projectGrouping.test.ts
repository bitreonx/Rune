import { EnvironmentId, ProjectId } from "@rune/contracts";
...
const repositoryIdentity = {
  ...
  locator: {
    ...
    remoteName: "upstream",
    remoteUrl: "https://github.com/rune-dev/rune.git",
  },
  provider: "github",
  owner: "rune-dev",
  name: "rune",
...
describe("buildProjectGroups", () => {
  it("preserves every physical clone as a selectable member in repository modes", () => {
    const projects = [
      makeProject("rune", "/work/rune"),
      makeProject("rune-2", "/work/rune-2"),
      makeProject("rune-3", "/work/rune-3"),
    ];
  ...
  it("uses a shared custom title as the repository group's label", () => {
    const projects = [
      makeProject("first", "/work/rune", { title: "Custom project" }),
      makeProject("second", "/work/rune-2", { title: "Custom project" }),
    ];
  ...
  it("keeps the repository label when shared titles match its repository name", () => {
    const projects = [
      makeProject("first", "/work/rune", { title: "rune" }),
      makeProject("second", "/work/rune-2", { title: "rune" }),
    ];
  ...
  it("keeps physical clones in separate groups when requested", () => {
    ...
    expect(groups.map((group) => group.label)).toEqual(["rune", "rune-2", "rune-3"]);
  });

  it("applies a physical-project override without dropping its siblings", () => {
    const first = makeProject("rune", "/work/rune");
    const second = makeProject("rune-2", "/work/rune-2");
    const third = makeProject("rune-3", "/work/rune-3");
  ...
  it("dedupes stale registrations at one physical path using the freshest project", () => {
    const stale = makeProject("stale", "/work/rune", {
      repositoryIdentity: null,
      updatedAt: "2026-07-01T00:00:00.000Z",
    });
    const fresh = makeProject("fresh", "/work/rune/", {
  ...
  it("uses repository identity from a duplicate registration when the winner lacks it", () => {
    const identified = makeProject("identified", "/work/rune", {
      updatedAt: "2026-07-01T00:00:00.000Z",
    });
    const freshUnidentified = makeProject("fresh", "/work/rune/", {
      repositoryIdentity: null,
      updatedAt: "2026-07-02T00:00:00.000Z",
    });
    const sibling = makeProject("sibling", "/work/rune-2");
  ...
  it("uses the freshest winner's repository identity when stale duplicates disagree", () => {
    const staleIdentity = {
      ...repositoryIdentity,
      canonicalKey: "github.com/rune-dev/old-repository",
      name: "old-repository",
  ...
  it("uses the freshest identity-bearing duplicate when the winner lacks identity", () => {
    ...
    const staleIdentified = makeProject("stale-identified", "/work/rune", {
      repositoryIdentity: staleIdentity,
      updatedAt: "2026-07-01T00:00:00.000Z",
    });
    const freshIdentified = makeProject("fresh-identified", "/work/rune/", {
      updatedAt: "2026-07-02T00:00:00.000Z",
    });
    const winner = makeProject("winner", "/work/rune", {
      repositoryIdentity: null,
