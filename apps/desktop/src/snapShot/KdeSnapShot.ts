...
import type { DesktopCaptureHelperState } from "@rune/contracts";
...
export const KDE_CAPTURE_EXECUTABLE = "RUNE-kde-snap-shot";
const DESKTOP_FILE = "com.runetools.Rune.KdeCapture.desktop";
const MARKER = "X-rune-Capture-Helper=true";
...
export function kdeCapturePaths(paths: KdeCapturePaths) {
  return {
    executable: NodePath.join(paths.dataHome, "rune", "kde-capture", KDE_CAPTURE_EXECUTABLE),
...
export class KdeCaptureSetup {
  ...
  async state(): Promise<DesktopCaptureHelperState> {
    try {
      ...
      if (!bundle)
        return {
          status: "error",
          message: "The capture helper is missing from this build. Update or reinstall RUNE Code.",
        };
  ...
  async perform(action: "install-kde-helper" | "remove-kde-helper") {
    ...
    if (action === "remove-kde-helper") {
      ...
      if (!bundle)
        throw new Error(
          "The capture helper is missing from this build. Update or reinstall RUNE Code.",
        );
...
export async function captureKdeWindow(
  ...
  const directory = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "RUNE-kde-capture-"));
  let retained = false;
      ...
      feedback: {
        ...
        activate: async (title) => {
          ...
          const activation = await NodeFSP.mkdtemp(
            NodePath.join(NodeOS.tmpdir(), "RUNE-kde-activate-"),
          );
          try {
            await run(
              executable,
