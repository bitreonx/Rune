...
version="${tag#v}"
pkgver="${version//-/_}"
asset_name="RUNE-Code-${version}-x86_64.AppImage"
...
cd "$aur_dir"
git rm --ignore-unmatch LICENSE .upstream-commit rune-icon.png
git config user.name 'rune-ci'
git config user.email 'rune-ci@users.noreply.github.com'
git add -A
