...
// session that did not survive a server restart. Clients match on this exact
// string to offer the one-click Continue recovery.
export const ORPHANED_PROVIDER_SESSION_ERROR =
  "Provider session did not survive a server restart. Send a new message to continue.";

// Invisible nudge text sent by that recovery — it reaches the provider CLI so
// the resumed session picks up where it left off, while clients hide the
// message itself from timelines, previews, and search.
export const CONTINUE_AFTER_RESTART_NUDGE = "Continue.";
...
export const OrchestrationMessage = Schema.Struct({
  ...
  // nudge behind the error-banner Continue action.
...
/**
 * Legacy single-PR link retained in the thread snapshot for clients that
 * predate the multi-link pull request model.
 */
...
/** Who created a thread ↔ pull request link. */
...
/** A host-native stack the pull request belongs to. */
...
/** Identity of a pull request as a thread link sees it. */
...
// "Keep this chat" for temporary threads. The decider stamps temporaryAt from
// its clock — clients express intent, never timestamps.
const ThreadTemporarySetCommand = Schema.Struct({
...
const ThreadTurnStartBootstrapCreateThread = Schema.Struct({
  ...
  branch: Schema.NullOr(TrimmedNonEmptyString),
  worktreePath: Schema.NullOr(TrimmedNonEmptyString),
  // Temporary chats ride the first turn's bootstrap so the thread is born
  // flagged — no visible-then-hidden window between create and flag.
  temporary: Schema.optional(Schema.Boolean),
  createdAt: IsoDateTime,
});
...
export const ThreadTemporarySetPayload = Schema.Struct({
  threadId: ThreadId,
...
export class OrchestrationAgentChatError extends Schema.TaggedError<OrchestrationAgentChatError>()(
