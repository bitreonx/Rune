...
import { resolveThreadCurrentPullRequestLink } from "@rune/shared/threadPullRequests";
...
const SidebarProjectItem = memo(function SidebarProjectItem(props: SidebarProjectItemProps) {
  ...
  // Temporary chats live only on the v2 Temporary shelf; the legacy sidebar
  // never shows them.
  const unfilteredProjectThreads = useThreadShellsForProjectRefs(project.memberProjectRefs);
  const sidebarThreads = useMemo(
    () => unfilteredProjectThreads.filter((thread) => thread.temporaryAt == null),
    [unfilteredProjectThreads],
  );
...
export default function LegacySidebar() {
  ...
  // Same contract as the v2 sidebar: temporary chats are reachable by link
  // but never listed here.
  const unfilteredSidebarThreads = useThreadShells();
  const sidebarThreads = useMemo(
    () => unfilteredSidebarThreads.filter((thread) => thread.temporaryAt == null),
