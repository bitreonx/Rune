...
describe("resolveNewTaskLocalWorkspaceSelection", () => {
  ...
  it("carries the worktree path when the current branch lives in another worktree", () => {
    expect(
      resolveNewTaskLocalWorkspaceSelection({
        branches: [
          { name: "feature/split", current: true, worktreePath: "/repo/.rune/worktrees/split" },
      ...
      worktreePath: "/repo/.rune/worktrees/split",
      awaitsCurrentBranch: false,
    });
  });
});
  ...
  it("moves Current checkout to the selected existing worktree", () => {
    expect(
      resolveNewTaskBranchWorktreePath({
        workspaceMode: "local",
        projectCwd: "/repo",
        branchWorktreePath: "/repo/.rune/worktrees/feature",
      }),
    ).toBe("/repo/.rune/worktrees/feature");
  });
