...
Use this when you want to connect to a RUNE server from another device such as a phone, tablet, or separate desktop app.
...
`rune pair` finds the running server (the shared `~/.rune` install, or the current worktree's dev server when run inside one), issues a one-time pairing token, and prints the pairing URL as a QR code you can scan from your phone.
...
If no server is running, `rune pair` says so and points you at `npx rune serve` or `npx rune connect`.
...
run a headless server from the CLI, or have the desktop app launch RUNE over SSH.
...
- HTTPS/WSS-compatible endpoints work from `https://app.rune.codes`, but are not made the default
  automatically.
...
If the copied link points directly at `http://192.168.x.y:3773`, open it from a client that can reach that LAN address. If it points at `https://app.rune.codes/pair?...`, the hosted web app will save the environment and connect directly to the backend URL in the link.
...
backend with the same server-side behavior as `rune serve --tailscale-serve`, then the server asks
...
For `https://app.rune.codes`, prefer an HTTPS Tailnet or other HTTPS endpoint. A plain `http://100.x.y.z:3773` endpoint can still work from a desktop client or another browser page served over HTTP, but it will not work from the hosted HTTPS app because of browser mixed-content rules.
...
Run the server with `rune serve`.

```bash
npx rune serve --host "$(tailscale ip -4)"
```

`rune serve` starts the server without opening a browser and prints:
...
Use `rune serve --help` for the full flag reference. It supports the same general startup options as the normal server command, including an optional `cwd` argument.

For hosted web pairing over Tailscale HTTPS, opt in to Tailscale Serve:

```bash
npx rune serve --tailscale-serve
```
...
```bash
npx rune serve --tailscale-serve --tailscale-serve-port 8443
```
...
Use this when you want the desktop app to start or reuse RUNE on another machine over SSH.
...
5. Confirm the launch. The desktop app probes the host, starts or reuses a remote RUNE server, opens a local port forward, and saves the environment.

After setup, the renderer connects to a local forwarded HTTP/WebSocket endpoint. The remote host still owns the actual RUNE server, projects, files, git state, terminals, and provider sessions.
...
The desktop SSH launcher connects with a non-interactive `sh` session, writes a small launcher script under `~/.rune/ssh-launch/<host-key>/`, starts or reuses a remote RUNE server, and forwards the remote loopback port back to your desktop.

The remote host must have a compatible Node.js runtime. RUNE uses the server package's `engines.node` requirement:

```text
^22.16 || ^23.11 || >=24.10
```

During SSH launch, RUNE first checks whether `node` is on `PATH`. If it is missing, the launcher
...
If launch fails with `node: command not found`, a port-scan failure, or a message that the remote Node version does not satisfy the required range, SSH into the host and check the same non-interactive shell path RUNE uses:
...
If reconnecting after an app update fails, retry the SSH launch once. The launcher now compares its generated runner script, stops stale launcher-managed remote servers, clears the SSH launch PID/port state, and starts a fresh remote server. You should not normally need to delete `~/.rune/ssh-launch` or kill `rune` processes manually.

## Updating a Remote Server

When the RUNE web or desktop app and a remote server use different versions, a warning appears in
the conversation and in **Settings** → **Connections**. Follow the action shown there: RUNE may
...
see [Keeping RUNE in Sync](./updating.md).

On a Linux host, you can keep the server running after logout and manage it independently of the
connection method. See [Running RUNE in the Background](./background-service.md).
...
1. `rune serve` issues a one-time owner pairing token.
...
The hosted web app at `https://app.rune.codes` can save a remote backend in browser local storage from a URL like:
...
Hosted pairing does not proxy traffic through RUNE. The browser still connects directly to the backend URL in the pairing link.

## Managing Access Later

Use `rune auth` to manage access after the initial pairing flow.
...
Use `rune auth --help` and the nested subcommand help pages for the full reference.

### Deregister a RUNE Connect Environment

Open your account menu and choose **RUNE Connect** to see every environment registered to your
account. On mobile, open **Settings** → **RUNE Connect**. Choose **Deregister** to revoke an
environment's RUNE Connect access, remove any managed tunnel, and free its host space.
...
- Use `rune auth` to revoke credentials or sessions you no longer trust.
