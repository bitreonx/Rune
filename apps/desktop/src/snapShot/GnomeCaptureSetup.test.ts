...
beforeEach(async () => {
  dataHome = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "RUNE-gnome-setup-test-"));
});
afterEach(async () => {
...
it("preserves the replaced extension as a recoverable backup", async () => {
  ...
  await installGnomeCaptureBundle({ bundle, dataHome });
  const backups = NodePath.join(dataHome, "rune/extension-backups");
...
it("does not offer host extension installation in another desktop or sandbox", () => {
  ...
  expect(isGnomeCaptureSession({ XDG_CURRENT_DESKTOP: "GNOME", FLATPAK_ID: "com.rune" })).toBe(false);
});
