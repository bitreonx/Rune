...
import type { DesktopSshEnvironmentTarget, DesktopUpdateChannel } from "@rune/contracts";
...
const PUBLISHABLE_RUNE_VERSION_PATTERN = /^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/u;
...
export function resolveRemoteT3CliPackageSpec(input: {
  ...
  if (!input.isDevelopment && PUBLISHABLE_RUNE_VERSION_PATTERN.test(appVersion)) {
    return `rune@${appVersion}`;
  }
  ...
  return input.updateChannel === "nightly" ? "rune@nightly" : "rune@latest";
}
