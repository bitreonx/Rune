      ...
      {showTerminalControl ? (
        <Tooltip>
          ...
          <TooltipPopup side="bottom">
            {terminalAvailable
              ? `${terminalOpen ? "Minimize" : "Expand"} Console${terminalShortcutLabel ? ` (${terminalShortcutLabel})` : ""}`
      ...
      <Tooltip>
        <TooltipTrigger render={<span className="flex shrink-0" />}>
          <Toggle
            ...
            aria-label={
              liveAgentCount > 0
                ? `${rightPanelOpen ? "Minimize" : "Expand"} right panel, ${liveAgentCount} ${liveAgentCount === 1 ? "agent" : "agents"} working`
                : `${rightPanelOpen ? "Minimize" : "Expand"} right panel`
            }
            variant="ghost"
            size="sm"
        ...
        <TooltipPopup side="bottom">
          {rightPanelAvailable
            ? `${rightPanelOpen ? "Minimize" : "Expand"} right panel${rightPanelShortcutLabel ? ` (${rightPanelShortcutLabel})` : ""}${
                liveAgentCount > 0
