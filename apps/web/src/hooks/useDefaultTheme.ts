...
/**
 * Scoped per environment: each machine's `RUNE theme set` is its own act, so
 * hopping between primary environments neither replays one environment's
 * theme over the user's pick nor swallows another's.
 */
const APPLIED_DEFAULT_THEME_STORAGE_PREFIX = "rune:default-theme-applied:v2:";
...
/**
 * Applies the environment's theme (`RUNE theme set <id>`). Each set switches
