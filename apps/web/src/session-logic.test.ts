...
import { resolveWorkEntryToolPresentation } from "@rune/client-runtime/work-log/presentation";
...
describe("deriveWorkLogEntries", () => {
  ...
  it("preserves MCP server, tool, arguments, and results for expanded display", () => {
    ...
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "mcp-tool-done",
        kind: "tool.completed",
        summary: "rune-code · preview_status",
        payload: {
          itemType: "mcp_tool_call",
          title: "rune-code · preview_status",
          data: { item },
        },
      }),
    ];
    ...
    const [entry] = deriveWorkLogEntries(activities);
    expect(entry?.toolTitle).toBe("rune-code · preview_status");
  ...
  it.each([
    ...
    (status, displayName) => {
      const data = {
        toolName: "mcp__rune_code__preview_click",
  ...
  it("keeps MCP payloads while collapsing lifecycle updates", () => {
    ...
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        ...
        kind: "tool.updated",
        summary: "rune-code · preview_snapshot",
        payload: {
          itemType: "mcp_tool_call",
          toolCallId: "call-1",
          toolSurface: "browser",
  ...
  it("extracts referenced file paths from read-tool inputs without marking them changed", () => {
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "read-file",
        kind: "tool.completed",
        ...
        payload: {
          ...
          data: {
            item: {
              input: {
                file_path: "apps/web/src/components/settings/ProviderSettingsPanel.tsx",
              },
            },
          },
        },
...
describe("deriveTimelineEntries", () => {
  ...
  it("excludes hidden messages from the timeline", () => {
    const entries = deriveTimelineEntries(
      [
        {
    ...
    expect(entries.map((entry) => entry.id)).toEqual([
      MessageId.make("message-visible"),
