# Keeping RUNE in Sync

The RUNE web or desktop app and the server it connects to work best when they use the same
version. If they do not match, RUNE shows a warning with the right update option for that server.
...
| Action                     | What to do                                                                                                                                                                  |
| -------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Update server**          | Available for the RUNE Linux background service. Select the button and leave RUNE open while it prepares, tests, restarts, and reconnects.                            |
| **Update the desktop app** | Open the RUNE desktop app on the machine that runs the server and install the app update there. Reopen it if needed.                                                     |
| **Copy update command**    | Copy the command, open a terminal on the server machine, stop the current RUNE server, and relaunch it with the copied command and any startup options you normally use. |

The available action depends on how that server was started. RUNE does not update connected
servers silently in the background.

An older background-service launcher may ask you to run the exact
`npx rune@<version> service update` command on the server machine. That one local update installs the
...
**Copy update command** gives you `npx rune@<client-version>`, which relaunches the server directly
at the matching version. Add whatever startup options you normally use.

If the server instead runs as the RUNE background service, update the service on the host and
pin the same version:

```sh
npx rune@<client-version> service update
```

`service update` installs the version of the CLI that invoked it, so `npx rune@latest service update`
...
See [Running RUNE in the Background](./background-service.md) for install, status, and removal
commands.
...
3. For a command-line server, relaunch it with `npx rune@<client-version>`, replacing
