...
const TestWorkspaceEntriesLive = WorkspaceEntries.layer.pipe(Layer.provide(WorkspacePaths.layer));
const TestWorkspaceFileSystemLive = WorkspaceFileSystem.layer.pipe(
  Layer.provide(WorkspacePaths.layer),
  Layer.provide(TestWorkspaceEntriesLive),
  Layer.provideMerge(Layer.mock(GitVcsDriver.GitVcsDriver)({})),
);
const TestApiToolServicesLive = Layer.mergeAll(
  TestWorkspaceEntriesLive,
  TestWorkspaceFileSystemLive,
...
it.layer(
  ...
  describe("ProviderRegistryLive", () => {
    ...
    it.effect("propagates real Codex probe failures to the aggregator at boot", () =>
      Effect.gen(function* () {
        ...
        const runtimeServices = yield* Layer.build(
          Layer.merge(providerRegistryLayer, TestApiToolServicesLive),
        ).pipe(Scope.provide(scope));
    ...
    it.effect("keeps cursor disabled and skips probing when the provider setting is disabled", () =>
      Effect.gen(function* () {
        ...
        yield* Effect.gen(function* () {
          ...
          assert.strictEqual(cursorProvider?.message, "Cursor is disabled in RUNE settings.");
  ...
  describe("checkClaudeProviderStatus", () => {
    ...
    it.effect("runs Claude status probes with the configured CLAUDE_CONFIG_DIR", () => {
      const claudeConfigDir = "/tmp/rune-claude-home";
