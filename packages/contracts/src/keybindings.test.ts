...
it.effect("lists each static command once", () =>
  Effect.sync(() => {
    const unique = new Set(STATIC_KEYBINDING_COMMANDS);
    assert.strictEqual(unique.size, STATIC_KEYBINDING_COMMANDS.length);
  }),
);
