...
describe("terminalLinkAtColumn", () => {
  it("maps terminal cells to UTF-16 offsets after a wide emoji", () => {
    const cells = [
      cell("🙂"),
      cell(""),
      ...Array.from("https://rune.dev", (character) => cell(character)),
    ];
    ...
    expect(terminalLinkAtColumn(row, 2)).toBe("https://rune.dev");
    expect(terminalLinkAtColumn(row, cells.length - 1)).toBe("https://rune.dev");
  ...
  it("refuses links truncated at the viewport edges instead of mis-resolving", () => {
    ...
    const tailCut = [row("https://rune.dev", false, true)];
    expect(terminalLinkAtPosition(tailCut, 0, 8)).toBeNull();
    // A partial bottom row is provably complete and still resolves.
    const complete = [row("https://rune.dev", false), row("", false)];
    expect(terminalLinkAtPosition(complete, 0, 8)).toBe("https://rune.dev");
    ...
    expect(terminalLinkAtPosition([unwrittenTail], 0, 8)).toBe("https://rune.dev");
  });
});
