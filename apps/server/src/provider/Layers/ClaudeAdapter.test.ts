...
describe("ClaudeAdapterLive", () => {
  ...
  it.effect("pins Claude model aliases to the selected custom model", () => {
    ...
    return Effect.gen(function* () {
      ...
      // Gateway accounts can't necessarily serve the first-party models the
      // CLI's aliases resolve to, so every alias must point at the custom model.
  ...
  it.effect("lets manually configured alias overrides outrank the custom-model pins", () => {
    const harness = makeHarness({
      claudeConfig: { customModels: ["stealth/ox-alpha"] },
      environment: { ANTHROPIC_DEFAULT_HAIKU_MODEL: "anthropic/claude-haiku-4.5" },
    });
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      yield* adapter.startSession({
      ...
      const env = harness.getLastCreateQueryInput()?.options.env;
      assert.equal(env?.ANTHROPIC_DEFAULT_HAIKU_MODEL, "anthropic/claude-haiku-4.5");
      // Pins still apply to the roles the user did not configure.
      assert.equal(env?.ANTHROPIC_DEFAULT_SONNET_MODEL, "stealth/ox-alpha");
    }).pipe(
      Effect.provideService(Random.Random, makeDeterministicRandomService()),
      Effect.provide(harness.layer),
