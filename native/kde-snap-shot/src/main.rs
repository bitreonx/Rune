...
#[zbus::interface(name = "com.runetools.KdeCapture")]
impl ScriptReply {
    fn reply(
        &self,
        value: &str,
...
fn script(connection: &Connection, directory: &Path, body: &str) -> Result<String> {
    ...
    let name = format!("RUNE-capture-{}", std::process::id());
    ...
    let source = format!(
        "function reply(value) {{ callDBus({destination}, '/com/runetools/KdeCapture', 'com.runetools.KdeCapture', 'Reply', JSON.stringify(value)); }}\ntry {{ {body} }} catch (error) {{ reply({{error: String(error)}}); }}"
    );
    ...
    connection
        .object_server()
        .remove::<ScriptReply, _>("/com/runetools/KdeCapture")?;
...
fn check(connection: &Connection) -> Result<()> {
    ...
    let reply: zbus::Result<HashMap<String, OwnedValue>> = proxy.call(
        "CaptureWindow",
        &(
            "RUNE-permission-check-not-a-window",
    ...
    match args.get(1).map(String::as_str) {
        ...
        Some("activate") if args.len() == 5 => {
            ...
            if serde_json::from_str::<serde_json::Value>(&value)?["activated"] != true {
                return Err("KDE could not identify the RUNE Code window to activate".into());
            }
