...
describe("buildUserMessageRewindConfirmation", () => {
  it("tells the edit flow the prompt returns to the composer", () => {
    expect(buildUserMessageRewindConfirmation("edit", 2)).toEqual({
      message: [
        "Rewind this thread to before this prompt (checkpoint 2)?",
        "Newer messages and file changes are discarded, and this action cannot be undone.",
        "Your prompt returns to the composer so you can change and resend it.",
      ].join("\n"),
      options: { variant: "destructive" },
    });
  });

  it("tells the delete flow the prompt itself is removed", () => {
    expect(buildUserMessageRewindConfirmation("delete", 0)).toEqual({
      message: [
        "Rewind this thread to before this prompt (checkpoint 0)?",
...
describe("buildTurnRevertConfirmation", () => {
  it("warns that the turn's changes and everything after it are discarded", () => {
    expect(buildTurnRevertConfirmation(3)).toEqual({
      message: [
        "Undo this turn (checkpoint 3)?",
        "Files go back to how they were before this turn.",
        "Newer messages and file changes are discarded. This action cannot be undone.",
      ].join("\n"),
      options: { variant: "destructive" },
    });
  });

  it("is the confirmation the rewind pipeline uses for turn-card reverts", () => {
    expect(buildUserMessageRewindConfirmation("turn", 3)).toEqual(buildTurnRevertConfirmation(3));
  });
});
